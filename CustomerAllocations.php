	...
	if ($InputError==0) {
		...
		foreach ($_SESSION['Alloc']->Allocs as $AllocnItem) {
			...
			if ($AllocnItem->AllocAmt > 0) {
				...
				$SQL = "SELECT type FROM debtortrans WHERE id=".$AllocnItem->ID;
				$typeOfTrans = mysqli_fetch_assoc(mysqli_query($db, $SQL))['type'];

				$totalAmountToPay = $AllocnItem->TransAmount;
				$amountPaid = $AllocnItem->AllocAmt;
				$amountPercent = ($amountPaid/$totalAmountToPay) * 100;

				$taxTotal = $AllocnItem->taxTotal;
				$taxAmountToDeduct = ($taxTotal/100)*$amountPercent;

				//$amountToEnterInBank = $amountPaid-$taxAmountToDeduct;
				$amountToEnterInBank = $amountPaid;

				if($AllocnItem->processed == -1 || $AllocnItem->processed == $_SESSION['Alloc']->AllocTrans)
					$amountToEnterInBank = $amountPaid-$taxTotal;

				if($AllocnItem->processed == -1){

					$SQL = "UPDATE debtortrans 
							SET processed='". $_SESSION['Alloc']->AllocTrans ."'
				...
				$SQL = "SELECT * FROM gltrans 
						...
						AND typeno='".$AllocnItem->TypeNo."'
						AND narrative LIKE '%Receipt (".$_SESSION['Alloc']->AllocTrans.")%'"; 
				...
				if($resc != 1){
					...
					$SQL = "INSERT INTO gltrans (
								...
								'" . date('Y-m-d') . "',
								'" . $PeriodNo . "',
								'" . $_SESSION['Alloc']->BankAccount . "',
								'Against Receipt (".$_SESSION['Alloc']->AllocTrans.")',
								'" . ($amountToEnterInBank) . "'
							)";
					...
					$SQL = "UPDATE gltrans SET
							amount='".($amountToEnterInBank)."',
							trandate = '".date('Y-m-d')."'
							WHERE account='".$_SESSION['Alloc']->BankAccount."'
							AND type='".$typeOfTrans."'
							AND typeno='".$AllocnItem->TypeNo."'
							AND narrative='Against Receipt (".$_SESSION['Alloc']->AllocTrans.")'";
			...
			$Settled = (abs($NewAllocTotal-$AllocnItem->TransAmount) < 5) ? 1 : 0;

			if($AllocnItem->settled == 1){
				$Settled = 1;
			}
			...
			if($Settled == 1 && $AllocnItem->settled == 0 && $typeOfTrans != 750 && $typeOfTrans != 602){
				...
				if(round($taxTotal) != 0){
					...
					$SQL = "INSERT INTO gltrans (
							...
							'" . $PeriodNo . "',
							'2310',
							'Against Invoice (".$AllocnItem->TypeNo.") and Receipt (".$_SESSION['Alloc']->AllocTrans.")',
							'" . $taxTotal . "'
						)";
			...
			if($Settled == 1 && $AllocnItem->settled == 0 && $typeOfTrans == 750){
				...
				$SQL = "INSERT INTO gltrans (
							...
							'" . $PeriodNo . "',
							'1100',
							'Against CSV/CRV (".$AllocnItem->TypeNo.") and Receipt (".$_SESSION['Alloc']->AllocTrans.")',
							'" . (-1*$AllocnItem->TransAmount) . "'
						)";
			...
			if($Settled == 1 && $AllocnItem->settled == 0 && $typeOfTrans == 602){
				...
				$SQL = "INSERT INTO gltrans (
							...
							'" . $PeriodNo . "',
							'1100',
							'Against MPO (".$AllocnItem->TypeNo.") and Receipt (".$_SESSION['Alloc']->AllocTrans.")',
							'" . (-1*$AllocnItem->TransAmount) . "'
						)";
...
if (isset($_GET['AllocTrans'])) {
	...
	$_SESSION['Alloc']->BankAccount = $myrow['bankaccount'];
...
if (isset($_POST['AllocTrans'])) {
	...
	*/
	...
	foreach ($_SESSION['Alloc']->Allocs as $AllocnItem) {
		...
		if($AllocnItem->settled == 1){
			echo '<tr style="display:none" class="OddTableRows">';
		}
		else if ( $AllocnItem->ID == $_POST['AllocTrans'] ) {
		...
		if($AllocnItem->TransType == "Receipt"){
			echo "<td></td>";
		}else{
			
			$SQLa = "SELECT shopinvoiceno FROM invoice WHERE invoiceno='".$AllocnItem->TypeNo."'";
			$re   = mysqli_query($db, $SQLa);
			echo "<td>".mysqli_fetch_assoc($re)['shopinvoiceno']."</td>";
		}
        $SQLa = "SELECT reverseHistory FROM debtortrans WHERE transno='".$AllocnItem->TypeNo."'";
        $re   = mysqli_query($db, $SQLa);
        echo "<td>".mysqli_fetch_assoc($re)['reverseHistory']."</td>";
		echo '<td>' . $AllocnItem->TypeNo . '</td>
		...
		if ($AllocnItem->TransAmount < 0) {
				...
				echo '<input tabindex="' . $j .'" style="display:none" type="checkbox" title="' . _('Check this box to allocate the entire amouunt of this transaction. Just enter the amount without ticking this check box for a partial allocation') . '" name="All' .  $Counter . '"';
	...
	$j++;
	echo '<td colspan="2">
			<input tabindex="'.$j.'" type="submit" style="display:none" name="RefreshAllocTotal" value="' . _('Recalculate Total To Allocate') . '" /></td>
        </tr>
		<tr>
