...
customer record returned by the search - this record is then auto selected */
	...
	$SQL = "SELECT debtorsmaster.name,
				...
				SUM(CASE WHEN debtortrans.GSTwithhold = 0 AND debtortrans.WHT = 0 THEN 
					debtortrans.ovamount + debtortrans.ovgst + debtortrans.ovfreight + debtortrans.ovdiscount - debtortrans.alloc
					WHEN debtortrans.GSTwithhold = 0 AND debtortrans.WHT = 1 THEN
					debtortrans.ovamount + debtortrans.ovgst + debtortrans.ovfreight + debtortrans.ovdiscount - debtortrans.alloc - debtortrans.WHTamt
					WHEN debtortrans.GSTwithhold = 1 AND debtortrans.WHT = 0 THEN
					debtortrans.ovamount + debtortrans.ovgst + debtortrans.ovfreight + debtortrans.ovdiscount - debtortrans.alloc - debtortrans.GSTamt
					WHEN debtortrans.GSTwithhold = 1 AND debtortrans.WHT = 1 THEN
					debtortrans.ovamount + debtortrans.ovgst + debtortrans.ovfreight + debtortrans.ovdiscount - debtortrans.alloc - debtortrans.GSTamt - debtortrans.WHTamt
					END
				) as balance,
...
echo '<form action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?Type='.$_GET['Type'] . '" method="post" id="form1" enctype="multipart/form-data">';
echo '<div>';
...
if (isset($_SESSION['CustomerRecord'])
	...
	echo "Here";
	if($_SESSION['CustomerRecord']['balance'] > 0){
		echo "<br> Balance > 0";
		if($_SESSION['CustomerRecord']['GSTwithhold'] == 1){
			echo '<br> GHTwithhold';
			$_SESSION['CustomerRecord']['balance'] -= $_SESSION['CustomerRecord']['GSTamt'];
		}
		if($_SESSION['CustomerRecord']['WHT'] == 1){
			echo '<br> WHT';
			$_SESSION['CustomerRecord']['balance'] -= $_SESSION['CustomerRecord']['WHTamt'];
		}
	}
...
if (isset($_SESSION['ReceiptBatch']->Items) AND count($_SESSION['ReceiptBatch']->Items) > 0){
	echo '<div class="centre">
			<br/>
			Cheque File: <input type="file" name="chequefile"><br>
			Cheque Deposit Receipt: <input type="file" name="cdr"><br>
			Cash Receipt Voucher: <input type="file" name="crv"><br>
