...
$SQL = "SELECT salesman from salescase where salescaseref='".$_SESSION['salescaseref']."'";
		...
		echo "<h3>Using Cart of ".$rowsp['salesman']."</h3>";
...
$nm='addnewitem'.$count;
	 ...
	 if (1)  
		...
		{
$sqlnewlineno="select MAX(internalitemno) as maxinternalitemno from dcdetails where
...
<input type="hidden" name="lne" value='. $_GET['line'].'>
			...
			$sqlOI="select MAX(optionitemno) as maxoptionitemno from dcdetails where
...
{$optionitemno=0;}
		...
		$SQL="SELECT * FROM dcdetails where orderno='".$_SESSION['orderno']."' AND orderlineno='".$_GET['line'] ."'
		AND lineoptionno='".$_GET['option'] ."'";
...
*/$QOHResult =  DB_query($QOHSQL,$db);
				...
				$sql = "SELECT SUM(dcdetails.quantity-dcdetails.qtyinvoiced) AS dem
						FROM dcdetails INNER JOIN dcs
						ON dcs.orderno = dcdetails.orderno
						 WHERE  dcs.fromstkloc='" . $_SESSION['loccode'] . "'
						 AND dcdetails.completed=0
						 AND dcs.quotation=0
						 AND dcdetails.stkcode='" . $myrow['stockid'] . "'";
