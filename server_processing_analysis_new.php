...
OR $rowR['closingreason']=="any other reason"
	...
	array(
		...
		'formatter' => function( $d, $row ) {
			$conn = mysqli_connect('localhost','root','','sahamid');
$sql = "SELECT ocvalue".$_SESSION['UserID'].".value as value FROM ocvalue".$_SESSION['UserID']."
INNER JOIN salescase on
 ocvalue".$_SESSION['UserID'].".salescaseref = salescase.salescaseref
			where 
			ocvalue".$_SESSION['UserID'].".salescaseref= '".$d."'
...
;
			...
			$myrow = mysqli_fetch_assoc($result);
				...
				return locale_number_format($myrow['value'],0);
	...
	$sql = 'SELECT * FROM dcs
			...
			$result = mysqli_query($conn,$sql);
			while($myrow = mysqli_fetch_assoc($result))
				$dcno.= date("d/m/Y", strtotime($myrow['orddate'])).'<br>';
				return $dcno;
	...
	array(
		...
		'formatter' => function( $d, $row ) {
			$conn = mysqli_connect('localhost','root','','sahamid');
$sql = "SELECT SUM(dcvalue".$_SESSION['UserID'].".value) as value FROM dcvalue".$_SESSION['UserID']."
INNER JOIN salescase on
 dcvalue".$_SESSION['UserID'].".salescaseref = salescase.salescaseref
			where 
			dcvalue".$_SESSION['UserID'].".salescaseref= '".$d."'
