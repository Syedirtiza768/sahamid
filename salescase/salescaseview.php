<?php 
	...
	$SQL = 'SELECT count(*) as count FROM pannel_costing WHERE salescaseref="'.$_GET['salescaseref'].'" AND closed="0" ';
	$result = mysqli_query($db, $SQL);
	...
	<body style="overflow: hidden;">
	      	...
	      	<div class="maincontentarea">
				...
				<span style="position: absolute; left: 0; cursor: pointer;" class="commentssection">++</span>
	      		...
	      		<?php 
					...
					if($salescase['closed'] == 0 && $salescase['priority'] == "medium" && $salescase['priority_added'] == "1"){
		      			echo '<p class="btn-warning" style="text-align: center; margin-bottom:0px; padding:10px">';
		      			echo 'Priority: '.$salescase['priority'];
						echo '<br/>';
						echo 'Updated By: '.$salescase['priority_updated_by'];
	      				echo '</p>';
						...
						<div class="col-md-12">
							...
							<div class="col-md-6" style="text-align: right;">
								...
								<a class="btn btn-danger" 
									href="api/removeSalescaseFromWatchlist.php?salescaseref=<?php echo $salescase['salescaseref']; ?>">
									Remove From Watchlist</a>
								<?php } else { ?>
								<a class="btn btn-success"
									href="api/addSalescaseToWatchlist.php?salescaseref=<?php echo $salescase['salescaseref']; ?>">Add To Watchlist</a>
								<?php } ?> 
							</div>
						</div>
					</div>
				...
				<div id="pctab" class="tab toggleviewnone">
					<?php
						...
						$SQL = 'SELECT costNo from pannel_costing where salescaseref="'.$salescase['salescaseref'].'" ';
						...
						while($panel_costing = mysqli_fetch_array($pc_reslut)){
							$count = $count+1;
							$panel_costings[] = $panel_costing['costNo']; 
						...
						if($salescase['closed']== 0 && $PanelInProgress == 0){
							...
							$SQL = 'SELECT costNo FROM pannel_costing  WHERE salescaseref="'.$_GET['salescaseref'].'"';
							$result = mysqli_query($db, $SQL);
							while($abcdef = mysqli_fetch_array($result)){
							$costNo = $abcdef['costNo'];
							}
		...
		<script>
			function pdfExternal($id){
				let costNo = $id;
					$.ajax({
						url: "../panel_costing/pdfExternal.php",
						method: "POST",
						data: {
							costNo: costNo
						},
						success: function (data) {
							window.open("../panel_costing/pdfExternal.php");
						},
						});
			}
			...
			function pdfInternal($id){
				let costNo = $id;
					$.ajax({
						url: "../panel_costing/pdfInternal.php",
						method: "POST",
						data: {
							costNo: costNo
						},
						success: function (data) {
							window.open("../panel_costing/pdfInternal.php");
						},
						});
			}
			...
			$(".salescaseval").on("change",function(){
				...
				$.ajax({
	             	...
	             	url: "<?php echo $RootPath; ?>"+"/api/updatevalues.php",
	              	data: {FormID: formid,salescaseref: salescaseref, name:name,value:val},
	              	dataType: "json",
	              	success: function(response){
		...
		<script>
			...
			$(document).ready(function(){
				$("#shouldscroll").nanoScroller({ scroll: 'bottom' });
			...
			$("#submitcontacts").on("click",function(){
				...
				$.ajax({
					...
					data: {FormID:FormID, salescaseref:salescaseref, contacts:selectedcontacts},
					success: function(response){
			...
			$(".salescaselogremarks").on("change",function(){
				...
				$.ajax({
					...
					data: {FormID:FormID, salescaseref:salescaseref, itemcode:itemcode, remarks},
					success: function(response){
						ref.css("border","2px green solid");	
					},
					error: function(){
