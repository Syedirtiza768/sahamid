<?php 
	...
	if(!userHasPermission($db, 'make_duplicate_quotation')){
		header("Location: ../../index.php");
		return;
	}

	$orderno = $_POST['orderno'];
	$salescaserefNew = $_POST['salescaseref'];

	$SQL = "SELECT * FROM salescase WHERE salescaseref='".$salescaserefNew."'";
	$res = mysqli_query($db, $SQL);
	$newSC = mysqli_fetch_assoc($res);
	...
	$SQL = "SELECT custbranch.brname,
				...
				custbranch.salesman
			FROM custbranch
			WHERE custbranch.branchcode='".$newSC['branchcode']."'
			AND custbranch.debtorno = '".$newSC['debtorno']."'";
	$res = mysqli_query($db, $SQL);
	...
	//Check InProgress
	$SQL = "SELECT orderno,buyername,debtorno,branchcode FROM salesordersip WHERE salescaseref='".$salescaserefNew."'";
	...
	if(mysqli_num_rows($result) == 1){
		...
		header('Location: /sahamid/makequotation.php?orderno='.$row['orderno'].'&salescaseref='.$salescaserefNew.'&selectedcustomer='.$row['buyername'].'&DebtorNo='.$row['debtorno'].'&BranchCode='.$row['branchcode']);    
	...
	`quotedate`, `poplaced`, `salesperson`,`GSTadd`,`services`,`WHT`,`umqd`,`validity`,`quickQuotation`,`rate_clause`,`rate_validity`) 
	VALUES (0,'".str_replace("'","\'",$so['orddate'])."','','".str_replace("'","\'",$salescaserefNew)."','".str_replace("'","\'",$newSC['debtorno'])."',
	'".str_replace("'","\'",$newSC['branchcode'])."','".str_replace("'","\'",$so['customerref'])."','".str_replace("'","\'",$so['buyername'])."','".str_replace("'","\'",$so['comments'])."',
	'".str_replace("'","\'",$so['ordertype'])."','".str_replace("'","\'",$so['shipvia'])."','".str_replace("'","\'",$cus['braddress1'])."','".str_replace("'","\'",$cus['braddress2'])."','".str_replace("'","\'",$cus['braddress3'])."',
	'".str_replace("'","\'",$cus['braddress4'])."','".str_replace("'","\'",$cus['braddress5'])."','".str_replace("'","\'",$cus['braddress6'])."','',
	'','".str_replace("'","\'",$cus['brname'])."','".str_replace("'","\'",$so['deliverblind'])."','".str_replace("'","\'",$so['freightcost'])."',
	'".str_replace("'","\'",$so['advance'])."','".str_replace("'","\'",$so['delivery'])."','".str_replace("'","\'",$so['commisioning'])."','".str_replace("'","\'",$so['after'])."','".str_replace("'","\'",$so['gst'])."',
	'".str_replace("'","\'",$so['afterdays'])."','".str_replace("'","\'",$so['fromstkloc'])."', '".date('Y-m-d')."','".date('Y-m-d')."',
	'".str_replace("'","\'",$so['printedpackingslip'])."','".str_replace("'","\'",$so['datepackingslipprinted'])."','".str_replace("'","\'",$so['quotation'])."',
	'".date('Y-m-d')."','".str_replace("'","\'",$so['poplaced'])."','".str_replace("'","\'",$so['salesperson'])."','".str_replace("'","\'",$so['GSTadd'])."','".str_replace("'","\'",$so['services'])."',
	'".str_replace("'","\'",$so['WHT'])."','".$so['umqd']."','".$so['validity']."','".$so['quickQuotation']."','".$so['rate_clause']."','".$so['rate_validity']."')";
...
mysqli_query($db, $SQL);
	...
	header('Location: /sahamid/makequotation.php?orderno='.$newOrderNo.'&salescaseref='.$salescaserefNew.'&selectedcustomer='.$so['buyername'].'&DebtorNo='.$so['debtorno'].'&BranchCode='.$so['branchcode']);  
