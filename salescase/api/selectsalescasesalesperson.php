<?php 
    ...
    if(in_array($_SESSION['AccessLevel'], $allowed)){
        ...
        $subquery = "SELECT salescase.salescaseref  FROM salescase 
        ...
        $SQLA = "SELECT can_access FROM salescase_permissions WHERE user='".$_SESSION['UserID']."'";
...
//Quotation Values
	$SQL = "SELECT salesorders.salescaseref, salesorderdetails.orderno, SUM(salesorderdetails.unitprice*
		...
		AND  salesorders.salescaseref IN ($subquery)
	...
	$SQL = "SELECT salescaseref,orderno FROM dcs 
		...
		AND dcs.salescaseref IN ($subquery)
	...
	$SQL = "SELECT salescaseref,commentcode FROM salescasecomments 
		...
		AND salescasecomments.salescaseref IN ($subquery)
	...
	AND  salescase_watchlist.salescaseref IN ($subquery)";
	$result = mysqli_query($db,$SQL);	
	...
	if($_SESSION['AccessLevel'] == 22 || $_SESSION['AccessLevel'] == 8 || $_SESSION['AccessLevel'] == 10){
		...
		$SQL = "SELECT `salescaseindex`, salescase.salescaseref, `salescasedescription`,
				...
				AND  salescase.salescaseref IN ($subquery)
				";
	...
	echo json_encode(array_values(array_filter($toFilter, function($val) use ($sp, $cl, $scr, $loc, $dir, $pri, $dba, $po, $dc, $quot){
