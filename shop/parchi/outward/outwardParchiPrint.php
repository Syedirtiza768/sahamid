<?php 
	...
	if(isset($_GET['internal']) && !userHasPermission($db,"outward_parchi_internal")){
	...
	$SQL = "SELECT bazar_parchi.*,custbranch.brname as name FROM bazar_parchi 
			LEFT OUTER JOIN custbranch ON bazar_parchi.svid = custbranch.branchcode
	...
	$parchi['parchino'] = "MPO-".explode("-", $parchi['parchino'])[1]."-".$parchi['payment_terms'];
					...
					<tr>
						...
						<td>
							<div style="text-align: left; vertical-align: top; display: block; position: absolute; left:2px; top: 83px; font-size: 10px;">
								<span style="font-weight: bold;">Dated: </span><?php echo date('d/m/Y'); ?><br/>
							</div>
						</td>
					</tr>
				</table>
			...
			<table border="2" style="margin-top: 10px; border-radius: 8px; width: 100%">
				...
				<tbody>
						...
						<tr style="font-size: 11px">
							...
							<?php 
								echo ($item['price'] == 0) ? 
										...
										($item['price']."<sub>PKR</sub>"); ?></td>
							<td style="text-align: center;"><?php echo locale_number_format($item['quantity_received']*$item['price'],2); ?><sub>PKR</sub></td>
							<?php if(isset($_GET['internal'])){ ?>
					...
					<tr style="font-size: 11px">
						...
						<td colspan="3" class="tdpa"><?php echo locale_number_format($tot,2); ?><sub>PKR</sub></td>
					</tr>
					...
					<tr style="font-size: 11px">
						...
						<td colspan="3" class="tdpa"><?php echo locale_number_format($row['amount'],2); ?> <span style="font-size: 11px"><sub>PKR</sub></span></td>
					</tr>
					<?php $parchi['advance'] += $row['amount']; } ?>
					<tr style="font-size: 11px">
						<th colspan="5" class="tdpa" style="text-align: right;">Total: </th>
						<td colspan="3" class="tdpa"><?php echo locale_number_format($tot-$parchi['advance'],2); ?><sub>PKR</sub></td>
					</tr>
					<?php }else{ $total = 0; foreach($items as $item){ ?>
						...
						<tr style="font-size: 11px">
							...
							<td style="text-align: center;"><?php echo locale_number_format(($item['quantity_received']*$item['price']),2); ?></td>
							<?php if(isset($_GET['internal'])){ ?>
							<td class="break tdpa" style="min-width: 3.3cm; max-width: 3.3cm"><?php echo $item['comments']; ?></td>
							<?php } ?>
						</tr>
					...
					<tr style="font-size: 11px">
						...
						<td colspan="3" class="tdpa"><?php echo locale_number_format($total,2); ?> <span style="font-size: 11px"><sub>PKR</sub></span></td>
					</tr>
					<?php while($row = mysqli_fetch_assoc($advance)){ ?>
					<tr style="font-size: 11px">
					...
					<?php $parchi['advance'] += $row['amount']; } ?>
					<?php $paid = 0; while($row = mysqli_fetch_assoc($payments)){ ?>
						<tr style="font-size: 11px">
							<th colspan="4" class="tdpa" style="text-align: right;">
					...
					<tr style="font-size: 11px">
						...
						<td colspan="3" class="tdpa"><?php echo locale_number_format($total-$paid-$parchi['advance'],2); ?> <span style="font-size: 11px"><sub>PKR</sub></span></td>
					</tr>
					<?php } ?>
				</tbody>
			</table>
