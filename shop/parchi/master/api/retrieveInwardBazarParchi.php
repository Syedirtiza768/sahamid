<?php
	...
	$parchi['canUpdateQuantity'] = userHasPermission($db, "inward_slip_item_quantity");
	$parchi['canDeleteItem'] = userHasPermission($db, "delete_inward_parchi_item");

	echo json_encode([

			'status' => 'success',
			'accessPrice' => ((userHasVendorPermission($db, $parchi['svid']) && $parchi['svid'] != "") ? "true":"false"),
			'data' => $parchi
