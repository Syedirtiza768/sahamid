<?php
	...
	if(!userHasPermission($db,"edit_inward_parchi")){ 
		header('Location: ' . $_SERVER['HTTP_REFERER']);
		return;
	}

	$SQL = "SELECT supplierid,suppname FROM suppliers WHERE supptype=2";
	...
	if(mysqli_num_rows($res) != 1){
		header("Location: listInwardBazarParchi.php");
		return;
	}
...
<html>
	<head>
		...
		<script src="../../../v2/assets/popper.min.js"></script>
	...
	<body>
		...
		<section class="body" style="overflow: auto">
	      	...
	      	<div style="display: flex; justify-content: center; width: 100%; margin-bottom: 50px">
	      		<div style="min-width: 75vw; max-width: 75vw; background-color: white;">
	      			<div class="col-md-12 border" style="min-width: 75vw;">
	      				<div class="detailsoverlay border">
	      					<div class="detailsbody">
	      						<table style="width: 100%; margin-top: 0px;">
	      							<tr>
	      								<td colspan="2">
											<?php if(userHasPermission($db, "inward_slip_orignal_vendor")){ ?>
	      									<button class="btn btn-success tab-btn ovbtn" data-tab="orignalvendor">
	      										Original Vendor
	      									</button>
											<?php } ?>
											<?php if(userHasPermission($db, "inward_slip_ledger")){ ?>
											...
											<?php if(userHasPermission($db, "inward_parchi_dc")){ ?>
	      									<button class="btn btn-primary tab-btn" data-tab="adttab">
	      										Attached DC
	      									</button>
											<?php } ?>
	      									...
	      									<?php if($parchiDetails['igp_created'] == 1){ ?>
	      									<a 	href="../../../PDFIGP.php?RequestNo=<?php echo $parchiDetails['igp_id']; ?>" 
	      										class="btn btn-info">
	      										Download IGP
	      									</a>		
	      									<?php } ?>
									...
									<?php if(userHasPermission($db, "inward_slip_orignal_vendor")){ ?>
	      							<tr style="border-top: 1px solid #424242; display: none;" id="orignalvendor" class="tabss">
	      								<td colspan="3" style="text-align: center; padding: 20px">
      										<div class="col-md-12">
	      										<h4 style="text-align: center;">Add Orignal Vendor</h4>
	      								...
	      								<td colspan="3" style="text-align: center;">
	      									...
	      									<div class="col-md-4">
	      										<div class="gstinvoicea" style="padding: 30px;">
	      											...
	      											<!-- <input type="checkbox" name="gstinvoice" class="gstinvoice"> -->
	      										</div>
	      									</div>
	      									<div class="col-md-4">
	      										<div class="gstinvoicea" style="padding: 30px;">
									...
									<?php if(userHasPermission($db, "inward_parchi_dc")){ ?>
	      							<tr style="border: 1px solid #424242" id="adttab" class="tabss">
	      								<td colspan="3">
	      									<h4 style="text-align: center;">Attached DC List</h4>
	      									<table style="width: 100%">
	      										<tr style="text-align: center;">
	      											<td class="col-md-6" style="text-align: center;">
	      												<input id="newdcno" type="text" style="margin: 0px auto; margin-bottom: 5px; border:1px solid #424242; border-radius: 7px; display: block; text-align: center;" placeholder="Attach New">
	      												<button class="btn btn-success" id="attachdc">Attach</button>
	      											</td>
	      											<td class="col-md-6">
	      												<ul id="attacheddccontainer">
										      				<?php while($row = mysqli_fetch_assoc($attachedDC)){ ?>

																<li><?php echo $row['dcno']; ?></li>
						...
						<?php if(userHasPermission($db, "attach_inward_slip_sku")){ ?>
						...
						<table style="width: 100%">
							<tr>
								<td colspan="2" style="text-align: right;">
									...
									<?php 
										if(!userHasVendorPermission($db, $parchiDetails['svid']) && $parchiDetails['svid'] != ""){ 
											echo "display:none;"; 
										} 
									?>"
									>Total: <span class="totalCalculatedAmount"></span></h2>
									<?php if($parchiDetails['igp_created'] == 0 && userHasPermission($db, "inward_slip_igp")){ ?>
									<button id="makeParchiIGP"
											class="btn btn-success">Make IGP</button>
									<?php } ?>
									<?php if(userHasPermission($db, "inward_parchi_internal")){ ?>
										<a class="btn btn-primary" style="margin-right: 10px;" target="_blank" href="inwardParchiPrint.php?parchi=<?php echo $_GET['parchi']; ?>&internal"><i class="fa fa-print" aria-hidden="true"></i>Internal Print</a>
									<?php } ?>
									<a class="btn btn-primary" style="margin-right: 10px;" target="_blank" href="inwardParchiPrint.php?parchi=<?php echo $_GET['parchi']; ?>"><i class="fa fa-print" aria-hidden="true"></i>External Print</a><button class="btn btn-info detailsbtn">+</button></td>
							</tr>
							<tr>
								<td></td>
							...
							<tr>
								<td></td>
								<td style="text-align: right;">Vendor: <span id="vendorabc">-------</span></td>
							</tr>
							<tr>
								<td></td>
							...
							<tr>
								<td colspan="2">
									<?php if($parchiDetails['igp_created'] == 0 && userHasPermission($db, "discard_inward_slip")) { ?>
							...
							<tr>
								<td colspan="2">
									<?php if(userHasPermission($db, "finalize_market_slip")){ ?>
	      	...
	      	<div id="popper">
	      		...
	      		<div class="col-md-12">
	      			<table class="table table-responsive" id="datatablepopup" border="1">
		      			<thead>
		      				<tr style="background: #424242; color: white">
		      					<th>MPI</th>
		      					<th>Name</th>
		...
		<script>
			...
			$(document).ready(function(){
				popupTable = $('#datatablepopup').DataTable({
					"pageLength": 5,
				    "bLengthChange": false,
				    "columnDefs": [{
					    "targets": 0,
					    ...
					    "render": function ( data, type, row, meta ) {
					      	return `
								<a href="/sahamid/shop/parchi/inward/inwardParchiPrint.php?parchi=${row[0]}&internal" target="_blank">
									${row[0]}
					      		</a>
					      	`;
					    }
			...
			function addNewItem(id,name,qty,rec,price,com,neg,lp,dis,stockid,igp_created,canAttachSKU,canNegotiatePrice,canUpdateQuantity,canDeleteItem,accessPrice){
				...
				/*if(stockid === ""){
					...
					html += "(<span class='itemSKUPopup'>"+stockid+"</span>) ";
				}*/
				...
				if(stockid != ""){
					html += "<p style='display:inline-block'>(<span class='itemSKUPopup'>"+stockid+"</span>)</p> ";
				}else{
				...
				html += `<td style="text-align:left; ${(accessPrice == "false") ? "visibility:hidden;":""} ">`;


				html += '<span class="actPrice">'+Math.round(price*100)/100+'</span><sub>PKR</sub>';
				...
				html += '</td>';
				html += `<td style="text-align:left; ${(accessPrice == "false") ? "visibility:hidden;":""}" 							
							 class="subtotal">${Math.round((price*rec)*100)/100}</td>`;
			...
			$(document).ready(function(){
				...
				$.post("api/retrieveInwardBazarParchi.php",
						{
							FormID: '<?php echo $_SESSION['FormID']; ?>',
							...
							if(res.status == "success"){
								...
								let accessPrice = res.accessPrice;
								
								$.each(res.data.items,function(key, value){
									addNewItem(value.id,value.name,value.quantity,value.quantity_received,value.price,value.comments,value.neg,value.listprice,value.discount,value.stockid,res.data.igp_created,canAttachSKU,canNegotiatePrice,canUpdateQuantity,canDeleteItem,accessPrice);
								});
								
								$(document.body).on('click','.itemSKUPopup', function(){
									popup.show();
									...
									new Popper($(this),$("#popper"),{
			    						...
			    						onCreate: function(data){
                                			$.get(`/sahamid/inventory/api/mpiRecentPurchase.php?stockid=${code}&parchi=<?php echo $_GET['parchi']; ?>`, function(res, status){
                                				res = JSON.parse(res);
												popupTable.clear().draw();
                                				popupTable.rows.add(res).draw();
                                			});
                       				 	},
									});
								});
			...
			$(document.body).on("click",".removeitem",function(){
				...
				swal({
					...
					$.post('api/removeInwardBazarParchiItem.php',
						{
			...
			$("#discardparchi").on("click",function(){
				...
				swal({
					...
					$.post('api/discardInwardBazarParchi.php',
						{
			...
			$(document.body).on('change','.qtyreceived', function(){
				...
				$.post('api/updateInwardBazarParchiItem.php',
					{
			...
			$(document.body).on("change",".comments",function(){
				...
				$.post('api/updateInwardBazarParchiItemComment.php',
					{
			...
			$("#saveparchi").on("click",function(){
				...
				swal({
					...
					$.post('api/finalizeInwardBazarParchi.php',
						{
							FormID: '<?php echo $_SESSION['FormID']; ?>',
							parchino: parchino
			...
			$("#makeParchiIGP").on("click",function(){

				swal({
					title: "Are you sure?",
					text: "This will create the igp and quantity received cannot be changed after this!",
					type: "warning",
					showCancelButton: true,
					confirmButtonClass: "btn-danger",
					confirmButtonText: "Yes! Create IGP!",
					showLoaderOnConfirm: true,
					closeOnConfirm: false
				}, function(){
					...
					$.post('api/makeInwardParchiIGP.php',
						{
							FormID: '<?php echo $_SESSION['FormID']; ?>',
							parchino: parchino
			...
			$("#attachdc").on("click",function(){

				let parchino = '<?php echo trim($_GET['parchi']); ?>';
				let newdcno = $("#newdcno").val();
				let ref = $(this);
				...
				$.post('api/attachNewDC.php',
					{
						...
						if(res.status == "error"){
							...
							$("#attacheddccontainer").append("<li>"+newdcno+"</li>");
							$("#newdcno").val("");
							ref.css("border","3px solid green");
						}
