<?php
	...
	if ($_GET['filters']=='yes')
    {
           $SQL = "SELECT bazar_parchi.*,bazar_parchi.settled as settled2, supptrans.id as traid, www_users.realname,
        ...
        $SQL.="GROUP BY bazar_parchi.transno";
    }
	else {
        ...
        if ($_SESSION['filter'] == "settled") {
            $SQL = "SELECT bazar_parchi.*,bazar_parchi. as settled2, supptrans.id as traid, www_users.realname,
                        suppliers.suppname as name
                        FROM bazar_parchi
                        INNER JOIN www_users ON bazar_parchi.user_id = www_users.userid
                        LEFT OUTER JOIN supptrans ON (bazar_parchi.transno = supptrans.transno
...
$res = mysqli_query($db, $SQL);
	...
	while($row = mysqli_fetch_assoc($res)){
        ...
        $SQLalloc = "SELECT * FROM suppallocs WHERE transid_allocto = '".$row['traid']."' ORDER BY id DESC";
        $resalloc = mysqli_query($db, $SQLalloc);
        $allocated=0;
        if(mysqli_num_rows($resalloc) > 0){
            while($rowalloc=mysqli_fetch_assoc($resalloc))
            {
                $allocated=$allocated+$rowalloc['amt'];
            }
        }
		$r[] = ($row['discarded'] == 1 ? "Discarded":($row['settled2'] == 1 ? "Settled":($row['inprogress'] == 1 ? "InProgress":"Saved $allocated")));
		$r[] = $row['realname'];
		if($canEdit){
			$r[] = (($row['inprogress'] == 1 AND $row['settled2'] == 0)?
