														...
														<thead style="background-color: #C5BD99; color: black">
															<tr>
																...
																<th>Discount<input type="hidden" class="manualDiscountLine" value="<?php echo $row['id']; ?>">
																	<button data-discountlineid="<?php echo $row['id']; ?>" class="btn btn-success manualDiscount" style="width: 50%">Manual</button>
																</th>
																<th>Quantity</th>
																<th></th>
															</tr>
	...
	<script>
		...
		$(document).ready(function() {
			...
			$(".searchitembtn").on("click", function() {
				let colCount = $("#searchresultsdatatab thead tr th").length;

				// Show loading row inside tbody
				$("#srb").html(
					`<tr><td colspan="${colCount}" style="text-align:center; font-size:18px; font-weight:bold;">
        Loading...
      </td></tr>`
				);
				...
				$.post("api/itemSearch.php", {
					...
					$("#srb").html(
						`<tr><td colspan="${colCount}" style="text-align:center; color:red;">
            Error loading data.
          </td></tr>`
					);
				});
			...
			swal({
				...
				function() {
					...
					$.post("api/finalizeShopSale.php", {
						...
						console.log(res);
						if (res.status == "success") {
							window.location.href = "estimateList.php";
						} else {
							swal("Error", res.message, "error");
						}
