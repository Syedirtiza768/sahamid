...
if (isset($_GET['OldStockID']) || isset($_POST['OldStockID']) ){ //we are cloning
	...
	if (isset($_POST['StockID']) ) {
        $_POST['StockID'] =$_POST['Brand'].trim(mb_strtoupper($_POST['ConditionID'])).trim(mb_strtoupper($_POST['CategoryID'])).$_POST['mnfpno'];
...
if (!isset($_POST['UpdateCategories']) AND $InputError!=1) { // Must be modifying an existing item and no changes made yet
...
echo '<tr>
		...
		<td><input type="text" name="mnfpno" size="12" maxlength="40" value="' . $_POST['mnfpno']. '" /></td></tr>';
...
while ($PropertyRow=DB_fetch_array($PropertiesResult)){
	...
	switch ($PropertyRow['controltype']) {
	 	...
	 	case 1; //select box
			...
			echo '<select name="PropValue' . $PropertyCounter . ' " size = "4">';
