...
*/
				...
				if ((($Quantity >= $RequestedQuantity) OR $Completed==True) and $_POST['slct'] == $RequestID) {
					$SQL="UPDATE stockrequestitems
								SET completed=1
							WHERE dispatchid='".$RequestID."'
								AND dispatchitemsid='".$LineID."'";
...
*/
	$sql="SELECT stockrequest.dispatchid,
		...
		LEFT JOIN www_users
			ON www_users.userid='".$_SESSION['UserID']."'
	...
	while ($myrow=DB_fetch_array($result)) {
		...
		echo '<tr>
			...
			echo '<input type="hidden" name="fromloc" value="' . $myrow['loccode'] . '" />';
			echo '<input type="hidden" name="toloc" value="' . $myrow['userlocation'] . '" />';
		...
		while ($LineRow=DB_fetch_array($LineResult)) {
			echo '<tr>
					...
					<td style = "position : fixed; left : - 999px;><select name="'. $LineRow['dispatchid'] . 'Tag'. $LineRow['dispatchitemsid'] . '">';
