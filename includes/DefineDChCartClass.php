	...
	function add_to_cart($StockID,
		...
		if (isset($StockID) AND $StockID!="" AND $Qty>0 AND isset($Qty)){
			...
			if ($UpdateDB=='Yes'){
				...
				committing the order to the DB in DeliveryDetailsDCh.php
				...
				global $db;
			$sql = "INSERT INTO dcdetails (internalitemno,
														orderlineno,
														lineoptionno,
														optionitemno,
...
UPDATE dcdetails
SET internalitemno = (SELECT (@rownum := @rownum + 1) FROM (SELECT @rownum := -1) r) where orderno='" . $_SESSION['ExistingOrder' . $identifier] . "'";
DB_query($SQLD,$db);
			}
	...
	//	echo $Price;
		...
		$result = DB_query("UPDATE dcdetails SET quantity=" . $Qty . ",
								...
								AND internalitemno=" . $UpdateLineNumber
													, $db
	...
	function remove_from_cart($LineNumber, $UpdateDB='No', $identifier=0){
		...
		if ($UpdateDB=='Yes'){
			...
			if ($this->Some_Already_Delivered($LineNumber)==0){
				...
				$result = DB_query("DELETE FROM dcdetails
									WHERE orderno='" . $_SESSION['ExistingOrder' . $identifier] . "'
									AND internalitemno='" . $LineNumber . "'",
									$db,
...
DB_query($SQLD,$db);
echo'<script>
window.location.assign("'.$RootPath.'/sahamid/SelectOrderItemsDCh.php?ModifyOrderNumber=' . $_SESSION['ExistingOrder' . $identifier] . '&salescaseref=' . $_SESSION['salescaseref'] . '");
			</script>';
			} else {
				/* something has been delivered. Clear the remaining Qty and Mark Completed */
				$result = DB_query("UPDATE dcdetails SET quantity=qtyinvoiced,
																completed=1
									WHERE orderno='" . $_SESSION['ExistingOrder' . $identifier] ."'
									AND internalitemno='" . $LineNumber . "'" ,
									$db,
...
Class externalline{
	Var $externallineindex;
	Var $externallineno;
	Var $orderno;
	Var $lineoptions;
	Var $clientreq;
	//function externalline();
	function externalline ($externallineindex,
	$orderno,
						$externallineno,
						$clientreq){

/* Constructor function to add a new line option object with passed params */
		$this->externallineindex = $externallineindex;
		$this->orderno = $orderno;
		
		$this->lineoptions = array();
	...
	} //end constructor function for lineption
...
/* Constructor function to add a new line option object with passed params */
		$this->lineoptionindex = $lineoptionindex;
		$this->orderno = $orderno;
...
/* Constructor function to add a new LineDetail object with passed params */
		...
		$this->lineoptionno = $lineoptionno;
