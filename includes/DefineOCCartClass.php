	...
	function add_to_cart($StockID,
		...
		if (isset($StockID) AND $StockID!="" AND $Qty>0 AND isset($Qty)){
			...
			if ($UpdateDB=='Yes'){
				...
				global $db;
			$sql = "INSERT INTO ocdetails (internalitemno,
														orderlineno,
														lineoptionno,
														optionitemno,
...
DB_query($SQLD,$db);
	...
	//	echo $Price;
		...
		$result = DB_query("UPDATE ocdetails SET quantity=" . $Qty . ",
	...
	function remove_from_cart($LineNumber, $UpdateDB='No', $identifier=0){
		...
		if ($UpdateDB=='Yes'){
			...
			if ($this->Some_Already_Delivered($LineNumber)==0){
				...
				$result = DB_query("DELETE FROM ocdetails
									WHERE orderno='" . $_SESSION['ExistingOrder' . $identifier] . "'
...
DB_query($SQLD,$db);
echo'<script>
window.location.assign("'.$RootPath.'/sahamid/SelectOrderItemsOC.php?ModifyOrderNumber=' . $_SESSION['ExistingOrder' . $identifier] . '&salescaseref=' . $_SESSION['salescaseref'] . '");
			</script>';
			} else {
				/* something has been delivered. Clear the remaining Qty and Mark Completed */
				$result = DB_query("UPDATE ocdetails SET quantity=qtyinvoiced,
																completed=1
									WHERE orderno='" . $_SESSION['ExistingOrder' . $identifier] ."'
									AND internalitemno='" . $LineNumber . "'" ,
									$db,
