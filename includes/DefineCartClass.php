	...
	function add_to_cart($StockID,
		...
		if (isset($StockID) AND $StockID!="" AND $Qty>0 AND isset($Qty)){
			...
			if ($UpdateDB=='Yes'){
				...
				global $db;
			$sql = "INSERT INTO salesorderdetails (internalitemno,
														orderlineno,
														lineoptionno,
														optionitemno,
...
DB_query($SQLD,$db);
echo'<script>
window.location.assign("'.$RootPath.'/sahamid/SelectOrderItems.php?ModifyOrderNumber=' . $_SESSION['ExistingOrder' . $identifier] . '&salescaseref=' . $_SESSION['salescaseref'] . '");
			</script>';
			} else {
