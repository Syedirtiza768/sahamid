...
if (isset($_GET['StockID'])){
	...
	//$StockID =trim(mb_strtoupper($_POST['StockID'])).$_POST['mnf'].$_POST['mnfpno'];
	$StockID =$_POST['Brand'].trim(mb_strtoupper($_POST['ConditionID'])).trim(mb_strtoupper($_POST['CategoryID'])).$_POST['mnfpno'];
			...
			*/
			...
			if (DB_num_rows($result)==1){
				...
				if (DB_error_no($db) ==0) {
					...
					$sql = "INSERT INTO substorestock (loccode,
														substoreid,
													stockid)
										SELECT substores.locid,substores.substoreid,
										'" . $StockID . "'
										FROM substores";

					$ErrMsg =  _('The substores for the item') . ' ' . $StockID .  ' ' . _('could not be added because');
				//	$DbgMsg = _('NB Locations records can be added by opening the utility page') . ' <i>Z_MakeStockLocns.php</i> ' . _('The SQL that was used to add the location records that failed was');
					$InsResult = DB_query($sql,$db,$ErrMsg,$DbgMsg,true);
					DB_Txn_Commit($db);
					if (DB_error_no($db) ==0) {
...
/*If the page was called without $StockID passed to page then assume a new stock item is to be entered show a form with a part Code field other wise the form showing the fields with the existing entries against the part will show for editing with only a hidden StockID field. New is set to flag that the page may have called itself and still be entering a new part, in which case the page needs to know not to go looking up details for an existing part*/
	...
	if ($New==1) {
		echo '<tr>
				<td style = "position:absolute; left:-999px;">' .  _('Item Code'). ':</td>
				<td><input type="hidden" ' . (in_array('StockID',$Errors) ?  'class="inputerror"' : '' ) .'" data-type="no-illegal-chars" autofocus="autofocus" required="required"  value="'.$StockID.'" name="StockID" size="20" maxlength="20"  title ="'._('Input the stock code, the following characters are prohibited:') . ' \' &quot; + . &amp; \\ &gt; &lt;" placeholder="'._('alpha-numeric only').'" /></td>
			</tr>';
	} else {
		echo '<tr>
				<td>' .  _('Item Code'). ':</td>
	...
	$_POST['Description'] = $myrow['Description'];
							...
							$_POST['mnf'] =  $myrow['mnfCode'];
							$_POST['Units'] =  $myrow['units'];
							$_POST['Brand'] =  $myrow['brand'];
							$_POST['LongDescription'] =  $myrow['longdescription'];
							$_POST['Description'] =  $myrow['description'];
							$_POST['mnfpno'] = $myrow['mnfpno'];
							$_POST['ConditionID'] =  $myrow['conditionID'];
...
echo '<tr>
		<td>' . _('Brand') . ':</td>
		<td><select name="Brand" onchange="ReloadForm(ItemForm.UpdateCategories)">';
...
while ($myrow=DB_fetch_array($result)){
	if (!isset($_POST['Brand']) OR  $myrow['manufacturers_id']==$_POST['Brand']){
		echo '<option selected="selected" value="'. $myrow['manufacturers_id'] . '">' . $myrow['manufacturers_name'] . '</option>';
	} else {
...
echo '<tr>
		<td>' . _('Condition') . ':</td>
		<td><select name="ConditionID" onchange="ReloadForm(ItemForm.UpdateCategories)">';

$sql = "SELECT conditionID, abbreviation FROM itemCondition";
$ErrMsg = _('The stock categories could not be retrieved because');
$DbgMsg = _('The SQL used to retrieve stock categories and failed was');
$result = DB_query($sql,$db,$ErrMsg,$DbgMsg);

while ($myrow=DB_fetch_array($result)){
	if (!isset($_POST['ConditionID']) OR  $myrow['conditionID']==$_POST['ConditionID']){
		echo '<option selected="selected" value="'. $myrow['conditionID'] . '">' . $myrow['abbreviation'] . '</option>';
	} else {
		echo '<option value="'. $myrow['conditionID'] . '">' . $myrow['abbreviation'] . '</option>';
	}
	$Condition=$myrow['conditionID'];
}

if (!isset($_POST['ConditionID'])) {
	$_POST['ConditionID']=$Condition;
}
...
{	
	echo '<tr>
		<td>' . _('Manufacturer Code') . ':</td>
		<td><input type="text" name="mnf" size="40" maxlength="100" value="' . $_GET['WO']. Date('Y-m-d'). '" /></td></tr>';
}
else
{
		echo '<tr>
		<td>' . _('Manufacturer Code') . ':</td>
		<td><input type="text" name="mnf" size="40" maxlength="100" value="' . $_POST['mnf']. '" /></td></tr>';
}
		if (isset ($_GET['WO']))
{
		echo '<tr>
		<td>' . _('Manufacturer Part Number') . ':</td>
		<td><input type="text" name="mnfpno" size="40" maxlength="100" value="' . $_GET['WO']. Date('Y-m-d'). '" /></td></tr>';
}
else
{
		echo '<tr>
		<td>' . _('Manufacturer Part Number') . ':</td>
		<td><input type="text" name="mnfpno" size="40" maxlength="100" value="' . $_POST['mnfpno']. '" /></td></tr>';
}
echo '<tr style = "position:fixed; left:-999px">
...
echo '<tr>
		...
		<td><select name="MBFlag" onchange="ReloadForm(ItemForm.UpdateCategories)">';
if ($_POST['MBFlag']=='A'){
...
echo '<tr style = "position:fixed; left:-999px">
		...
		<td><select name="Controlled" onchange="ReloadForm(ItemForm.UpdateCategories)">';
...
echo '<tr style = "position:fixed; left:-999px"><td>' . _('Serialised') . ':</td><td><select ' . (in_array('Serialised',$Errors) ?  'class="selecterror"' : '' ) .'  name="Serialised">';
...
echo '<tr>
		...
		<td><input type="hidden" class="number" name="Pansize" size="6" maxlength="6" value="' . locale_number_format($_POST['Pansize'],0) . '" /></td>
	</tr>
	 <tr>
		
		<td><input type="hidden" class="number" name="ShrinkFactor" size="6" maxlength="6" value="' . locale_number_format($_POST['ShrinkFactor'],0) . '" /></td>
	</tr>';
...
while ($PropertyRow=DB_fetch_array($PropertiesResult)){
	...
	switch ($PropertyRow['controltype']) {
	 	...
	 	case 1; //select box
			...
			echo '<select name="PropValue' . $PropertyCounter . '" size = "1">';
