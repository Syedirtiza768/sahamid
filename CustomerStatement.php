	...
	while( $row = mysqli_fetch_assoc($res) ){
		...
		if($row['type'] == 12){
			...
			$SQL = "SELECT invoice.shopinvoiceno,debtortrans.type,debtortrans.transno 
					...
					INNER JOIN invoice ON invoice.invoiceno = debtortrans.transno
					WHERE custallocns.transid_allocfrom = '".$row['id']."' GROUP BY invoice.shopinvoiceno";
            ...
            <tr class="top">
                <td colspan="2">
                    <table>
                        <tr>
                            <td class="title">
                                <img src="/sahamid/companies/sahamid/EDI_Incoming_Orders/companylogos/<?php echo $customerStatement['dba']; ?>.jpg" style="width:auto; max-width:auto; height: 50px">
                            </td>
