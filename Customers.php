		...
		if (!isset($_POST['New'])) {
			...
			if ($myrow[0] == 0) {
			  $sql = "UPDATE debtorsmaster SET	name='" . $_POST['CustName'] . "',
												...
												typeid='" . $_POST['typeid'] . "',
												dba='" .htmlspecialchars_decode($_POST['DBA']) . "',
			...
			//Auto Generated For Market Slip Outward Clients
			if($_POST['typeid'] == 28){
				$_POST['DebtorNo'] = "MC-".GetNextTransNo(603, $db);
			}
			...
			$sql = "INSERT INTO debtorsmaster (
				...
				VALUES ('" . $_POST['DebtorNo'] ."',
						...
						'" . $_POST['typeid'] . "',
						'" .htmlspecialchars_decode($_POST['DBA']) . "',
						'" . $_POST['LanguageID'] . "')";
...
// Show DBA drop down list
	$result=DB_query("SELECT * FROM dba ORDER BY companyname",$db);
	if (DB_num_rows($result)==0){
		$DataError =1;
		echo '<tr>
				<td colspan="2">' . prnMsg(_('No Company defined'),'error') . '<br /><a href="wotemplates.php?" target="_parent">' . _('Setup Types') . '</a></td>
			</tr>';
	} else {
        echo '<tr>
				<td>' . _('DBA') . '' . _('') . ':</td>
			   <td><select tabindex="9" name="DBA" required="required">';

		while ($myrow = DB_fetch_array($result)) {
		   echo '<option value="'. $myrow['companyname'] . '">' . $myrow['companyname'] . '</option>';
		} //end while loopre
		DB_data_seek($result,0);
        echo '</select></td>
			</tr>';
...
// Select dba for drop down list
	if (isset($_GET['Modify'])) {
		$result=DB_query("SELECT* FROM dba WHERE companyname='".$_POST['DBA']."'",$db);
		$myrow=DB_fetch_array($result);

		echo '<tr>
				<td>' . _('DBA') . ':'.$_POST['DBA'].' '.$myrow['companyname'].'</td>
				<td>' . $myrow['companyname'] . '</td></tr>';
	} else {
		$result=DB_query("SELECT * FROM dba",$db);
		echo '<tr>
				<td>' . _('DBA') . '' . _('') . ':</td>
				<td><select name="DBA" required="required">';
		while ($myrow = DB_fetch_array($result)) {

			if (htmlspecialchars_decode($_POST['DBA'])==htmlspecialchars_decode($myrow['companyname'])){
				echo '<option selected="selected" value="' . $myrow['companyname'] . '">' . $myrow['companyname'] . '</option>';
			} else {
				echo '<option value="'. $myrow['companyname'] . '">' . $myrow['companyname'] . '</option>';
			}
		} //end while loop
