		...
		$.ajax({
		    ...
		    success: function(response) { 
		    	...
		    	if(status == "success"){
					...
					$("#order-details").find(".dispatchthrough").val(data.dispatchthrough);

		    		
		    		if(data.mp == 1){
		    			$("#order-details").find(".mp").prop('checked',true);
		    		}else{
		    			$("#order-details").find(".mp").prop('checked',false);
		    		}
		    		...
		    		for (var index in lines) {
					    ...
					    for(var optionIndex in options){
						    ...
						    var optprice  = options[optionIndex].optprice;

						    addoptionCallback(lineno,optionno,opttext,stkstatus,quanty,optprice);
						    ...
						    for(var itemIndex in items){
							    ...
							    var indx = items[itemIndex].dcdetailsindex;
					...
					let k = 0;
					for(;k<data.slips.length;k++){
						$("#slipscontainer").append('<a class="btn btn-info" style="width:100%" target="_blank" href="../shop/parchi/inward/inwardParchiPrint.php?parchi='+data.slips[k]+'">'+data.slips[k]+'</a>')
					}
		...
		for(index in dirtyEditors){
			...
			if(dirtyEditors[index].length > 0){
				...
				if(dEditor.editor.content.isDirty()){
					...
					$.ajax({
						type: 'POST',
					    url: rootpath+"/shopdc/api/saveDescription.php",
					    data: {orderno: order,lineno: line, description: text, type: type,index: index,option: option,FormID:formID},
					    dataType: "json",
					    success: function(response) { 
					    	var status = response.status;
	...
	function addline(){
		...
		$.ajax({
			type: 'POST',
		    url: rootpath+"/shopdc/api/createNewLine.php",
		    ...
		    success: function(response) { 
		    	...
		    	if(status == "success"){
		    		...
		    		var opt  = data.option_id;
		    		var req  = "";
		    		addlineCallback(line,req);
		    		addoptionCallback(line,opt,"","",0,0);
		    	}else{
	...
	function addlineCallback(line, requirements){
		...
		descHTML += '<h3>Line No. '+vline+'</h3>';
		descHTML += '<textarea  style="display:none; width: 100%; height: 100%;" id = "l'+line+'o---desc" cols="90" rows="10"></textarea>';
		...
		//inittextboxio(line,"---",requirements);

		$('#linescontainer').append(html);
	}

	function addoptionCallback(line,option, description,stockstatus, quantity,optprice){
		...
		descHTML += '</td>';
		descHTML += '<td>';
		descHTML += '<input class="descinfo stockstatus" type="hidden" name="stockstatus" value="'+stockstatus+'">';
		descHTML += '</td>';
		descHTML += '<td>';
		...
		descHTML += '</td>';
		descHTML += '<td>';
		descHTML += '<input type="number" class="descinfo optprice" name="optprice" value="'+optprice+'">';
		descHTML += '</td>';
		descHTML += '<td>';
		descHTML += '<b>Price:</b>';
		descHTML += '</td>';
    ...
    function additemCallback(indx,line,option,stkc,bname,description,quantity,sprice,uprice,tot,qoh,dis,update,modal,part){
        ...
        var html = '<section id="l'+line+'o'+option+'i'+indx+'" class="panel panel-featured panel-featured-info '+stkc+'">';
        ...
        var descItemHTML = "<tr id='opdcd"+indx+"' class='"+stkc+"'>";
        descItemHTML += "<td>";
        descItemHTML += modal;
        descItemHTML += "</td>";
        descItemHTML += "<td>";
	...
	function removeitem(line, option, index, description){
		...
		function(){
		  ...
		  $.ajax({
		  	type: 'POST',
		    url: rootpath+"/shopdc/api/deleteItem.php",
		    ...
		    success: function(response) { 
		    	...
		    	if(response.status == "success"){
		    		...
		    		$("."+response.data.stkcode+"").each(function(){
		    			$(this).find(".qohabc").val(response.data.qoh);
		    			$(this).find(".quabc").html(response.data.qoh);
		    		});
	...
	function removeoption(line, option, voption){
		...
		function(){
		  ...
		  $.ajax({
		  	type: 'POST',
		    url: rootpath+"/shopdc/api/deleteOption.php",
		    data: {line: line, option: option,orderno:order},
		    dataType: "json",
		    success: function(response) { 
		    	...
		    	if(response.status == "success"){
		    		...
		    		if(response.data.linedeleted == 1)
	...
	function removeline(line, vline){
		...
		function(){
		  ...
		  $.ajax({
		  	type: 'POST',
		    url: rootpath+"/shopdc/api/deleteLine.php",
		    data: {line: line,orderno:order},
		    dataType: "json",
		    success: function(response) { 
		...
		$.ajax({
		    ...
		    success: function(response) { 
		    	...
		    	if(status == "success"){
		    		...
		    		$("#"+item+"").find("input.quantity").val(response.minimum);
		    		$("#opdcd"+id+"").find(".quantity").html(response.minimum);
		    		$("#"+item+"").find("input.quantity").prop("disabled", false);
	...
	function updatePrice(item,uprice,discount){
		...
		$.ajax({
			type: 'POST',
		    url: rootpath+"/shopdc/api/updateItem.php",
		    data: {orderno: order, salescaseref: salesref, name: "uprice",value:uprice,item:item,discount:discount,FormID:formID},
		    dataType: "json",
		    success: function(response) { 
		    	var status = response.status;
	...
	$("#bss").click(function(e){
		...
		$.ajax({
			...
			type: 'POST',
		    url: rootpath+"/shopdc/api/search.php",
	...
	function insertitem(itemindex, brand){
		...
		$.ajax({
			type: 'POST',
		    url: rootpath+"/shopdc/api/addNewItem.php",
	...
	$(document.body).on('change','.order-detailss',function(){
	    ...
	    $.ajax({
			type: 'POST',
		    url: rootpath+"/shopdc/api/updateOrderDetails.php",
	...
	$(document.body).on('change','.descinfo',function(){
	    ...
	    $.ajax({
			type: 'POST',
		    url: rootpath+"/shopdc/api/updateOption.php",
		    data: {orderno: order, salescaseref: salesref, name:name, value:value, option:option,FormID:formID},
		    dataType: "json",
		    success: function(response) { 
	...
	$("#erbtn").click(function(e){
		...
		$.ajax({
			type: 'GET',
		    url: rootpath+"/previewShopDC.php?DCNo="+order+"&salescaseref="+salesref,
		    dataType: "json",
		    success: function(response) { 

	    		var ihtml = "<iframe ";
				ihtml += 'id="previewframe" ';
				ihtml += 'src = "../ViewerJS/#../tempPDF/dc/'+order+'ints.pdf" ';
	...
	$("#checkforwarnings").click(function(e){
		...
		$.ajax({
			type: 'POST',
		    url: rootpath+"/shopdc/api/saveDC.php",
		    data:{salescaseref:salesref,orderno:order,FormID:formID},
		    dataType: "json",
		    success: function(r) { 
		    	...
		    	if(r.status == "success")
		    		...
		    		$("#warningscontainer").html("Some error occured");
	...
	function addBrandDiscountEntries(brandname){
		...
		html += '<div class="col-md-9">';
		html += '<input type="number" name="discountpercent" class="discountpercent" style="border: 1px solid #424242" value="0" max="99">';
		html += '</div>';
		html += '</div>';
	...
	$("#updategroupdiscountsbutton").on('click',function(){
		...
		$.ajax({
			type: 'POST',
			url: rootpath+'/shopdc/api/groupDiscount.php',
