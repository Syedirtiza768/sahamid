<?php
	...
	while( $row = mysqli_fetch_assoc($res) ){
        ...
        $row['dcnoslinks'] = explode(",", $row['dcnos']);
    ...
    <div class="invoice-box">
        ...
        <table cellpadding="0" cellspacing="0">
            ...
            <tr class="heading" style="text-align: left;">
                ...
                <td colspan="3" class="nostretch" style="font-size: 12px;">
                ...
                <td colspan="3" class="nostretch" style="font-size: 12px;">
                 <center>Amount Remaining</center>
                </td>
            ...
            <?php
            	...
            	foreach ($customerStatement['statement'] as $statement) {
                    if($statement['type'] == 13)
                        continue;
                        $ob += $statement['ovamount'] - $statement['alloc'];
            ...
            <tr class="item">
            	<?php 
            		...
            		if($statement['type'] == 12){
            			...
            			$bl = $statement['ovamount']-$statement['alloc']-$statement['WHTamtc']-$statement['GSTamtc'];
            	...
            	<?php 
						...
						if($statement['type'] == 13){
							$totalCredit += abs($statement['ovamount']-$statement['alloc']);
						}else{
							$totalDebit += abs($statement['ovamount']-$statement['alloc']);
						}
            	...
            	?>
            		...
            		<td class="nostretch" style="font-size: 12px">
	                    <?php //PO
                        echo (($statement['type'] == 750 || $statement['type'] == 602) ? strtoupper($statement['payment']):$statement['customerref']) ?><br>
	              	...
	              	<td class="nostretch" style="font-size: 12px">
						...
						<?php }else{ ?>
                           ...
                           $shopinvoicelink= '<a href="'.$RootPath.'/companies/sahamid/EDI_Incoming_Orders/Invoice_'.$dclink.'.pdf"
										>'.$statement['shopinvoiceno'].'</a><br>';
                           $crvlink='<a href="'.$RootPath.'/shop/pos/shopSalePrint.php?orderno='.$statement['transno'].'" target="_blank"
										>'.$statement['transno'].'</a><br/>';?>

                            <?php foreach ($statement['dcnoslinks'] as $dclink)
                            {
                                $invoiceFiles = glob(''.$_SESSION['part_pics_dir'].'/Invoice_'.$dclink."*");
                                ...
                                foreach($invoiceFiles as $invoiceFile) {
                                    ...
                                    $shopinvoicelink= '<a id="viewenquiry" style="width: available;" class="btn btn-warning" href = "'.$RootPath.'/'.$invoiceFile.'" target = "_blank" >'.$statement['shopinvoiceno'].'</a><br />';

                                }
                            echo (($statement['type'] == 750 || $statement['type'] == 602) ? $crvlink:$shopinvoicelink) ?><br>
	              	...
	              	<td class="nostretch" style="font-size: 12px">
	              		<?php //dcnos
                        foreach ($statement['dcnoslinks'] as $dclink)
                        {
                            echo '<a href="'.$RootPath.'/PDFDChExternal.php?DCNo='.$dclink.'"
										>'.$dclink.'</a><br/>';
                        }
                       ?>
	              	</td>
	              	...
	              	<td class="nostretch" style="text-align: center !important; font-size: 12px">
	              		<?php //aging
                        if($statement['type'] == 10 || $statement['type'] == 750 || $statement['type'] == 602) echo $statement['ageing']; ?>
	              	</td>
                    <td colspan="3" class="nostretch" style="font-size: 12px">
                        <center> <?php //invoice total
                            if($statement['type'] == 10 || $statement['type'] == 750|| $statement['type'] == 602) echo locale_number_format(abs(round($statement['ovamount'],2)),2); ?>
                        </center>
                    </td>
                    <td colspan="3" class="nostretch" style="font-size: 12px">
                        <center> <?php //paid
                            if($statement['type'] == 10 || $statement['type'] == 750|| $statement['type'] == 602) echo locale_number_format(abs(round($statement['alloc'],2)),2); ?>
                        </center>
                    </td>
	              	<td colspan="3" class="nostretch" style="font-size: 12px">
	                 <center> <?php //amount remaining
                         if($statement['type'] == 10 || $statement['type'] == 750|| $statement['type'] == 602) echo locale_number_format(abs(round($statement['ovamount']-$statement['alloc'],2)),2); ?>
                     </center>
                    </td>
	              <?php } ?>
            </tr>
            ...
            <tr class="heading">
                ...
                <td class="nostretch" style="text-align: left !important; font-size: 12px;">Debit: <?php echo locale_number_format(round($totalDebit,2),2); ?></td>
                <td class="nostretch" style="text-align: left !important; font-size: 12px;">Credit: <?php echo locale_number_format(round($totalCredit,2),2); ?></td>
                <td class="nostretch" style="text-align: right !important; font-size: 12px;">Remaining Total: <?php echo locale_number_format(round($ob,2),2); ?></td>
            </tr>
            <tr class="item">
