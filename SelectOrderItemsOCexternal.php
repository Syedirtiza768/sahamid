...
// everything below here only do if a customer is selected
	...
	//	echo '</b><br />' . _('Terms') . ':<b> ' . $_SESSION['Items'.$identifier]->PaymentTerms;
		echo '</b></div>';
		echo '<table border="1"><tr><td><a href="SelectOrderItemsOC.php?identifier='.$identifier .'" onclick="return confirm(\'' . _('Press OK only if description is saved') . '\');">Bill of Quantities</a></td><td><a style="color:green;" href="SelectOrderItemsOCexternal.php?identifier='.$identifier .'">Descriptions</a></td></tr></table>';
...
//header('"Location: sahamIdtest/SelectOrderItemsOC.php?identifier=?identifier='.$identifier .'"' );
}
...
<h3>Line No. '.($line+1).' Client Requirements </h3><textarea  style="width: 700px; height: 100%;" id = "clientreq'.$line.'"
...
for ($option=0; $option<= $optionscount; $option++ )
	{
		...
		<tr><td>
		<h2>Line No. '.($line+1)." Option No. ".($option+1).'</h2>
		</td>';
...
//	'',$_SESSION['Items'.$identifier]->externallines[$line]->lineoptions[$option]->optiontext);
	...
	echo'<table width="100%" cellpadding="2">
		...
		</tr><tr><td>Stock status</td><td><input name = "stockstatus'.$line.'_'.$option.'" 
