	...
	function sessionStoragefun(line)
	{
		var lineno = line;
		 $(".line").val(lineno);
	}
	...
	function newLine(id){
		...
		if(id=='n'){
			...
			if(split[2] == null){ addline(); }
				...
				lineno= $(".line").val();
				alert(lineno);
		// addoptionCallback(lineno,split[3],split[4],
		// split[5],split[6],split[7],split[8]);
		var lineno= split[0];
		 additemcb(lineno);
		// additemCallback(split[9],split[0],split[11],split[12]
		// ,split[13],split[14],split[15],split[16],split[17],split[18],split[19],split[20],split[21],split[22],split[23],split[24]);
