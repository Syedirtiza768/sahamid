...
//	echo $ExternalLinesSQL;
	...
	if ($_SESSION['Items'.$identifier]->Quotation==1){
		prnMsg(_('OC Number') . ' ' . $OrderNo . ' ' . _('has been entered'),'success');
	} else {
		...
		if ($_POST['Quotation']==0) { /*then its not a quotation its a real order */
			...
			echo '<br /><table class="selection">
					<tr>
						...
						<td>' . ' ' . '<a href="' . $RootPath . '/PDFOC.php?identifier='.$identifier . '&amp;OrderConfirmationNo=' . $OrderNo . '">' .  _('Print OC (Internal)')  . '</a></td>
					</tr>
					</table>';
			echo '<br /><table class="selection">
					<tr>
						<td><img src="'.$RootPath.'/css/'.$Theme.'/images/reports.png" title="' . _('Order') . '" alt="" /></td>
						<td>' . ' ' . '<a href="' . $RootPath . '/PDFOCExternal.php?identifier='.$identifier . '&amp;OrderConfirmationNo=' . $OrderNo . '">' .  _('Print OC (External)')  . '</a></td>
					</tr>
...
$HeaderSQL = "UPDATE ocs SET debtorno = '" . $_SESSION['Items'.$identifier]->DebtorNo . "',
	...
	if($Quotation){ //handle Quotations and Orders print after modification
		...
		echo '<br /><table class="selection">
				<tr>
					<td><img src="'.$RootPath.'/css/'.$Theme.'/images/reports.png" title="' . _('Order') . '" alt=""></td>
					<td>' . ' ' . '<a href="' . $RootPath . '/PDFOC.php?identifier='.$identifier . '&amp;OrderConfirmationNo=' . $_SESSION['ExistingOrder'.$identifier] . '">' .  _('Print OC (Landscape)')  . '</a></td>
				</tr>
				</table>';
		echo '<br /><table class="selection">
				<tr>
					<td><img src="'.$RootPath.'/css/'.$Theme.'/images/reports.png" title="' . _('Order') . '" alt="" /></td>
					<td>' . ' ' . '<a href="' . $RootPath . '/PDFOCExternal.php?identifier='.$identifier . '&amp;OrderConfirmationNo=' . $_SESSION['ExistingOrder'.$identifier] . '">' .  _('Print OC (External)')  . '</a></td>
				</tr>
...
echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />';
	...
	$sqlL="select MAX(orderlineno) as maxlineno from ocdetails where orderno = ".$_SESSION['ExistingOrder' .$identifier].""; 		
