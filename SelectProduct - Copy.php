...
if (DB_num_rows($result2) == 0) {
	echo '<p class="bad">' . _('Problem Report') . ':<br />' . _('There are no brands currently defined please use the link below to set them up') . '</p>';
	echo '<br /><a href="' . $RootPath . '/Manufacturers.php">' . _('Define Stock Categories') . '</a>';
	exit;
}
...
if ($Its_A_Kitset_Assembly_Or_Dummy == False) {
	...
	echo '<a href="' . $RootPath . '/StockCostUpdate.php?StockID=' . $StockID . '">' .  '</a><br />';
...
echo '<td><b>' . _('') . ' ' . '</b>' . _('Enter partial') . ' <b>' . _('Stock Code') . '</b>:</td>';
...
echo '<td><b>' . _('') . ' ' . '</b>' . _('Enter partial') . ' <b>' . _('Item properties') . '</b>:</td>';

echo '<td>';
if (isset($_POST['keyproperties'])) {
	echo '<input type="text" name="keyproperties"  value="' . $_POST['keyproperties'] . '" title="' . _('Enter text that you wish to search for in the item properties') . '" size="15" maxlength="18" />';
} else {
	echo '<input type="text" name="keyproperties"  title="' . _('Enter text that you wish to search for in the item properties') . '" size="15" maxlength="18" />';
}
echo '</td>';
echo '</td></tr></table><br />';
...
if (isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
	...
	if (isset($_POST['keyproperties'])) {
		...
		$_POST['keyproperties'] = mb_strtoupper($_POST['keyproperties']);
		$SearchString = '%' . str_replace(' ', '%', $_POST['keyproperties']) . '%';
		...
		if ($_POST['brand'] == 'All') {
			$SQL = "SELECT stockmaster.stockid,
						...
						ON stockmaster.categoryid=stockcategory.categoryid,
							locstock,stockitemproperties
						WHERE (stockmaster.stockid=locstock.stockid
						AND stockitemproperties.value " . LIKE . " '%" . $_POST['keyproperties'] . "%'
						
						AND stockcategory.categoryid='" . $_POST['StockCat'] . "')
						AND stockmaster.stockid " . LIKE . " '$SearchString2'
			...
			$SQL = "SELECT stockmaster.stockid,
						...
						ON stockmaster.stockid=locstock.stockid, stockitemproperties
						WHERE stockmaster.stockid=locstock.stockid
						AND stockitemproperties.value " . LIKE . " '%" . $_POST['keyproperties'] . "%'
						AND categoryid='" . $_POST['StockCat'] . "'
						AND brand='" . $_POST['brand'] . "'
						GROUP BY stockmaster.stockid,
							stockmaster.description,
							stockmaster.longdescription,
							stockmaster.units,
	...
	} elseif (!isset($_POST['StockCode']) AND !isset($_POST['keyproperties'])) {
		if ($_POST['brand'] == 'All') 
		{
			$SQL = "SELECT stockmaster.stockid,
						stockmaster.description,
