...
if (isset($_GET['NewOrder'])){
  /*New order entry - clear any existing order details from the Items object and initiate a newy*/
	...
	$_SESSION['Items'.$identifier] = new cart;
		...
		$sql='DELETE FROM salesorderlines where orderno = 0
	
	';
	DB_query($sql,$db);
		$sql='DELETE FROM salesorderoptions where orderno = 0
	
	';
	DB_query($sql,$db);
	
		$sql='INSERT INTO salesorderlines (orderno,lineno) VALUES ('.$identifier.',
	0
	
	)';
	DB_query($sql,$db);
	
	$sql='INSERT into salesorderoptions (orderno,lineno,optionno) VALUES ('.$identifier.',
	0,
	0
	...
	$newlinesql='SELECT * FROM salesorderlines where orderno = "'.$_SESSION['ExistingOrder' .$identifier].'"
	AND lineno=0
	';
	...
	$newlinerow = DB_fetch_array($newlineresult);
		...
		$newoptionsql='SELECT * FROM salesorderoptions where orderno = "'.$_SESSION['ExistingOrder' .$identifier].'"
...
/*need to look up customer name from debtors master then populate the line items array with the sales order details records */
	...
	$externallinessql = "SELECT *
								...
								WHERE  salesorderlines.orderno ='" . $_GET['ModifyOrderNumber'] . "'
								ORDER BY salesorderlines.lineno";
...
//($externallinessqlresult);
	...
	$lineoptionsssql = "SELECT *
								...
								WHERE  salesorderoptions.orderno ='" . $_GET['ModifyOrderNumber'] . "'
								AND 	salesorderoptions.lineno ='" . $myrow['lineno'] . "'
								ORDER BY salesorderoptions.lineno";
...
// everything below here only do if a customer is selected
 	...
 	if (isset($_POST['CancelOrder'])) {
		...
		if ($_SESSION['Items'.$identifier]->Quotation==1){
			echo _('<h3>Quotation for customer</h3>') . ' ';
		} else {
	...
	//	echo '</b><br />' . _('Terms') . ':<b> ' . $_SESSION['Items'.$identifier]->PaymentTerms;
		...
		echo '<table border="1"><tr><td><a href="SelectOrderItems.php?identifier='.$identifier .'">Bill of Quantities</a></td><td><a href="SelectOrderItemsexternal.php?identifier='.$identifier .'">Descriptions</a></td></tr></table>';
		echo '<h1>Bill of Quantities</h1>';
	}
	$msg ='';?>
...
<input type="button" onclick="SetAllCheckBoxes(true);" value="SELECT ALL LINES">
&nbsp;&nbsp;
<input type="button" onclick="SetAllCheckBoxes(false);" value="UNSELECT ALL LINES">
<?php
...
echo		$SQL='SELECT MAX(lineoptionno) as maxoption FROM salesorderdetails WHERE
		orderno="'.$_SESSION['ExistingOrder'.$identifier].'"
			AND orderlineno="'.$_GET['line'].'"';
$result=DB_query($SQL,$db);
$row=DB_fetch_array($result);
		if(isset($_GET['DeleteOption']) AND $row['maxoption']>0){
		$SQL='DELETE FROM salesorderoptions where orderno="'.$_SESSION['ExistingOrder'.$identifier].'"
			AND lineno="'.$_GET['line'].'" AND optionno="'.$_GET['option'].'"';
			DB_query($SQL,$db);
			$SQLD = "

UPDATE salesorderoptions
...
DB_query($SQLD,$db);
			$SQL='DELETE FROM salesorderdetails where orderno="'.$_SESSION['ExistingOrder'.$identifier].'"
...
orderno='" . $_SESSION['ExistingOrder' . $identifier] . "'";
DB_query($SQLD,$db);
$SQL='SELECT MAX(lineoptionno) as maxoption FROM salesorderdetails WHERE orderno="'.$_SESSION['ExistingOrder'.$identifier].'"
			AND orderlineno="'.$_GET['line'].'"';
$result=DB_query($SQL,$db);
$row=DB_fetch_array($result);
...
echo'<script>
window.location.assign("'.$RootPath.'/SelectOrderItems.php?ModifyOrderNumber=' . $_SESSION['ExistingOrder' . $identifier] . '&salescaseref=' . $_SESSION['salescaseref'] . '");
		</script>';
...
//delete line
 ...
 {
	 for($line=$rowline['maxline'];$line>=0;$line--)
	 {
		 $lineindex = "lindex".$line;
//	$lineindex.'--'.$rowline['maxline'].'=='.$_POST[$lineindex].'<br>';
	//print_r($_POST);
//	echo $_POST[$lineindex].'<br>'.$rowline['maxline'];
	// $_SESSION[$lineindex]=$_POST[$lineindex];
		if(isset($_POST[$lineindex]) AND $rowline['maxline']>0){
			
		$SQL='DELETE FROM salesorderlines where orderno="'.$_SESSION['ExistingOrder'.$identifier].'"
			AND lineno="'. $_POST[$lineindex].'"';
DB_query($SQL,$db);
$SQLline='SELECT MAX(lineno) as maxline FROM salesorderlines WHERE 
orderno="'.$_SESSION['ExistingOrder'.$identifier].'"
		';
...
// $rowline['maxline'];
		...
		$SQL='DELETE FROM salesorderoptions where 
		orderno="'.$_SESSION['ExistingOrder'.$identifier].'"
			AND lineno="'. $_POST[$lineindex].'"';
DB_query($SQL,$db);
		...
		$SQL='DELETE FROM salesorderdetails where 
		orderno="'.$_SESSION['ExistingOrder'.$identifier].'"
			AND orderlineno="'. $_POST[$lineindex].'"
			';
			DB_query($SQL,$db);

	
		}
//unset( $_SESSION[$lineindex]);
...
$rowline=DB_fetch_array($resultline);
 //$rowline['maxline'];
 $count=0;
 	 for($line=0;$line<=$rowline['maxline'];$line++)
	 {
		$lineindex = "lindex".$line;
		if($rowline['maxline']>0){
	...
	//	echo $count;
			...
			UPDATE salesorderoptions
			SET lineno = lineno-'".$count."' where 
			orderno='" . $_SESSION['ExistingOrder' . $identifier] . "'AND lineno='".$line."'
			...
			$SQLD = "
					...
					UPDATE salesorderdetails
					SET orderlineno = orderlineno-'".$count."' where 
					orderno='" . $_SESSION['ExistingOrder' . $identifier] . "'AND 
					orderlineno='".$line."'
...
window.location.assign("'.$RootPath.'/SelectOrderItems.php?ModifyOrderNumber=' . $_SESSION['ExistingOrder' . $identifier] . '&salescaseref=' . $_SESSION['salescaseref'] . '");
	...
	//echo "<script> document.getElementById('Recalculate').click();</script>";
		if ((isset($_POST['Recalculate']) AND $_POST['Recalculate']!='0' )OR isset($_POST['DeliveryDetails'])){
	...
	$sqlex = "UPDATE salesorderlines SET clientrequirements='" . $clientrequirements . "',
															orderno=" . $_SESSION['ExistingOrder'.$identifier] . "
...
//			$result = DB_query($sqlex
	...
	//	 $linecounter."_".$optioncounter;
	 ...
	 $sqllo = "UPDATE salesorderoptions SET optiontext='" . $optiontext. "',
...
$sqlL="select MAX(lineno) as maxlineno from salesorderlines where orderno = ".$_SESSION['ExistingOrder' .$identifier].""; 		
...
if (isset($_POST['newline']) AND $_POST['newline']!='0' AND $_SESSION['flag']!="up")
{
	$linescount = $resultrow['maxlineno'];
	$linescount++;
	$sql='INSERT into salesorderlines (orderno,lineno) VALUES ("'.$_SESSION['ExistingOrder' .$identifier].'",
	'.$linescount.'
	...
	$sql='INSERT into salesorderoptions (orderno,lineno,optionno) VALUES ("'.$_SESSION['ExistingOrder' .$identifier].'",
	"'.$linescount.'",
	0
...
//header('"Location: sahamIdtest/SelectOrderItems.php?identifier=?identifier='.$identifier .'"' );
...
for ($line=0; $line<= $linescount; $line++ )
{
	...
	$sqlO="select MAX(optionno) as maxlineoptionno from salesorderoptions where orderno = ".$_SESSION['ExistingOrder' .$identifier]."
AND lineno = ".$line.""; 		
...
echo '<table width="90%" cellpadding="2"><tr><td>
<input type="hidden" name = "clientreq'.$line.'">
...
for ($option=0; $option<= $optionscount; $option++ )
	{
		...
		<tr><td>
		<h4>Line No. '.($line+1)." Option No. ".($option+1).'</h4>
		</td>';
...
/* This is where the order as selected should be displayed  reflecting any deletions or insertions*/
     ...
     /*   echo '<div class="page_help_text">' . _('Quantity (required) - Enter the number of units ordered.  Price (required) - Enter the unit price.  Discount (optional) - Enter a percentage discount.  GP% (optional) - Enter a percentage Gross Profit (GP) to add to the unit cost.  Due Date (optional) - Enter a date for delivery.') . '</div><br />';
		echo '<br />*/
				...
				echo '<tr style="background-color:#800000">';
...
echo '</td>
				...
				value="' . $OrderLine->Price . '" title="' . _('Enter the price to charge the customer for this item') . '" />
					...
					size="5" maxlength="4"
					value="' . locale_number_format(($OrderLine->DiscountPercent * 100),2) . '" title="' . _('Enter the discount percentage to apply to the price for this item') . '" 
			...
			id="LineTotal_' . $OrderLine->LineNumber . '" 
					value="' . locale_number_format(((1 - $OrderLine->DiscountPercent)*$OrderLine->Price*$OrderLine->Quantity),2) .
					'" title="' . _('Enter the discount percentage to apply to the price for this item') . '" size="16"
					readonly="readonly" /></td>
					';
			
			$SQLmaxinternalitemno="SELECT MAX(internalitemno) as maxinternalitemno FROM salesorderdetails
			
			WHERE orderno='".$_SESSION['ExistingOrder' . $identifier]."'";
			$resultintermalitemno=DB_query($SQLmaxinternalitemno,$db);
			$rowinternalitemno=DB_fetch_array($resultintermalitemno);
			echo'			<script>
			
			function myFunctionQty() {
				
				var z = 0;
				while (z<=' . $rowinternalitemno['maxinternalitemno'] . ')
				{
 document.getElementById("LineTotal_"+z).value=  Math.round(document.getElementById("UnitRate_"+z).value * document.getElementById("Quantity_"+z).value*100)/100;
			...
			function myFunctiondiscount() {
				...
				var y = 0;
				while (y<=' . $rowinternalitemno['maxinternalitemno'] . ')
				{
					
					
 document.getElementById("Discount_"+y).value= (Math.round( (1 - document.getElementById("UnitRate_"+y).value / document.getElementById("PriceJS_"+y).value)*10000000000)/10000000000)*100;
...
function myFunctionunitrate() {
				var x = 0;
				while (x<=' . $rowinternalitemno['maxinternalitemno'] . ')
				{
...
if($_SESSION['ExistingOrder' . $identifier]!=$_GET['identifier'])
		...
		$DisplayTotal = locale_number_format($_SESSION['Items'.$identifier]->total,$_SESSION['Items'.$identifier]->CurrDecimalPlaces);
			...
			echo'</table>';
			echo'<a   href = "'.$RootPath.'/additems.php?line='.$line.'
...
//	'',$_SESSION['Items'.$identifier]->externallines[$line]->lineoptions[$option]->optiontext);
	...
	echo'<table width="100%" cellpadding="2">
		...
		<tr>
		<td colspan="8"><input type="hidden" name = "SAHdesc'.$line.'_'.$option.'"
		></td>
	...
	$varoptionindex= "optionindex".$line;
			...
			echo'	
				...
				if($_SESSION['ExistingOrder' . $identifier]!=$_GET['identifier'])
	//		echo'	<a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?identifier=' . $identifier . '&amp;DeleteLine=1&amp;line=' .$line . '" onclick="return confirm(\'' . _('Are You Sure?') . '\');">Delete Line</a>';
echo'		<input type="checkbox" id="delline" name="'.$lineindex.'" value="' . $line. '" /> Delete Line';
...
echo'	<br/>
						<br/>
							<input type="submit" name="deleteline" value="' . _('Delete Lines') . '" />
