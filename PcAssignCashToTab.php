...
$sql = "select max(counterindex) as i from pcashdetails";
 ...
 if ( isset($_GET['edit'])) {
	...
	$ReceiptID = 'receipt_'.$myrow['counterindex'];
...
if (!isset($SelectedTabs)){
	...
	if (mysqli_num_rows($result)==0) {
		...
		exit("You are not authorized to assign cash to any user");
		//exit(header("Location: /sahamid"));
		//return;
	}
...
/* Ricard: needs revision of this date initialization */
		...
		echo '<tr>
				...
				<td><input type="date" class="date" alt="'.$_SESSION['DefaultDateFormat'].'" name="Date" required="required" autofocus="autofocus" size="10" maxlength="10" value="' . date('Y-m-d') . '" /></td>
			</tr>';
