...
if (isset($_POST['SelectedTabs'])){
	...
	$_SESSION['SelectedTabs'] = $SelectedTabs;
...
if ((! isset($_POST['FromDate']) AND ! isset($_POST['ToDate'])) OR isset($_POST['SelectDifferentDate'])){
	...
	$SQL = "SELECT * FROM pcashdetails
			WHERE tabcode='".$_SESSION['SelectedTabs']."'
	...
	$LeftOvers = $pdf->addTextWrap($Left_Margin+110,$YPos,70,$FontSize,$_SESSION['SelectedTabs']);
...
echo '<form method="post" action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '">';
	...
	echo '<form name="reportform" method="post" action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '">';
    echo '<div>';
	...
	echo '<tr><td>' . _('Tab Code') . '</td>
				<td>:</td>
				<td style="width:200px">' . ''.$_SESSION['SelectedTabs'] . '</td>'  .
	...
	echo '<tr><td>' . _('Total not authorised before ') . ''.$_POST['ToDate'] . '</td><td>:</td><td>' . ''.locale_number_format($BalanceNotAut['0'],$_SESSION['CompanyRecord']['decimalplaces']) . ' ' . $Tabs['currency'] . '</td></tr>';
echo '<tr>
			<td>' . _('Expense') . ':</td>
			<td><select name="SelectedExpense" onchange = "this.form.submit()">';
			echo "<option value = ''>All</option> ";
	$SQL = "SELECT * from (pcexpenses inner join pctabexpenses 
	on pcexpenses.codeexpense = pctabexpenses.codeexpense )inner join pctabs on pctabexpenses.typetabcode = pctabs.typetabcode
	where tabcode = '".$_SESSION['SelectedTabs']."'
		";
	$result = DB_query($SQL,$db);
	...
	$SQL = "SELECT * FROM pcashdetails
			WHERE tabcode='".$_SESSION['SelectedTabs']."'
				AND date >='" . $SQL_FromDate . "'
				AND date <= '" . $SQL_ToDate . "'
					AND codeexpense like '%" . $_POST['SelectedExpense'] . "%'
...
if ($myrow['3']!= 'ASSIGNCASH')
	...
	echo '<tr><td colspan="2" style="text-align:right">' . _('Total') .' ' . $row_result['description'].' '.$_POST['ToDate'].':</td>
				<td>' . locale_number_format(-1*$total,$_SESSION['CompanyRecord']['decimalplaces']).' </td><td>' . $Tabs['currency'] . '</td></tr>';
	...
	<input type="submit" name="UpdateForm" style="visibility:hidden;width:1px" value="' . _('Categories') . '" />
	<input type="submit" name="SelectDifferentDate" value="' . _('Select A Different Date') . '" /></div>';
