...
if (isset($_GET['tranno'])) {
	...
	if ($InputError==0) {
		 $SQL = 'SELECT * from dcclient where dispatchid = '.$_SESSION['tranno'].'';
...
$myrow = DB_fetch_array($Result);
		...
		$_SESSION['Request']->advance=$myrow['advance'];
...
if (isset($_POST['Update'])) {
	...
	$_SESSION['Request']->Narrative=$_POST['narrative'];
		...
		$_SESSION['Request']->advance=$_POST['advance'];
		...
		$_SESSION['Request']->linetotal=$_POST['linetotal']; 
...
if (isset($_POST['Edit'])) {
	...
	$_SESSION['Request']->LineItems[$_POST['LineNumber']]->linetotal=$_POST['linetotal']; 
		
}

if (isset($_GET['Delete'])) {
	unset($_SESSION['Request']->LineItems[$_GET['Delete']]);
	 $SQL = 'delete from dcclientitems where dispatchid = '.$_SESSION['tranno'].'
...
SELECT COUNT(dispatchitemsid) as count from dcclientitems where dispatchid = '.$_SESSION['tranno'];
$RESULTD1 = DB_query($SQLD1,$db);
$count = DB_fetch_array($RESULTD1);

	 $SQLD = '

UPDATE dcclientitems SET dispatchitemsid=0 WHERE dispatchid='.$_SESSION['tranno'];
DB_query($SQLD,$db);
...
UPDATE dcclientitems
SET dispatchitemsid = (SELECT (@rownum := @rownum + 1) FROM (SELECT @rownum := -1) r) where dispatchid='.$_SESSION['tranno'];
DB_query($SQLD,$db);

 $SQL = 'SELECT * from dcclientitems where dispatchid = '.$_SESSION['tranno'].'';
...
while ($myrow = DB_fetch_array($Result))
{
	$StockID=$myrow['stockid'];
			...
			$NewComment_array[$StockID] =filter_number_format($myrow['rate']);
			$NewRate_array[$StockID] =filter_number_format($myrow['rate']);
			$NewLineTotal_array[$StockID] =filter_number_format($myrow['rate']*$myrow['quantity']);
			
			$_POST['Units'.$StockID]=$_POST['Units'.$Index];
			
			
			$_SESSION['Request']->AddLine($StockID, $ItemDescription, $NewItem_array[$StockID],$NewComment_array, $NewRate_array[$StockID],$NewLineTotal_array[$StockID],-1);
...
foreach ($_POST as $key => $value) {
	if (mb_strstr($key,'StockID')) {
		...
		if (filter_number_format($_POST['Quantity'.$Index])>0) {
			...
			$_POST['Units'.$StockID]=$_POST['Units'.$Index];
			$_SESSION['Request']->AddLine($StockID, $ItemDescription, $NewItem_array[$StockID],$NewComment_array[$StockID], $_POST['Units'.$StockID],-1);
		}
	}
}
$count = 0;
...
if (isset($_POST['Submit']) and $count > 0) {
	...
	if ($InputError==0) {
		...
		foreach ($_SESSION['Request']->LineItems as $LineItems) {
		//echo " $LineItems->ItemDescription<br/>";
		//echo $LineItems->ItemDescription = 	str_ireplace("\r\n",">><<",$LineItems->ItemDescription);
		 $LineSQL="update dcclientitems set 
...
if (isset($_GET['Edit'])) {
	...
	echo '<tr>
		...
		<tr>
			...
			<td><textarea name="ItemDescription" value="' . $_SESSION['Request']->LineItems[$_GET['Edit']]->ItemDescription. '" cols="30" rows="5" >'.$_SESSION['Request']->LineItems[$_GET['Edit']]->ItemDescription.'</textarea></td>
		</tr>
		<tr>
	...
	//if (isset($_SESSION['Request']->LineItems[$_GET['Edit']]->rate)
	//	and $_SESSION['Request']->LineItems[$_GET['Edit']]->rate != '')
	//{
	
	$linetotal = (int)$_SESSION['Request']->LineItems[$_GET['Edit']]->Quantity
	* 
	(int)$_SESSION['Request']->LineItems[$_GET['Edit']]->rate
	;
	//echo '<input type="hidden" name="linetotal" value='.(floatval(round($_SESSION['Request']->LineItems[$_GET['Edit']]->rate,0))
	//*floatval(round($_SESSION['Request']->LineItems[$_GET['Edit']]->Quantity,0))).' />';
	//}
	echo '<input type="hidden" name="linetotal" value="321" />';
...
$sql = "select * from dba";
$result=DB_query($sql, $db);
echo '<td><select disabled="disabled" name="dba">';
while ($myrow=DB_fetch_array($result)){
...
foreach ($_SESSION['Request']->LineItems as $LineItems) {
	...
	echo '<td>' . $LineItems->LineNumber . '</td>
			...
			<td>' . locale_number_format($LineItems->rate, $LineItems->DecimalPlaces) . '</td>
			<td>' . $LineItems->UOM . '</td>
...
echo '<tr><td>Advance</td><td><input type = "text" name="advance" value = "' . $_SESSION['Request']->advance . '"></td></tr>';
