...
include('includes/SQL_CommonFunctions.inc');
	if(!userHasPermission($db, 'Supplier_Allocations')) {
		...
		foreach		 ($_SESSION['Alloc']->Allocs as $AllocnItem) {
			 ...
			 the transaction with the new alloc amount and diff on exch */
				     ...
				     if ($AllocnItem->AllocAmt > 0){
					     ...
					     $Result=DB_query($SQL, $db, $ErrMsg, $DbgMsg, True);
						 $totalAmountToPay =  $AllocnItem->AllocAmt;

						 $SQL = "SELECT type FROM supptrans WHERE id=".$AllocnItem->ID;
						...
						AND narrative LIKE '%Payment (".$_SESSION['Alloc']->AllocTrans.")%'";
						 ...
						 if($resc != 1){
							 ...
							 $SQL = "INSERT INTO gltrans (
								...
								'Against Payment (".$_SESSION['Alloc']->AllocTrans.")',
								'" . (-1*$totalAmountToPay) . "'
							)";
							...
							amount='".(-1*$totalAmountToPay)."',
							trandate = '".date('Y-m-d')."'
							WHERE account='".$_SESSION['Alloc']->BankAccount."'
							AND type='".$typeOfTrans."'
							AND typeno='".$AllocnItem->TypeNo."'
							AND narrative='Against Payment (".$_SESSION['Alloc']->AllocTrans.")'";
				 ...
				 if($typeOfTrans == 601){
					 ...
					 $SQL = "INSERT INTO gltrans (
							...
							'" . $PeriodNo . "',
							'2100',
							'Against MPI (".$AllocnItem->TypeNo.") and Payment (".$_SESSION['Alloc']->AllocTrans.")',
							'" . ($totalAmountToPay) . "'
						)";
...
If (isset($_GET['AllocTrans'])){
	...
	$SQL= "SELECT systypes.typename,
				...
				suppliers.suppname,
				supptrans.rate,
				supptrans.reverseHistory,
				supptrans.bankaccount,
...
if (isset($_POST['AllocTrans'])){
  ...
  unset($_SESSION['Alloc']);
   ...
   $sql = "SELECT id,
			...
			AND abs(ovamount+ovgst-alloc)>0.9
			AND settled=0
			AND supptrans.reversed=0
			ORDER BY id";
