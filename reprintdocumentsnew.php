...

{
	...
	{
		$sql = 'update dcs set dcstatus = "'.$_POST['invoicesent'.$i].'" where orderno = '.$i.'';
		DB_query($sql, $db);
	}
	if (isset ($_POST['dccomplete'.$i]))
	{
		$sql = 'update dcs set dccomplete = "'.$_POST['dccomplete'.$i].'" where orderno = '.$i.'';
		DB_query($sql, $db);
	}
}
for ($i=0; $i<=$_SESSION['maxdispatch']; $i++)
...
if (isset($_POST['ClearImagePurchaseOrder' .$i]) ) {
        $SQL = "UPDATE dcs SET podate='0000-00-00 00:00:00' WHERE orderno=".$i;
        DB_Query($SQL,$db);
    $SQL = "UPDATE dcs SET podeleted=1 WHERE orderno=".$i;
    DB_Query($SQL,$db);
		echo "namely";
		...
		/*   @unlink($_SESSION['part_pics_dir'] . '/' .'PurchaseOrder_'.$i.'.pdf');
		    if(is_file($_SESSION['part_pics_dir'] . '/' .'PurchaseOrder_'.$myrow['orderno'].'.pdf')) {
...
if (isset($_FILES['CourierSlip'.$i]) AND $_FILES['CourierSlip'.$i]['name'] !='') {
	...
	if ($UploadTheFile=='Yes'){
		...
		$sql = "UPDATE dcs
					...
					WHERE orderno='".$i."'";
        $SQL = "UPDATE dcs SET courierslipdeleted=1 WHERE orderno=".$i;
        DB_Query($SQL,$db);
...
if (isset($_POST['ClearImageCourierSlip' .$i]) ) {
    $SQL = "UPDATE dcs SET courierslipdate='0000-00-00 00:00:00' WHERE orderno=".$i;
    DB_Query($SQL,$db);
    $SQL = "UPDATE dcs SET courierslipdeleted=1 WHERE orderno=".$i;
    DB_Query($SQL,$db);
		   //workaround for many variations of permission issues that could cause unlink fail
...
if (isset($_POST['ClearImageInvoice' .$i]) ) {
    $SQL = "UPDATE dcs SET invoicedate='0000-00-00 00:00:00' WHERE orderno=".$i;
    DB_Query($SQL,$db);
    $SQL = "UPDATE dcs SET dcstatus='' WHERE orderno=".$i;
    DB_Query($SQL,$db);
    $SQL = "UPDATE dcs SET invoicedeleted=1 WHERE orderno=".$i;
    DB_Query($SQL,$db);
    DB_Query($SQL,$db);
		   //workaround for many variations of permission issues that could cause unlink fail
...
if (isset($_POST['ClearImageGRB' .$i]) ) {
    $SQL = "UPDATE dcs SET grbdate='0000-00-00 00:00:00' WHERE orderno=".$i;
    DB_Query($SQL,$db);
    $SQL = "UPDATE dcs SET grbdeleted=1 WHERE orderno=".$i;
    DB_Query($SQL,$db);
			//workaround for many variations of permission issues that could cause unlink fail
...
if ($_POST['document'] == 'IGP')
{
	...
	//$DemandResult = DB_query($linesql,$db,$ErrMsg);
		if ($k==1){
			...
			echo '<td><a href = "PDFIGP.php?RequestNo=' .$myrow['orderno']. '">
			'.$myrow['orderno'].'</a>
			</td>
...
if ($_POST['document'] == 'OGP')
{
		...
		$sql = "SELECT posdispatch.orderno,
		posdispatch.loccode,
		locations.locationname,
...
//	$DemandResult = DB_query($linesql,$db,$ErrMsg);
		if ($k==1){
			...
			echo '<td><a href = "PDFOGP.php?RequestNo=' .$myrow['orderno']. '">
			'.$myrow['orderno'].'</a>
			</td>
						<td>' .$myrow['locationname'] . '</td>
						<td>' .$myrow['despatchdate']. '</td>
...
if ($_POST['document'] == 'DC')
{
		...
		date_format(dcs.orddate, '%Y-%m-%d')>='".FormatDateForSQL($_SESSION['FromDate'])."'
			AND date_format(dcs.orddate, '%Y-%m-%d')<='".FormatDateForSQL($_SESSION['ToDate'])."'
	...
	if($_SESSION['AccessLevel'] == 22 || $_SESSION['AccessLevel'] == 8 || $_SESSION['AccessLevel'] == 10){
		...
		$sql = "SELECT * FROM dcs 
				...
				WHERE date_format(dcs.orddate, '%Y-%m-%d')>='".FormatDateForSQL($_SESSION['FromDate'])."'
				AND date_format(dcs.orddate, '%Y-%m-%d')<='".FormatDateForSQL($_SESSION['ToDate'])."'";
		
		if($_POST['user'] != "" && $_POST['user'] != "selected" && $_POST['user'] != "All")
			$sql .= ' AND salescase.salesman="'.$_POST['user'].'"';	
	...
	echo '<br />';
	    $TableHeader = '<tr>
    					<th>' . _('Document ID') . '</th>
						<th>' . _('For Client') . '</th>';
						if ($_SESSION['AccessLevel'] == 10 || $_SESSION['AccessLevel']  == 22|| $_SESSION['AccessLevel']  == 8 )
						{		
		...
		$TableHeader.= '<th>' . _('Location') . '</th>
				...
				if ($_SESSION['AccessLevel'] == 14||$_SESSION['AccessLevel'] == 22 || $_SESSION['AccessLevel'] == 23 || $_SESSION['AccessLevel'] == 8)
						{		
	...
	while ($myrow=DB_fetch_array($Result)) {
	$_SESSION['maxdispatch'] = $myrow['orderno'];
 echo  '<table cellpadding="5" cellspacing="4" class="selection">';
	...
	//$DemandResult = DB_query($linesql,$db,$ErrMsg);
		if ($k==1){
			...
			echo '<td><a href = "PDFDCh.php?DCNo=' .$myrow['orderno']. '&salescaseref=' .$myrow['salescaseref'] . '">
			'.$myrow['orderno'].'</a>
			</td><td><a href = "PDFDChExternal.php?DCNo=' .$myrow['orderno']. '">
			'.$myrow['orderno'].'</a>
			</td>';	
			if ($_SESSION['AccessLevel'] == 10 || $_SESSION['AccessLevel'] == 22 || $_SESSION['AccessLevel'] == 8)
						{		
		echo '<td><a target = "_blank" href="'.$RootPath.'/SelectOrderItemsDChexternalEdit.php?
		ModifyOrderNumber=' . $myrow['orderno'] . '&salescaseref=' . $salescaseref . '">'.'
		EDIT  DC no'.$myrow['orderno'].'</a>
			</td>';
						}
			echo '<td>' .$myrow['locationname'] . '</td>
						...
						if (userHasPermission($db, 'sent_for_invoicing') && $myrow['dcstatus'] == '')
						{
					echo	'<td><input type = "checkbox" name = "sendinvoice' .$myrow['orderno']. 
					'" value = "sent to accounts for invoice"></td>';
						}
					elseif(userHasPermission($db, 'sent_for_invoicing')) {	
					...
					if (userHasPermission($db, 'sent_for_invoicing') && $myrow['dccomplete'] == '')
						{
					echo	'<td><input type = "checkbox" name = "dccomplete' .$myrow['orderno']. '" value = "Delivery Completed"></td>';
						}
					else{	
		...
		<td><input type="file" id="PurchaseOrder" name="PurchaseOrder' .$myrow['orderno']. '" />
		 
		<br />';
		
		if($_SESSION['AccessLevel'] == 10 || $_SESSION['AccessLevel'] == 23 || $_SESSION['AccessLevel'] == 22 || $_SESSION['AccessLevel'] == 8)
		echo '<input type="checkbox" name="ClearImagePurchaseOrder' .$myrow['orderno']. '" id="ClearImagePurchaseOrder' .$myrow['orderno']. '" value="1" > '._('Clear Image').'';
...
if (function_exists('imagecreatefromjpg')){
	$StockImgLink = '<img src="PurchaseOrderSTStockImage.php?automake=1&amp;textcolor=FFFFFF&amp;bgcolor=CCCCCC'.
		'&amp;StockID='.urlencode('PurchaseOrder_'.$myrow['orderno']).
		'&amp;text='.
		'&amp;width=100'.
		'&amp;height=100'.
		'" alt="" />';
} else {
	if( file_exists($_SESSION['part_pics_dir'] . '/' .'PurchaseOrder_'.$myrow['orderno'].'.pdf') ) {
		$StockImgLink = '<img src="' . $_SESSION['part_pics_dir'] . '/' . 'PurchaseOrder_'.$myrow['orderno'] . '.'.$ext.'" height="100" width="100" />';
	...
	if ($StockImgLink!=_('No Image')){
        $deleted=$myrow['podeleted']==1 ? "<span style='color: darkred'><br/>deleted</br></span>":"";
        echo $deleted;
	echo '' . _('Image') . '<br /><a target = "_blank" href = "'.$_SESSION['part_pics_dir'] . '/' .'PurchaseOrder_'.$myrow['orderno'].'.pdf'.'">download</a></td>';
	}
		...
		<td><input type="file" id="CourierSlip" name="CourierSlip' .$myrow['orderno']. '" />
		 ';
		
		if($_SESSION['AccessLevel'] == 10||$_SESSION['AccessLevel'] == 23 || $_SESSION['AccessLevel'] == 22 || $_SESSION['AccessLevel'] == 8)
		echo '<br /><input type="checkbox" name="ClearImageCourierSlip' .$myrow['orderno']. '" id="ClearImageCourierSlip' .$myrow['orderno']. '" value="1" > '._('ClearImage').'';
$path = $_FILES['CourierSlip'.$i]['name'];
$ext = pathinfo($path, PATHINFO_EXTENSION);

if (function_exists('imagecreatefromjpg')){
	$StockImgLink = '<img src="CourierSlipSTStockImage.php?automake=1&amp;textcolor=FFFFFF&amp;bgcolor=CCCCCC'.
		'&amp;StockID='.urlencode('CourierSlip_'.$myrow['orderno']).
		'&amp;text='.
		'&amp;width=100'.
		'&amp;height=100'.
		'" alt="" />';
} else {
	if( file_exists($_SESSION['part_pics_dir'] . '/' .'CourierSlip_'.$myrow['orderno'].'.pdf') ) {
		$StockImgLink = '<img src="' . $_SESSION['part_pics_dir'] . '/' . 'CourierSlip_'.$myrow['orderno'] . '.'.$ext.'" height="100" width="100" />';
	...
	if ($StockImgLink!=_('No Image')){
        $deleted=$myrow['courierslipdeleted']==1 ? "<span style='color: darkred'><br/>deleted</br></span>":"";
        echo $deleted;
	echo '' . _('Image') . '<br /><a target = "_blank" href = "'.$_SESSION['part_pics_dir'] . '/' .'CourierSlip_'.$myrow['orderno'].'.pdf'.'">download</a></td>';
	}
		...
		<td><input type="file" id="Invoice" name="Invoice' .$myrow['orderno']. '" />
		 ';
		}
			else
			{
		...
		if($_SESSION['AccessLevel'] == 10 || $_SESSION['AccessLevel'] == 23 || $_SESSION['AccessLevel'] == 22 || $_SESSION['AccessLevel'] == 8)
		echo '<br /><input type="checkbox" name="ClearImageInvoice' .$myrow['orderno']. '" id="ClearImageInvoice' .$myrow['orderno']. '" value="1" > '._('Clear Image').'';
$path = $_FILES['Invoice'.$i]['name'];
$ext = pathinfo($path, PATHINFO_EXTENSION);

if (function_exists('imagecreatefromjpg')){
	$StockImgLink = '<img src="InvoiceSTStockImage.php?automake=1&amp;textcolor=FFFFFF&amp;bgcolor=CCCCCC'.
		'&amp;StockID='.urlencode('Invoice_'.$myrow['orderno']).
		'&amp;text='.
		'&amp;width=100'.
		'&amp;height=100'.
		'" alt="" />';
} else {
	if( file_exists($_SESSION['part_pics_dir'] . '/' .'Invoice_'.$myrow['orderno'].'.pdf') ) {
		$StockImgLink = '<img src="' . $_SESSION['part_pics_dir'] . '/' . 'Invoice_'.$myrow['orderno'] . '.'.$ext.'" height="100" width="100" />';
	...
	if ($StockImgLink!=_('No Image')){
	    $deleted=$myrow['invoicedeleted']==1 ?"<span style='color: darkred'><br/>deleted</br></span>":"";
	    echo $deleted;
	echo '' . _('Image') . '<br /><a target = "_blank" href = "'.$_SESSION['part_pics_dir'] . '/' .'Invoice_'.$myrow['orderno'].'.pdf'.'">download</a></td>';
	}
		...
		<td><input type="file" id="GRB" name="GRB' .$myrow['orderno']. '" />
		 ';
		
		if($_SESSION['AccessLevel'] == 10 || $_SESSION['AccessLevel'] == 23 || $_SESSION['AccessLevel'] == 22 || $_SESSION['AccessLevel'] == 8)
		echo '<br /><input type="checkbox" name="ClearImageGRB' .$myrow['orderno']. '" id="ClearImageGRB' .$myrow['orderno']. '" value="1" > '._('Clear Image').'';
$path = $_FILES['GRB'.$i]['name'];
$ext = pathinfo($path, PATHINFO_EXTENSION);

if (function_exists('imagecreatefromjpg')){
	$StockImgLink = '<img src="GRBSTStockImage.php?automake=1&amp;textcolor=FFFFFF&amp;bgcolor=CCCCCC'.
		'&amp;StockID='.urlencode('GRB_'.$myrow['orderno']).
		'&amp;text='.
		'&amp;width=100'.
		'&amp;height=100'.
		'" alt="" />';
} else {
	if( file_exists($_SESSION['part_pics_dir'] . '/' .'GRB_'.$myrow['orderno'].'.pdf') ) {
		$StockImgLink = '<img src="' . $_SESSION['part_pics_dir'] . '/' . 'GRB_'.$myrow['orderno'] . '.'.$ext.'" height="100" width="100" />';
	...
	if ($StockImgLink!=_('No Image')){
        $deleted=$myrow['grbdeleted']==1 ? "<span style='color: darkred'><br/>deleted</br></span>":"";
        echo $deleted;
	echo '' . _('Image') . '<br /><a target = "_blank" href = "'.$_SESSION['part_pics_dir'] . '/' .'GRB_'.$myrow['orderno'].'.pdf'.'">download</a></td>';
	}
