...
// Check the date is OK
	...
	*/
	...
	if ($InputErrors==0){
		...
		$_SESSION['Items'.$identifier]->GSTadd = $_POST['GSTadd'];
		$_SESSION['Items'.$identifier]->services = $_POST['services'];
		$_SESSION['Items'.$identifier]->WHT = $_POST['WHT'];
...
/* finally write the order header to the database and then the order line details */
	...
	$sqlono = 'SELECT MAX(orderno) as maxorderno from salesorders ';
	...
	$HeaderSQL = "INSERT INTO salesorders (
							...
							VALUES (
								...
								'" . $_SESSION['Items'.$identifier]->GSTadd ."',
								'" . $_SESSION['Items'.$identifier]->services ."',
								'" . $_SESSION['Items'.$identifier]->WHT ."',
								'" . $_SESSION['Items'.$identifier]->Location ."',
								'" . $DelDate . "',
								'" . $QuotDate . "',
								'" . $ConfDate . "',
		...
		if ($_POST['Quotation']==0) { /*then its not a quotation its a real order */
			...
			echo '<br /><table class="selection">
					<tr>
						...
						<td>' . ' ' . '<a href="' . $RootPath . '/PDFQuotation.php?identifier='.$identifier . '&amp;QuotationNo=' . $OrderNo . '">' .  _('Print Quotation (Internal)')  . '</a></td>
					</tr>
					</table>';
			echo '<br /><table class="selection">
					<tr>
						<td><img src="'.$RootPath.'/css/'.$Theme.'/images/reports.png" title="' . _('Order') . '" alt="" /></td>
						<td>' . ' ' . '<a href="' . $RootPath . '/PDFQuotationExternal.php?identifier='.$identifier . '&amp;QuotationNo=' . $OrderNo . '">' .  _('Print Quotation (External)')  . '</a></td>
					</tr>
					</table>';
		}
		echo '<br /><table class="selection">
...
$HeaderSQL = "UPDATE salesorders SET debtorno = '" . $_SESSION['Items'.$identifier]->DebtorNo . "',
										...
										GSTadd = '" . $_SESSION['Items'.$identifier]->GSTadd ."',
										
										afterdays = '" . $_SESSION['Items'.$identifier]->afterdays ."',
										
										services = '" . $_SESSION['Items'.$identifier]->services ."',
										WHT = '" . $_SESSION['Items'.$identifier]->WHT ."',
	...
	$LineItemsSQL = "UPDATE salesorderdetails SET unitprice='" . $StockItem->Price . "',
						...
						AND salesorderdetails.internalitemno='" . $StockItem->LineNumber . "'";
	...
	if($Quotation){ //handle Quotations and Orders print after modification
		...
		echo '<br /><table class="selection">
				<tr>
					<td><img src="'.$RootPath.'/css/'.$Theme.'/images/reports.png" title="' . _('Order') . '" alt="" /></td>
					<td>' . ' ' . '<a href="' . $RootPath . '/PDFQuotationExternal.php?identifier='.$identifier . '&amp;QuotationNo=' . $_SESSION['ExistingOrder'.$identifier] . '">' .  _('Print Quotation (External)')  . '</a></td>
				</tr>
				</table>';
	}else{
...
echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />';
	...
	$sqlL="select MAX(orderlineno) as maxlineno from salesorderdetails where orderno = ".$_SESSION['ExistingOrder' .$identifier].""; 		
...
for ($line=0; $line<= $resultrow['maxlineno']; $line++ )
{
	...
	WHERE salesorderlines.orderno='" . $_SESSION['ExistingOrder' .$identifier] . "'
	AND salesorderlines.lineno='" .$line. "'
	";
					$resultB = DB_query($sqlB,$db);
			$TransferRowB = DB_fetch_array($resultB);
$sqlO="select MAX(lineoptionno) as maxlineoptionno from salesorderdetails where orderno = ".$_SESSION['ExistingOrder' .$identifier]."
...
for ($option=0; $option<= $resultrowO['maxlineoptionno']; $option++ )
	...
	WHERE salesorderoptions.orderno='" . $_SESSION['ExistingOrder' .$identifier]. "'
...
if(1){
	...
	if ($_SESSION['Items'.$identifier]->Quotation=1){
		echo '<tr><th colspan="7">' . '<h3>Line No. '.($line+1).' Option No. '.($option+1).'</h3>' . '</th></tr>';
...
echo	'
		<tr><td>
			After Days </td><td><select name="afterdays">';
...
if ($_SESSION['Items'.$identifier]->afterdays==''){
	...
	echo '<option value="">' . _('Select Credit Days') . '</option>
		  ...
		  ';
			...
			echo'</select></td></tr>';
			if ($_SESSION['Items'.$identifier]->services==1)
			echo '<tr><td>Services</td><td><input type="checkbox" name="services" value=1 checked="checked"></td></tr>';
			else
			echo '<tr><td>Services</td><td><input type="checkbox" name="services" value=1></td></tr>';
		echo	'
		<tr><td>
			GST </td><td><select name="GSTadd">';
			
if ($_SESSION['Items'.$identifier]->GSTadd==''){
	echo '<option selected="selected" value="">' . _('None') . '</option>
          <option value="inclusive">' . _('Inclusive GST') . '</option>
          <option value="exclusive">' . _('Exclusive GST') . '</option>
          
		  ';
		  
} else if ($_SESSION['Items'.$identifier]->GSTadd=='inclusive'){
	echo '<option value="">' . _('None') . '</option>
          <option  selected="selected" value="inclusive">' . _('Inclusive GST') . '</option>
          <option value="exclusive">' . _('Exclusive GST') . '</option>
          
		  ';
		  }
 else if ($_SESSION['Items'.$identifier]->GSTadd=='exclusive'){
	echo '<option value="">' . _('None') . '</option>
          <option   value="inclusive">' . _('Inclusive GST') . '</option>
          <option selected="selected" value="exclusive">' . _('Exclusive GST') . '</option>
		...
		echo	'
		<tr><td>
			Witholding Tax </td><td><select name="WHT">';
			
if ($_SESSION['Items'.$identifier]->WHT== 0){
	echo '<option selected="selected" value=0>' . _('None') . '</option>
          <option value=4.5>' . _('4.5%') . '</option>
          <option value=6.5>' . _('6.5%') . '</option>
          
		  ';
		  
} else if ($_SESSION['Items'.$identifier]->WHT==4.5){
	echo '<option value=0>' . _('None') . '</option>
          <option selected="selected" value=4.5>' . _('4.5%') . '</option>
          <option value=6.5>' . _('6.5%') . '</option>
          
		  ';
		  }
 else if ($_SESSION['Items'.$identifier]->WHT==6.5){
	echo '<option value=0>' . _('None') . '</option>
          <option value=4.5>' . _('4.5%') . '</option>
          <option selected="selected" value=6.5>' . _('6.5%') . '</option>
