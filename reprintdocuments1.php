...
if ($_POST['document'] == 'IGP')
{
		...
		or igp.receivedfrom like '%Head Office%'
		)
 ...
 echo  '<table cellpadding="5" cellspacing="4" class="selection">';
	...
	//$DemandResult = DB_query($linesql,$db,$ErrMsg);
		if ($k==1){
				...
				echo $TableHeader;
			echo '<td><a href = "PDFDC.php?RequestNo=' .$myrow['dispatchid']. '">
			'.$myrow['dispatchid'].'</a>
		...
		echo '<td><a href = "dcclient.php?tranno=' .$myrow['dispatchid']. '">
			...
			echo '<td>' .$myrow['locationname'] . '</td>
						...
						if ($_SESSION['AccessLevel'] == 14 and $myrow['dcstatus'] == '')
						{
					echo	'<td><input type = "checkbox" name = "sendinvoice' .$myrow['dispatchid']. '" value = "sent to accounts for invoice"></td>';
						}
					elseif($_SESSION['AccessLevel'] == 14){	
					echo	'<td style = "color:red">' .$myrow['dcstatus']. '</td>';
					}
						if ($_SESSION['AccessLevel'] != 14 and $myrow['dcstatus'] == 'sent to accounts for invoice')
						{
					echo	'<td style = "color:red"><input type = "checkbox" name = "invoicesent' .$myrow['dispatchid']. '" value = "This DC is invoiced"></td>';
						}
					elseif($_SESSION['AccessLevel'] != 14){	
					echo	'<td style = "color:red">' .$myrow['dcstatus']. '</td>';
					}
					if ($_SESSION['AccessLevel'] == 14 and $myrow['dccomplete'] == '')
						{
