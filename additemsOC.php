...
$nm='addnewitem'.$count;
	 ...
	 if (1)  
		...
		{
$sqlnewlineno="select MAX(internalitemno) as maxinternalitemno from ocdetails where
...
<input type="hidden" name="lne" value='. $_GET['line'].'>
			...
			$sqlOI="select MAX(optionitemno) as maxoptionitemno from ocdetails where
...
{$optionitemno=0;}
		...
		$SQL="SELECT * FROM ocdetails where orderno='".$_SESSION['orderno']."' AND orderlineno='".$_GET['line'] ."'
		AND lineoptionno='".$_GET['option'] ."'";
	...
	echo '" /></td>
		...
		if (isset($SearchResult)) {
			...
			while ($myrow=DB_fetch_array($SearchResult)) {
				...
				$sql = "SELECT SUM(ocdetails.quantity-ocdetails.qtyinvoiced) AS dem
						FROM ocdetails INNER JOIN ocs
						ON ocs.orderno = ocdetails.orderno
						 WHERE  ocs.fromstkloc='" . $_SESSION['loccode'] . "'
						 AND ocdetails.completed=0
						 AND ocs.quotation=0
						 AND ocdetails.stkcode='" . $myrow['stockid'] . "'";
