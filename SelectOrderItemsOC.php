...
if (isset($_GET['NewOrder'])){
  /*New order entry - clear any existing order details from the Items object and initiate a newy*/
	...
	$_SESSION['Items'.$identifier] = new cart;
		...
		$sql='DELETE FROM oclines where orderno = 0
	
	';
	DB_query($sql,$db);
		$sql='DELETE FROM ocoptions where orderno = 0
	
	';
	DB_query($sql,$db);
	
		$sql='INSERT INTO oclines (orderno,lineno) VALUES ('.$identifier.',
	0
	
	)';
	DB_query($sql,$db);
	
	$sql='INSERT into ocoptions (orderno,lineno,optionno) VALUES ('.$identifier.',
	0,
	0
	...
	$newlinesql='SELECT * FROM oclines where orderno = "'.$_SESSION['ExistingOrder' .$identifier].'"
	AND lineno=0
	';
	...
	$newlinerow = DB_fetch_array($newlineresult);
		...
		$newoptionsql='SELECT * FROM ocoptions where orderno = "'.$_SESSION['ExistingOrder' .$identifier].'"
...
/*read in all the guff from the selected order into the Items cart  */
	...
	$OrderHeaderSQL = "SELECT ocs.debtorno,
			 				  debtorsmaster.name,
							  ocs.branchcode,
							  ocs.customerref,
							  ocs.comments,
						...
						ON ocs.ordertype=salestypes.typeabbrev
						...
						WHERE ocs.orderno = '" . $_GET['ModifyOrderNumber'] . "'";
...
/*need to look up customer name from debtors master then populate the line items array with the sales order details records */
		...
		$LineItemsSQL = "SELECT ocdetails.internalitemno,
								...
								AND ocdetails.orderno ='" . $_GET['ModifyOrderNumber'] . "'
	...
	$externallinessql = "SELECT *
								FROM oclines 
								WHERE  oclines.orderno ='" . $_GET['ModifyOrderNumber'] . "'
								ORDER BY oclines.lineno";
...
//($externallinessqlresult);
	...
	$lineoptionsssql = "SELECT *
								FROM ocoptions 
								WHERE  ocoptions.orderno ='" . $_GET['ModifyOrderNumber'] . "'
								AND 	ocoptions.lineno ='" . $myrow['lineno'] . "'
								ORDER BY ocoptions.lineno";
...
// everything below here only do if a customer is selected
 	...
 	if (isset($_POST['CancelOrder'])) {
		...
		if ($OK_to_delete==1){
			if($_SESSION['ExistingOrder' . $identifier]!=0){
				...
				$SQL = "DELETE FROM ocdetails WHERE ocdetails.orderno ='" . $_SESSION['ExistingOrder' . $identifier] . "'";
				$ErrMsg =_('The order detail lines could not be deleted because');
				$DelResult=DB_query($SQL,$db,$ErrMsg);

				$SQL = "DELETE FROM ocs WHERE ocs.orderno='" . $_SESSION['ExistingOrder' . $identifier] . "'";
	...
	//	echo '</b><br />' . _('Terms') . ':<b> ' . $_SESSION['Items'.$identifier]->PaymentTerms;
		...
		echo '<table border="1"><tr><td><a href="SelectOrderItemsOC.php?identifier='.$identifier .'">Bill of Quantities</a></td><td><a href="SelectOrderItemsOCexternal.php?identifier='.$identifier .'">Descriptions</a></td></tr></table>';
...
unset($_SESSION[$nm]);
			...
			if (DB_num_rows($KitResult)==0){
				...
				if ($myrow['mbflag']=='K'){	/*It is a kit set item */
					...
					while ($KitParts = DB_fetch_array($KitResult,$db)){
						...
						$NewPOLine = 0;
						include('includes/SelectOrderItems_IntoCartOC.inc');
					}
	...
	if ((isset($_SESSION['Items'.$identifier])) OR isset($NewItem)){
		...
		$SQL='SELECT MAX(lineoptionno) as maxoption FROM ocdetails WHERE orderno="'.$_SESSION['ExistingOrder'.$identifier].'"
			AND orderlineno="'.$_GET['line'].'"';
$result=DB_query($SQL,$db);
$row=DB_fetch_array($result);
		if(isset($_GET['DeleteOption']) AND $row['maxoption']>0){
		$SQL='DELETE FROM ocoptions where orderno="'.$_SESSION['ExistingOrder'.$identifier].'"
...
DB_query($SQLD,$db);
			$SQL='DELETE FROM ocdetails where orderno="'.$_SESSION['ExistingOrder'.$identifier].'"
...
echo'<script>
window.location.assign("'.$RootPath.'/SelectOrderItemsOC.php?ModifyOrderNumber=' . $_SESSION['ExistingOrder' . $identifier] . '&salescaseref=' . $_SESSION['salescaseref'] . '");
		</script>';

		}
 $SQLline='SELECT MAX(lineno) as maxline FROM oclines WHERE orderno="'.$_SESSION['ExistingOrder'.$identifier].'"
		';
...
//	echo $_POST[$lineindex].'<br>'.$rowline['maxline'];
	// $_SESSION[$lineindex]=$_POST[$lineindex];
		...
		$SQL='DELETE FROM oclines where orderno="'.$_SESSION['ExistingOrder'.$identifier].'"
 ...
 $count=0;
	...
	//	echo $count;
			...
			$SQLD = "
					...
					orderno='" . $_SESSION['ExistingOrder' . $identifier] . "'AND orderlineno='".$line."'
...
window.location.assign("'.$RootPath.'/SelectOrderItemsOC.php?ModifyOrderNumber=' . $_SESSION['ExistingOrder' . $identifier] . '&salescaseref=' . $_SESSION['salescaseref'] . '");
 ...
 $AlreadyWarnedAboutCredit = false;
	...
	$sqlex = "UPDATE oclines SET clientrequirements='" . $clientrequirements . "',
															orderno=" . $_SESSION['ExistingOrder'.$identifier] . "
...
//			$result = DB_query($sqlex
	...
	if (isset($_POST['DeliveryDetails'])){
		echo '<meta http-equiv="refresh" content="0; url=' . $RootPath . '/DeliveryDetailsOC.php?identifier='.$identifier . '">';
		prnMsg(_('You should automatically be forwarded to the entry of the delivery details page') . '. ' . _('if this does not happen') . ' (' . _('if the browser does not support META Refresh') . ') ' .
		   '<a href="' . $RootPath . '/DeliveryDetailsOC.php?identifier='.$identifier . '">' . _('click here') . '</a> ' . _('to continue'), 'info');
	   	exit;
	}
...
$sqlL="select MAX(lineno) as maxlineno from oclines where orderno = ".$_SESSION['ExistingOrder' .$identifier].""; 		
...
if (isset($_POST['newline']) AND $_POST['newline']!='0' AND $_SESSION['flag']!="up")
{
	...
	$linescount++;
	$sql='INSERT into oclines (orderno,lineno) VALUES ("'.$_SESSION['ExistingOrder' .$identifier].'",
	'.$linescount.'
	...
	$sql='INSERT into ocoptions (orderno,lineno,optionno) VALUES ("'.$_SESSION['ExistingOrder' .$identifier].'",
	"'.$linescount.'",
	0
...
for ($line=0; $line<= $linescount; $line++ )
{
	...
	$sqlO="select MAX(optionno) as maxlineoptionno from ocoptions where orderno = ".$_SESSION['ExistingOrder' .$identifier]."
AND lineno = ".$line.""; 		
...
<input type="hidden" name = "clientreq'.$line.'" readonly="readonly">
...
echo '</td>
			...
			id="LineTotal_' . $OrderLine->LineNumber . '" 
					...
					$SQLmaxinternalitemno="SELECT MAX(internalitemno) as maxinternalitemno 
					FROM ocdetails
 ...
 document.getElementById("Discount_"+y).value= ( (1 - document.getElementById("UnitRate_"+y).value / document.getElementById("PriceJS_"+y).value))*100;
...
if($_SESSION['ExistingOrder' . $identifier]!=$_GET['identifier'])
		...
		$DisplayTotal = locale_number_format($_SESSION['Items'.$identifier]->total,$_SESSION['Items'.$identifier]->CurrDecimalPlaces);
			...
			echo'</table>';
			echo'<a   href = "'.$RootPath.'/additemsOC.php?line='.$line.'
...
//	'',$_SESSION['Items'.$identifier]->externallines[$line]->lineoptions[$option]->optiontext);
	...
	echo'<table width="100%" cellpadding="2">
		...
		<tr>
		<td colspan="8"><input type="hidden" name = "SAHdesc'.$line.'_'.$option.' readonly="readonly""
		></td>
...
echo'	<br/>
	...
	<input type="submit" name="DeliveryDetails" value="' . _('Enter Delivery Details and Confirm OC') . '" />
