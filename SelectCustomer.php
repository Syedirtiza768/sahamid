...
if(!userHasPermission($db, 'select_customer')) {
...
if (isset($_POST['Search']) OR isset($_POST['CSV']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
	...
	if (($_POST['Keywords'] == '') AND ($_POST['CustCode'] == '') AND ($_POST['CustPhone'] == '') AND ($_POST['CustType'] == 'ALL') AND ($_POST['Area'] == 'ALL') AND ($_POST['CustAdd'] == '')) {
		...
		$SQL = "SELECT debtorsmaster.debtorno,
				...
				INNER JOIN salesman ON custbranch.salesman = salesmancode";
	} else {
		...
		$SQL = "SELECT debtorsmaster.debtorno,
					...
					LEFT JOIN custbranch
						ON debtorsmaster.debtorno = custbranch.debtorno
					LEFT OUTER JOIN salesman ON custbranch.salesman = salesmancode
	...
	if ($_SESSION['Extended_CustomerInfo'] == 1) {
		...
		if (DB_num_rows($result) <> 0) {
			...
			echo '<tr>
				...
				echo '<th> <a href="AddCustomerContacts.php?DebtorNo=' . $_SESSION['CustomerID'] . '">' . _('Add New Contact') . '</a> </th>';
