	...
	while ($myrow = DB_fetch_row($result)) {
		...
		if ((trim($_SESSION['SQLString']['table']) == $_POST['SelectedTable'])  ||
			...
			echo '<tr style="background-color: '.$RowColour.'">
				...
				echo $_SESSION['SQLString']['fields'][0].
				'</td>
			...
			for ($i=1; $i<sizeof($_SESSION['SQLString']['fields']); $i++) {
				...
				(trim($_SESSION['SQLString']['fields'][$i]) != 'www_users.password')) {
					echo '<tr style="background-color:' . $RowColour . '">';
					if (trim($_SESSION['SQLString']['table']) == "stockmaster" )
					{
					if ($i<7 || $i >21)
					{
					echo '<td></td>
				...
				echo '<td></td>
						...
						<td></td>
						<td></td>';
					 if (trim($_SESSION['SQLString']['fields'][$i]) == "stkcatpropid" )
					{
					
				$sqlA = "select label from stockcatproperties where stockcatproperties.stkcatpropid =" .
					(int)trim(str_replace("'","",$_SESSION['SQLString']['values'][$i]));
					
						$resultA = DB_query($sqlA,$db);
						$myrowA = DB_fetch_row($resultA);
