...
if (isset($_POST['submit'])) {
	...
	if ( $InputError !=1 ) {
		...
		$checkSql = "SELECT count(*)
				 ...
				 AND conditionID = '" .  $_POST['SelectedCondition'] . "'";
		...
		if ( $checkrow[0] >0) {
			$InputError = 1;
			prnMsg( _('The Stock Condition') . ' ' . $_POST['conditionID'] . ' ' ._('already allowed as internal for this security role'),'error');
		} else {
			// Add new record on submit
			$sql = "INSERT INTO internalstockcatrole (secroleid,
												conditionID)
										VALUES ('" . $_POST['SelectedRole'] . "',
												'" . $_POST['SelectedCondition'] . "')";

			$msg = _('Stock Condition:') . ' ' . $_POST['SelectedCondition'].' '._('has been allowed to user role') .' '. $_POST['SelectedRole'] .  ' ' . _('as internal');
	...
	//run the SQL from either of the above possibilites
		...
		prnMsg($msg,'success');
		unset($_POST['SelectedCondition']);
	}

} elseif ( isset($_GET['delete']) ) {
	$sql="DELETE FROM internalstockcatrole
		WHERE secroleid='".$SelectedRole."'
		AND conditionID='".$SelectedType."'";

	$ErrMsg = _('The Stock Condition by Role record could not be deleted because');
	$result = DB_query($sql,$db,$ErrMsg);
	prnMsg(_('Internal Stock Condition').' '. $SelectedType .' '. _('for user role').' '. $SelectedRole .' '. _('has been deleted') ,'success');
	unset($_GET['delete']);
}
...
if (!isset($SelectedRole)){
	...
	while ($myrow = DB_fetch_array($result)) {
		...
		echo $myrow['secroleid'] . '">' . ' - ' . $myrow['secrolename'] . '</option>';
...
if (isset($_POST['process'])OR isset($SelectedRole)) {
	...
	$sql = "SELECT internalstockcatrole.conditionID,
					itemcondition.abbreviation
			FROM internalstockcatrole INNER JOIN itemcondition
			ON internalstockcatrole.conditionID=itemcondition.conditionID
			WHERE internalstockcatrole.secroleid='".$SelectedRole."'
			ORDER BY internalstockcatrole.conditionID ASC";
...
while ($myrow = DB_fetch_array($result)) {
	...
	if (! isset($_GET['delete'])) {
		...
		echo '<tr><td>' . _('Select Stock Condition Code') . ':</td><td><select name="SelectedCondition">';
		...
		$result = DB_query($SQL,$db);
		if (!isset($_POST['SelectedCondition'])){
			echo '<option selected="selected" value="">' . _('Not Yet Selected') . '</option>';
		}
		while ($myrow = DB_fetch_array($result)) {
			if (isset($_POST['SelectedCondition']) AND $myrow['conditionID']==$_POST['SelectedCondition']) {
				echo '<option selected="selected" value="';
			} else {
				echo '<option value="';
			}
			echo $myrow['conditionID'] . '">' . $myrow['conditionID'] . ' - ' . $myrow['abbreviation'] . '</option>';
