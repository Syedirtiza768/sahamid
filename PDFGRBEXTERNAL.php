...
$html2.= '<td width = "30%" align = "center">'.'Item'.'</td>';
...
$sqlA = "SELECT *,SUM(quantity) as qty
	    ...
	    GROUP BY grbdetails.orderlineno,grbdetails.stkcode,grbdetails.quantity";
...
while ($TransferRow = DB_fetch_array($resultA)){
    ...
    $html2.= '<td width = "30%" >'.$TransferRow['categorydescription'].'</td>';
...
//$pdf->writeHTML($html3, true, false, true, false, '');
 ob_end_clean();
$pdf->OutputD($_SESSION['DatabaseName'] . '_GRB_' . $_GET['grbno'] . '.pdf');
$pdf->__destruct();
?>