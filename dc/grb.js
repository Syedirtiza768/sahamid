		...
		$.ajax({
		    ...
		    success: function(response) { 
		    	...
		    	if(status == "success"){
		    		...
		    		window.currentCredit = (data.credit) ? data.credit:0;
		    		window.creditLimit = data.creditlimit;
		...
		for(index in dirtyEditors){
			...
			if(dirtyEditors[index].length > 0){
				...
				if(dEditor.editor.content.isDirty()){
					...
					$.ajax({
						type: 'POST',
					    url: rootpath+"/dc/api/saveDescription.php",
	...
	function addline(){
		...
		$.ajax({
			type: 'POST',
		    url: rootpath+"/dc/api/createNewLine.php",
	...
	function addlineCallback(line, requirements){
		...
		descHTML += '<textarea  style="display:none; width: 100%; height: 100%;" id = "l'+line+'o---desc" cols="90" rows="10" readonly></textarea>';
	...
	function addoptionCallback(line,option, description,stockstatus, quantity){
		...
		descHTML += '</tbody>';
		descHTML += '</table>';
		descHTML += '<textarea  style="width: 100%; height: 100%;" id = "l'+line+'o'+option+'desc" cols="90" rows="10" readonly></textarea>';
    ...
    function additemCallback(indx,line,option,stkc,bname,description,quantity,sprice,uprice,tot,qoh,dis,update,modal,part){
        ...
        html+='</div>';
        html+='<div class="col-md-4">';
        html+='<input class="discount" type="number" value="'+dis+'" disabled>';
        html+='</div>';
        html+='<div class="col-md-2">';
        html+='UnitRate:';
        html+='</div>';
        html+='<div class="col-md-4">';
        html+='<input class="uprice" type="number" value="'+uprice+'" disabled>';
        html+='</div>';
        html+='<br>';
        html+='<br>';
        html+='<div class="col-md-2">';
	...
	function removeitem(line, option, index, description){
		...
		function(){
		  ...
		  $.ajax({
		  	type: 'POST',
		    url: rootpath+"/dc/api/deleteItem.php",
		    data: {itemIndex: index,grbno,FormID:formID},
		    dataType: "json",
		    success: function(response) { 
	...
	function removeoption(line, option, voption){
		...
		function(){
		  ...
		  $.ajax({
		  	type: 'POST',
		    url: rootpath+"/dc/api/deleteOption.php",
	...
	function removeline(line, vline){
		...
		function(){
		  ...
		  $.ajax({
		  	type: 'POST',
		    url: rootpath+"/dc/api/deleteLine.php",
	...
	function updateQuantity(item,quantity){
		...
		$.ajax({
			type: 'POST',
		    url: rootpath+"/dc/api/updateItem.php",
		    data: {orderno: order,grbno, salescaseref: salesref, name: "quantity",value:quantity,item:item,FormID:formID},
		    dataType: "json",
		    success: function(response) { 
		    	var status = response.status;
	...
	$("#bss").click(function(e){
		...
		$.ajax({
			...
			type: 'POST',
		    url: rootpath+"/dc/api/search.php",
	...
	function insertitem(itemindex, brand){
		...
		$.ajax({
			type: 'POST',
		    url: rootpath+"/dc/api/addNewItem.php",
	...
	$(document.body).on('change','.order-detailss',function(){
	    ...
	    $.ajax({
			type: 'POST',
		    url: rootpath+"/dc/api/updateOrderDetails.php",
	...
	$(document.body).on('change','.descinfo',function(){
	    ...
	    $.ajax({
			type: 'POST',
		    url: rootpath+"/dc/api/updateOption.php",
	...
	$("#erbtn").click(function(e){
		...
		$.ajax({
			type: 'GET',
		    url: rootpath+"/previewDC.php?DCNo="+order+"&salescaseref="+salesref,
		    dataType: "json",
		    success: function(response) { 

	    		var ihtml = "<iframe ";
				ihtml += 'id="previewframe" ';
				ihtml += 'src = "../ViewerJS/#../tempPDF/dc/'+order+'int.pdf" ';
	...
	$("#checkforwarnings").click(function(e){
		...
		$.ajax({
			type: 'POST',
		    url: rootpath+"/dc/api/saveDC.php",
	...
	$("#updategroupdiscountsbutton").on('click',function(){
		...
		$.ajax({
			type: 'POST',
			url: rootpath+'/dc/api/groupDiscount.php',
	...
	function updateQuotationValue(){
		...
		if((window.currentCredit + quotTotal) > window.creditLimit){
			...
			overLimit = ` ( ${(window.currentCredit + quotTotal) - window.creditLimit} Over Credit Limit)`;
		}else{
			$('#totalquotationvalue').css("color","#424242");
			overLimit = ` ( ${window.creditLimit - (window.currentCredit + quotTotal)} Credit Remaining)`;
		}
		...
		$('#totalquotationvalue').html(quotTotal+overLimit);
