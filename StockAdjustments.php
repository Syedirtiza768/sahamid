...
if (isset($_POST['CheckCode'])) {
	...
	if (mb_strlen($_POST['StockText'])>0) {
		...
		$sql="SELECT stockid,
				...
				or mnfCode " . LIKE  . " '%" . $_POST['StockCode'] ."%'
				";
	}
...
/*Insert the StockSerialMovements and update the StockSerialItems  for controlled items*/
		...
		$Result = DB_query($SQL, $db, $ErrMsg, $DbgMsg, true);
	echo	$SQL = "UPDATE substorestock SET quantity = quantity + " . floatval($_SESSION['Adjustment' . $identifier]->Quantity) . "
				WHERE stockid='" . $_SESSION['Adjustment' . $identifier]->StockID . "'
				AND substoreid=" . $_POST['StockLocationFrom'] . "";
...
echo '<tr>
		...
		<td><input type="text" name="StockText" size="21" value="' . $_GET['Description'] .'" />&nbsp; &nbsp;'._('Partial Stock Code/Part Number/ Manufacturer Code'). ':</td>
		<td>';
if (isset($StockID)) {
...
//$sql = "select defaultlocation from ";
		//$sql = "SELECT substoreid, description FROM substores ";
		 $sql = "SELECT substoreid, description FROM substores where locid like '".$_SESSION['UserStockLocation']."'";
$resultStkLocs = DB_query($sql,$db);
while ($myrow=DB_fetch_array($resultStkLocs)){
