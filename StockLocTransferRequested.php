...
if (isset($_POST['authorizer' . $_POST['slct']])) {
	...
	$sql = "SELECT salescaseref FROM `ogpsalescaseref` WHERE `dispatchid` = '" . $RequestID . "' ";
	$Result = DB_query($sql, $db);
	while ($row = mysqli_fetch_assoc($Result)) {

		$salesperson = $row["salescaseref"];
	}

	$_SESSION['salescaseref'] = $salesperson;

	$sql = "SELECT salesman FROM `salescase` WHERE `salescaseref` = '" . $salesperson . "' ";
	$Result = DB_query($sql, $db);
...
if (isset($_POST['Submit']) and $InputError == False) {
	...
	if ($_POST['toloc' . $_POST['slct']] == $_POST['fromloc' . $_POST['slct']]) {
		...
		for ($i = 0; $i < $_POST['LinesCounter']; $i++) {
			$quantity = locale_number_format($_POST['StockQTY' . $i], 'Variable');
			$item = $_POST['StockID' . $i];
			...
			$requestedby = NULL;
			$sql = "SELECT requestedby FROM `ogpsalescaseref` WHERE `dispatchid` = '" . $Trf_ID . "' ";
			$Result = DB_query($sql, $db);
			while ($row = mysqli_fetch_assoc($Result)) {
