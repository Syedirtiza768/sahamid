...
if(isset($_POST['MakeRecurringOrder']) AND ! $InputErrors){
	...
	echo '<meta http-equiv="Refresh" content="0; url=' . $RootPath . '/Recurringdcs.php?identifier='.$identifier   .  '&amp;NewRecurringOrder=Yes">';
...
if (isset($_POST['BackToLineDetails']) and $_POST['BackToLineDetails']==_('Modify Order Lines')){
	...
	echo '<meta http-equiv="Refresh" content="0; url=' . $RootPath . '/SelectOrderItemsDCh.php?identifier='.$identifier   . '">';
...
if (isset($OK_to_PROCESS) AND $OK_to_PROCESS == 1 AND $_SESSION['ExistingOrder'.$identifier]==$identifier){
		...
		$OrderNo = GetNextTransNo(38, $db);
		$PeriodNo = GetPeriod (Date($_SESSION['DefaultDateFormat']), $db);
		
		$_SESSION['tranno'] = $OrderNo;
...
/* finally write the order header to the database and then the order line details */
	...
	$sqlono = 'SELECT MAX(orderno) as maxorderno from dcs ';
	...
	//external lines
	$StartOf_ExternalLinesSQL = "INSERT INTO dclines (
	...
	$ExternalLinesSQL = $StartOf_ExternalLinesSQL ."
		...
		$StartOf_LineOptionSQL = "INSERT INTO dcoptions (
...
//	echo $ExternalLinesSQL;
	...
	//end external lines
	$StartOf_LineItemsSQL = "INSERT INTO dcdetails (
											internalitemno,
														orderlineno,
														lineoptionno,
														optionitemno,
	...
	foreach ($_SESSION['Items'.$identifier]->LineItems as $StockItem) {
		...
		$SQL="SELECT quantity as optqty from dcoptions WHERE orderno=".$OrderNo." AND lineno =".$StockItem->orderlineno."
			AND optionno=".$StockItem->lineoptionno."";
		$ResultX = DB_query($SQL, $db);
		$rowX=DB_fetch_array($ResultX);	
		$SQL="SELECT locstock.quantity
						FROM locstock
						WHERE locstock.stockid='" . $StockItem->StockID . "'
		...
		$SQL = "INSERT INTO stockmoves (stockid,
					...
					VALUES (
						...
						'" . $_SESSION['Items'.$identifier]->Location . "',
							'" . Date('Y-m-d') . "',
						'" . _('Delivered To') . ' ' . DB_escape_string($_SESSION['Items'.$identifier]->DeliverTo) ."'
						,'" . round($StockItem->Quantity*$rowX['optqty'] ,0). "'
						,'" . $PeriodNo . "'
...
$SQL = "SELECT salesman from salescase where salescaseref='".$_SESSION['salescaseref']."'";
	...
	$SQL = "select stockid, salesperson from stockissuance where stockid = '" . $StockItem->StockID . "'
	
	and salesperson = '" . $row['salesman'] . "'
			...
			if (mysqli_num_rows($Result)>0)
				{
					...
					SET issued = issued - '" . round($StockItem->Quantity*$rowX['optqty'], 0) . "'
					WHERE stockid='" . $StockItem->StockID . "'
					AND salesperson='" .$row['salesman'] . "'";
	$Result = DB_query($SQL, $db);
			$SQL = "UPDATE stockissuance
					SET dc = dc + '" . round($StockItem->Quantity*$rowX['optqty'], 0) . "'
					WHERE stockid='" . $StockItem->StockID . "'
					AND salesperson='" .  $row['salesman'] . "'";
	$Result = DB_query($SQL, $db);
				...
				else
				{
					...
					('" . $row['salesman'] . "','" . $LineItems->StockID . "'
					,'" . round($StockItem->Quantity*$rowX['optqty'], 0) . "')";
	$Result = DB_query($SQL, $db);
				...
				$Odr=$OrderNo+1;
			$SQLDEL="DELETE FROM dcdetails where orderno>".$Odr."";
		DB_query($SQLDEL,$db);
		
		$SQLDEL="DELETE FROM dclines where orderno>".$Odr."";
		DB_query($SQLDEL,$db);
	$SQLDEL="DELETE FROM dcoptions where orderno>".$Odr ."";
		DB_query($SQLDEL,$db);
		...
		if ($StockItem->MBflag=='M'
			...
			$SQL = "SELECT SUM(dcdetails.quantity - dcdetails.qtyinvoiced) AS qtydemand
					FROM dcdetails INNER JOIN dcs
					ON dcdetails.dispatchid=dcs.dispatchid
					WHERE dcdetails.stkcode = '" . $StockItem->StockID . "'
					AND dcdetails.completed = 0
					AND dcs.quotation=0";
			$DemandResult = DB_query($SQL,$db);
			$DemandRow = DB_fetch_row($DemandResult);
			$QuantityDemand = $DemandRow[0];

			$SQL = "SELECT SUM((dcdetails.quantity-dcdetails.qtyinvoiced)*bom.quantity) AS dem
					FROM dcdetails INNER JOIN dcs
					ON dcdetails.dispatchid=dcs.dispatchid
					INNER JOIN bom ON dcdetails.stkcode=bom.parent
					INNER JOIN stockmaster ON stockmaster.stockid=bom.parent
					WHERE dcdetails.quantity-dcdetails.qtyinvoiced > 0
	...
	if ($_SESSION['Items'.$identifier]->Quotation==1){
		prnMsg(_('DC Number') . ' ' . $OrderNo . ' ' . _('has been entered'),'success');
	} else {
		...
		if ($_POST['Quotation']==0) { /*then its not a quotation its a real order */
			...
			echo '<br /><table class="selection">
					<tr>
						...
						<td>' . ' ' . '<a href="' . $RootPath . '/PDFDCh.php?identifier='.$identifier . '&amp;DCNo=' . $OrderNo . '">' .  _('Print DC (Internal)')  . '</a></td>
					</tr>
					</table>';
			echo '<br /><table class="selection">
					<tr>
						<td><img src="'.$RootPath.'/css/'.$Theme.'/images/reports.png" title="' . _('Order') . '" alt="" /></td>
						<td>' . ' ' . '<a href="' . $RootPath . '/PDFDChExternal.php?identifier='.$identifier . '&amp;DCNo=' . $OrderNo . '">' .  _('Print DC (External)')  . '</a></td>
					</tr>
					<tr>
						<td><img src="'.$RootPath.'/css/'.$Theme.'/images/reports.png" title="' . _('Order') . '" alt="" /></td>
						<td>' . ' ' . '<a href="' . $RootPath . '/PDFDChExternalWithoutRates.php?identifier='.$identifier . '&amp;DCNo=' . $OrderNo . '">' .  _('Print DC (External) Without Rates')  . '</a></td>
					</tr>
					<tr>
						<td><img src="'.$RootPath.'/css/'.$Theme.'/images/reports.png" title="' . _('Order') . '" alt="" /></td>
						<td>' . ' ' . '<a href="' . $RootPath . '/PDFDChExternalBill.php?identifier='.$identifier . '&amp;DCNo=' . $OrderNo . '">' .  _('Print DC (External) Bill')  . '</a></td>
					</tr>
					</table>';
		}
		echo '<br /><table class="selection">
...
/* update the order header then update the old order line details and insert the new lines */
	...
	$HeaderSQL = "UPDATE dcs SET debtorno = '" . $_SESSION['Items'.$identifier]->DebtorNo . "',
						...
						WHERE dcs.orderno='" . $_SESSION['ExistingOrder'.$identifier] ."'";
	...
	$LineItemsSQL = "UPDATE dcdetails SET unitprice='" . $StockItem->Price . "',
													quantity='" . $StockItem->Quantity . "',
													discountpercent='" . floatval($StockItem->DiscountPercent) . "',
													completed='" . $Completed . "',
													poline='" . $StockItem->POLine . "',
													itemdue='" . FormatDateForSQL($StockItem->ItemDue) . "'
						WHERE dcdetails.orderno='" . $_SESSION['ExistingOrder'.$identifier] . "'
						AND dcdetails.internalitemno='" . $StockItem->LineNumber . "'";
	...
	if($Quotation){ //handle Quotations and Orders print after modification
		prnMsg(_('DC Number') .' ' . $_SESSION['ExistingOrder'.$identifier] . ' ' . _('has been updated'),'success');

		/*link to print the quotation */
		echo '<br /><table class="selection">
				<tr>
					<td><img src="'.$RootPath.'/css/'.$Theme.'/images/reports.png" title="' . _('Order') . '" alt=""></td>
					<td>' . ' ' . '<a href="' . $RootPath . '/PDFDCh.php?identifier='.$identifier . '&amp;DCNo=' . $_SESSION['ExistingOrder'.$identifier] . '">' .  _('Print DC')  . '</a></td>
				</tr>
				</table>';
		echo '<br /><table class="selection">
				<tr>
					<td><img src="'.$RootPath.'/css/'.$Theme.'/images/reports.png" title="' . _('Order') . '" alt="" /></td>
					<td>' . ' ' . '<a href="' . $RootPath . '/PDFDChExternal.php?identifier='.$identifier . '&amp;DCNo=' . $_SESSION['ExistingOrder'.$identifier] . '">' .  _('Print DC (External)')  . '</a></td>
				</tr>
				<tr>
					<td><img src="'.$RootPath.'/css/'.$Theme.'/images/reports.png" title="' . _('Order') . '" alt="" /></td>
					<td>' . ' ' . '<a href="' . $RootPath . '/PDFDChExternalWithoutRates.php?identifier='.$identifier . '&amp;DCNo=' . $_SESSION['ExistingOrder'.$identifier] . '">' .  _('Print DC (External) Without Rates')  . '</a></td>
				</tr>
				<tr>
					<td><img src="'.$RootPath.'/css/'.$Theme.'/images/reports.png" title="' . _('Order') . '" alt="" /></td>
					<td>' . ' ' . '<a href="' . $RootPath . '/PDFDChExternalBill.php?identifier='.$identifier . '&amp;DCNo=' . $_SESSION['ExistingOrder'.$identifier] . '">' .  _('Print DC (External) Bill')  . '</a></td>
				</tr>
				</table>';
	}else{
	...
	echo '<tr>
			...
			<td><a href="' . $RootPath .'/Selectdc.php?identifier='.$identifier   . '">' .  _('Select A Different Order')  . '</a></td>
		</tr>
		</table>';
	}//end of print orders
	include('includes/footer.inc');
...
echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />';
	...
	$sqlL="select MAX(orderlineno) as maxlineno from dcdetails where orderno = ".$_SESSION['ExistingOrder' .$identifier].""; 		
...
for ($line=0; $line<= $resultrow['maxlineno']; $line++ )
{
	$sqlB = "SELECT *
	FROM dclines 
	WHERE dclines.orderno='" . $_SESSION['ExistingOrder' .$identifier] . "'
	AND dclines.lineno='" .$line. "'
	";
					$resultB = DB_query($sqlB,$db);
			$TransferRowB = DB_fetch_array($resultB);
$sqlO="select MAX(lineoptionno) as maxlineoptionno from dcdetails where orderno = ".$_SESSION['ExistingOrder' .$identifier]."
...
for ($option=0; $option<= $resultrowO['maxlineoptionno']; $option++ )
	{
				$sqlC = "SELECT *
	FROM dcoptions 
	WHERE dcoptions.orderno='" . $_SESSION['ExistingOrder' .$identifier]. "'
...
if(1){
	...
	if ($_SESSION['Items'.$identifier]->Quotation=1){
		echo '<tr><th colspan="7">' . '<h3>Line No. '.($line+1).'</h3>' . '</th></tr>';
...
if ($_SESSION['ExistingOrder'.$identifier]==0){
	...
	echo '<br /><input type="submit" name="ProcessOrder" value="' . _('Commit Changes') . '" />';
}
