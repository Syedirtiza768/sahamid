...
if (isset($_POST['wotemplate']))
{
	$_SESSION['wotemplate'] = $_POST['wotemplate'];
}
	
	
if (isset($_GET['WO'])) {
	$_SESSION['WO'] = $_GET['WO'];
	$SelectedWO = $_SESSION['WO'];
...
if (isset($_POST['submit']) OR isset($_POST['Search'])) { //The update button has been clicked
	...
	if ($Input_Error == false) {
		...
		$sql[] = "UPDATE workorders SET requiredby='" . $SQL_ReqDate . "',
												startdate='" . FormatDateForSQL($_POST['StartDate']) . "'
											WHERE wo='" . $_POST['WO'] . "'";
		$sql[] = "UPDATE workorders SET wotemplateid='" . $_SESSION['wotemplate'] . "'
											WHERE wo='" . $_POST['WO'] . "'";
		
//delete any properties for the item no longer relevant with the change of category
				$result = DB_query("DELETE FROM woproperties
									WHERE wo ='" . $SelectedWO . "'",
									$db,$ErrMsg,$DbgMsg,true);

				//now insert any properties
				for ($i=0;$i<$_POST['PropertyCounter'];$i++){
					...
					$result = DB_query("INSERT INTO woproperties (wo,
																		wopropid,
																		value)
														VALUES ('" . $SelectedWO . "',
																'" . $_POST['PropID' . $i] . "',
																'" . $_POST['PropValue' . $i] . "')",
										$db,$ErrMsg,$DbgMsg,true);
				} //end of loop around properties defined for 
		for ($i=1;$i<=$_POST['NumberOfOutputs'];$i++){
...
echo '</select></td></tr>';
echo '<tr><td class="label">' . _('Customer') .':</td>
	<td><select name="StockLocation" onChange="ReloadForm(form1.submit)">';
$LocResult = DB_query("SELECT loccode,locationname FROM locations",$db);
/*while ($LocRow = DB_fetch_array($LocResult)){
	if ($_POST['StockLocation']==$LocRow['loccode']){
...
echo '</select></td></tr>';
echo '<tr><td class="label">' . _('Customer contact') .':</td>
	<td><select name="StockLocation" onChange="ReloadForm(form1.submit)">';
$LocResult = DB_query("SELECT loccode,locationname FROM locations",$db);
/*while ($LocRow = DB_fetch_array($LocResult)){
	if ($_POST['StockLocation']==$LocRow['loccode']){
...
<table>
<tr>
<td>BOM</td><td>Drawings</td><td>General Supporting Documents</td>
</tr>
<tr>
		<td>' .  _('Image File (.jpg)') . ':
		...
		<br /><input type="submit" name="submit" id="ClearImage" value="upload" >
 ...
 $SQL2="SELECT wotemplateid
		FROM workorders
		
		where wo = '".$SelectedWO."'";
	$result2 = DB_query($SQL2,$db);
$wotemplate = DB_fetch_array($result2);
...
echo '<a href = "Stocks.php?WO='.$SelectedWO.'" target = "_blank"> Add new item </a><br>';
echo '<a href = "StockCategories.php" target = "_blank"> Add new category </a><br>';
...
$SQL="select wotemplateid,
		...
		FROM wotemplate
		ORDER BY wotemplatedescription";
	$result1 = DB_query($SQL,$db);
 /*
 $SQL2="SELECT wotemplateid
		FROM workorders
...
$wotemplate = DB_fetch_array($result2);
*/
echo '<table class="selection"><tr><td>' . _('Select a Work Order Template') . ':<select name="wotemplate" onChange="ReloadForm(form1.updatewotemplate)">';

if (!isset($_SESSION['wotemplate'])){
	echo '<option selected="True" value="All">' . _('All') . '</option>';
	$_SESSION['wotemplate'] ='All';
} else {
	echo '<option value="All">' . _('All') . '</option>';
}
...
while ($myrow1 = DB_fetch_array($result1)) {
	...
	if ($_SESSION['wotemplate']==$myrow1['wotemplateid']){
		
			echo '<option selected="True" value=' . $myrow1['wotemplateid'] . '>' . $myrow1['wotemplatedescription'] . '</option>';
	} else {
		echo '<option value='. $myrow1['wotemplateid'] . '>' . $myrow1['wotemplatedescription'] . '</option>';
	}
...
$sql = "SELECT wotemplatepropid,
		...
		WHERE wotemplateid ='" . $_SESSION['wotemplate'] . "'
		
		ORDER BY wotemplateid";
...
echo '<br />
	...
	echo '<tr>
			<th colspan="2">' . _('Work Order Template Properties') . '</th>
		</tr>';
...
while ($PropertyRow=DB_fetch_array($PropertiesResult)){
	...
	if (isset($SelectedWO)) {
		$PropValResult = DB_query("SELECT value FROM
									woproperties
									WHERE wo='" . $SelectedWO . "'
									AND wopropid ='" . $PropertyRow['wotemplatepropid']."'",
								$db);
    ...
    echo '<tr>
            <td>';
  	        echo '<input type="hidden" name="PropID' . $PropertyCounter . '" value="' .$PropertyRow['wotemplatepropid'] .'" />';
...
echo '<input type="submit" name="updatewotemplate" style="visibility:hidden;width:1px" value="' . _('wotemplates') . '" />';
