...
if ($filters['view'] === 'suppliers') {
	...
	$supplierCount = SP_ReportRunOne($db, "SELECT COUNT(*) AS row_count FROM suppliers AS s WHERE s.suppliersince <= " . SP_ReportSqlQuote($filters['as_of']) . SP_ReportSupplierWhere($filters, 's'));
	$supplierCount = isset($supplierCount['row_count']) ? (int)$supplierCount['row_count'] : 0;
...
$filterOptions = array('currencies' => array(), 'supplier_types' => array(), 'payment_methods' => array(), 'cost_centers' => array());
...
$result = DB_query('SELECT tagref, tagdescription FROM tags ORDER BY tagdescription', $db, '', '', false, false);
if ($result) while ($row = DB_fetch_assoc($result)) $filterOptions['cost_centers'][] = $row;
...
echo '<form class="sp-filter" method="get" action="' . $baseUrl . '"><div class="sp-filter-grid">';
...
echo '</select></div>';
echo '<div class="sp-field"><label for="cost_center">' . _('Cost center / GL tag') . '</label><select id="cost_center" name="cost_center"><option value="all">' . _('All tags') . '</option>';
foreach ($filterOptions['cost_centers'] as $option) echo '<option value="' . (int)$option['tagref'] . '"' . ((string)$filters['cost_center'] === (string)$option['tagref'] ? ' selected="selected"' : '') . '>' . SP_ReportH($option['tagref'] . ' · ' . $option['tagdescription']) . '</option>';
echo '</select></div>';
...
echo '</div><div class="sp-actions"><button class="sp-button" type="submit">' . _('Apply filters') . '</button><a class="sp-reset" href="' . $baseUrl . '">' . _('Reset all') . '</a></div><p class="sp-filter-note">' . _('Aging uses the selected as-of date. Payment method/status filters apply to payment analysis; supplier, currency, category, and GL dimensions apply throughout.') . '</p></form>';

$activeChips = array();
foreach (array('supplier' => 'Supplier', 'supplier_type' => 'Category', 'currency' => 'Currency', 'invoice_status' => 'Invoice status', 'aging_bucket' => 'Aging', 'payment_status' => 'Payment status', 'payment_method' => 'Payment method', 'cost_center' => 'Cost center', 'project' => 'Project') as $key => $label) {
...
if ($filters['view'] === 'suppliers') {
	...
	foreach ($supplierRows as $row) {
		...
		$dimensions = trim($row['project_refs'] . ($row['cost_centers'] !== '' ? ' · ' . $row['cost_centers'] : ''));
