	...
	<script>
		...
		function render(data){
			(function salesPersonDetail(salesman, fromDate, toDate,salespersonTotalInvoiceValue,salespersonTotalInvoicePercentage){
				...
				<div class="row">
  				<div class="col-md-3">Salesperson Invoice Volume</div>
 				 <div class="col-md-3">${parseInt(data['salespersonTotalInvoiceValue']).toLocaleString()}</div>
				</div>
				<div class="row">
  				<div class="col-md-3">Salesperson Invoice percentage</div>
		...
		/*	if(value['invoiceValue'] === undefined || value['invoiceValue'] <= 0){
			...
			return `
				<tr>
					<td>${typeof value['quotationValue'] != 'undefined' ? parseInt(value['quotationValue']).toLocaleString(): 0}</td>
					<td>${typeof value['ocValue'] != 'undefined' ? parseInt(value['ocValue']).toLocaleString() : 0}</td>
					<td>${typeof value['dcValue'] != 'undefined' ? parseInt(value['dcValue']).toLocaleString() : 0}</td>
			...
			items.forEach(function(item){
				html += `
					<tr>
						...
						<td>${parseInt(item['materialcost']).toLocaleString()}</td>
						<td>${Math.round((item['averageInvoiceFactor'] * 100.0)) / 100.0}</td>
