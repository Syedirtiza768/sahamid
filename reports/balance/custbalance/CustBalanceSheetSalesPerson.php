<?php 
	...
	if(isset($_GET['location']) && $_GET['location'] == 'SR'){
		...
		if(!userHasPermission($db, 'CustomerBalanceSheetMTSP')) {
			header("Location: /sahamid/v2/reportLinks.php");
			exit;
		}
	}else{
	...
	<script>
		(function( $ ) {
					...
					drawCallback: function () {
				     	...
				     	$( api.table().footer() ).html(
                            ...
                            '<th>'+api.column(8,{search:'applied'}).data().sum()+'</th>'+
				     		'<th>'+api.column(9,{search:'applied'}).data().sum()+'</th></tr>'
				      	);
				    }
				});

				$('#datatable_length').find("label").html("<h3 style='margin:0; padding:0; font-variant: petite-caps;'><i class='fa fa-money' aria-hidden='true'></i> Customers Balance Sheet</h3>");
			};
			...
			$(function() {
				...
				$.ajax({
					type: 'GET',
					url: "<?php echo $RootPath; ?>"+"/CustBalanceSheetAPISalesPerson.php"+"?location=<?php echo $_GET['location']; ?>",
					dataType: "json",
