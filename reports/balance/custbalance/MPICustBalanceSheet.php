...
<head>
	...
	<title>MPI Customer Balance</title>
...
<body>
	...
	<div class="col-md-12">
		...
		<div class="btn-danger" style="text-align:center; padding:20px; margin:20px 0; ">
			...
			<h3 style='margin:0; padding:0; font-variant: petite-caps;'><i class='fa fa-money' aria-hidden='true'></i> Total UnAllocated Balance : <span id="totalBalance">Calculating Balance...</span> <span style="font-size:1.5rem">PKR</span></h3>
	...
	<script>
		(function( $ ) {
				...
				datatable = $('#datatable').DataTable({
					...
					drawCallback: function () {
				     	...
				     	$( api.table().footer() ).html(
				     		...
				     		'<th>'+api.column(7,{search:'applied'}).data().sum()+'</th><th>--</th></tr>'
				      	);
				    }
				});

				$('#datatable_length').find("label").html("<h3 style='margin:0; padding:0; font-variant: petite-caps;'><i class='fa fa-money' aria-hidden='true'></i>MPI Customers Balance Sheet</h3>");
			};
			...
			$(function() {
				...
				$.ajax({
					type: 'GET',
					url: "<?php echo $RootPath; ?>"+"/MPICustBalanceSheetAPI.php"+"?location=<?php echo $_GET['location']; ?>",
					dataType: "json",
					success: function(response) { 
						datatable.rows.add(response.res).draw(false);
						$("#totalBalance").html(response.total);
					},
					error: function(){
