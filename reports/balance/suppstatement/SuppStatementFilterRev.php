<?php 
	...
	include('../../../includes/SQL_CommonFunctions.inc');
		if(!userHasPermission($db, 'Supp_Statement_Filter')) {
	...
	if(isset($client) && trim($client) != ""){
		...
		$SQL = "SELECT * FROM suppliers 
			WHERE supplierid LIKE '%$client%'
			OR suppname LIKE '%$client%'";
	...
	<header style="font-size:1em; padding:7px; width:100%; background:#424242;">
  	...
  	<div class="col-md-12" style="margin-top: 20px">
  		...
  		<div class="col-md-4 col-md-offset-4">
	  		...
	  		<form>
	  			Market Place Business Name: 
	  			<input type="text" name="client" style="width: 100%; border:1px #424242 solid; border-radius: 4px; padding:10px" placeholder="Business Name or Business No Here...">
  		...
  		<table class="table table-bordered table-striped mb-none" id="datatable">
  			...
  			<tbody>
  				<?php if(!is_array($res)) while ($customer = mysqli_fetch_assoc($res)) { ?>
  					<tr>
  						<td class="btn-primary"><?php echo $customer['supplierid']; ?></td>
  						<td><?php echo $customer['suppname']; ?></td>
						<td>
							<form target="_blank" action="SupplierStatementRev.php" method="post">
							<input type="hidden" name="FormID" value="<?php echo $_SESSION['FormID']; ?>">
							<input type="hidden" name="cust" value="<?php echo $customer['supplierid']; ?>">
	...
	<script>
		(function( $ ) {
			...
			var datatableInit = function() {
				...
				$('#datatable_length').find("label").html("<span style='margin:0; padding:0; padding-right:20px; font-variant: petite-caps; font-size:2.4rem'><i class='fa fa-users' aria-hidden='true'></i> Vendor Reverse Allocation Filter</span> ");
			};
