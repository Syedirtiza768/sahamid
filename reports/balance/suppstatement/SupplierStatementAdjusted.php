<?php
	...
	$SQL = 'SELECT SUM(ovamount) as openingbalance
			FROM supptrans
			INNER JOIN bazar_parchi ON supptrans.transno=bazar_parchi.transno
	  		WHERE supplierno="'.$debtorno.'" 
	  		AND bazar_parchi.created_at < "'.($fromdate).'"'; //FormatDateForSQL
	
	$res = mysqli_query($db, $SQL);

	$customerStatement['openingbalance'] = mysqli_fetch_assoc($res)['openingbalance'] ?: 0;

	$SQL = 'SELECT supptrans.*, bazar_parchi.created_at FROM supptrans 
            INNER JOIN bazar_parchi ON supptrans.transno=bazar_parchi.transno
			WHERE supplierno="'.$debtorno.'"
			AND bazar_parchi.created_at >= "'.($fromdate).'"
			AND bazar_parchi.created_at <= "'.($todate).'"
			ORDER BY bazar_parchi.created_at';
                        ...
                        <tr>
                            <td style="padding-bottom: 0px">
                                <span style="font-size: 35px;">
       	 							<img src="/sahamiduat/companies/sahamiduat/EDI_Incoming_Orders/companylogos/SAH.jpg"
        ...
        <table cellpadding="0" cellspacing="0">
            ...
            <tr class="item">
        		<td class="nostretch" style="font-size: 12px; width: 1%">
					<?php echo date('d/m/Y',strtotime(explode(' ',$statement['created_at'])[0])); ?>
