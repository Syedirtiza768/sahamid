<?php 
    ...
    if(!userHasPermission($db, 'mpi_weekly_projections')){
	...
	if(isset($_GET['json'])){
        ...
        $SQL="SELECT suppliers.supplierid,suppliers.suppname,
			...
			AND bazar_parchi.created_at < '$sdate1'
			GROUP BY suppliers.supplierid 
			";
        $res=mysqli_query($db,$SQL);
        while($row=mysqli_fetch_assoc($res)){

            $carryforward[$row['supplierid']]=round($row['curr'],0);
        }
        $mpobalance=[];
        $SQL="SELECT debtortrans.debtorno, (CASE WHEN GSTwithhold = 0 AND WHT = 0 
								THEN ovamount
        ...
        while($row=mysqli_fetch_assoc($res)){
            ...
            $mpobalance[$row['debtorno']]=round($row['mpobalance'],0);

        }

        $mpicount=[];
        $SQL="SELECT svid,COUNT(*) as mpicount FROM bazar_parchi 
			WHERE bazar_parchi.inprogress=1
			GROUP BY bazar_parchi.svid 
			";
        $res=mysqli_query($db,$SQL);
        while($row=mysqli_fetch_assoc($res)){

            $mpicount[$row['svid']]=$row['mpicount'];
        }
        ...
        while ($sdate<=$edate) {
			...
			WHERE supptrans.type=601
			AND supptrans.settled=0
			AND bazar_parchi.created_at BETWEEN '$sdate1' AND '$sdate2'
			GROUP BY suppliers.supplierid 
			";
            $res=mysqli_query($db,$SQL);

            $columns[]=date('Y-W', $sdate);
            $week=date('Y-W', $sdate);
            while($row=mysqli_fetch_assoc($res)){

                $calculations[$row['supplierid']][$week]=round($row['curr'],0);
            }
        ...
        while ($row=mysqli_fetch_assoc($res))
        {
            $suppliers[$row['supplierid']]['suppname']=$row['suppname'];
            $suppliers[$row['supplierid']]['carryforward']=$carryforward[$row['supplierid']];
            $suppliers[$row['supplierid']]['mpicount']=$mpicount[$row['supplierid']];
            $suppliers[$row['supplierid']]['mpobalance']=$mpobalance[$row['supplierid']];
        }
        $response=[];
        foreach ($suppliers as $key=>$value)
        {
            $response[$key]['id']=$key;
            $response[$key]['suppname']=$suppliers[$key]['suppname'];
            if ($suppliers[$key]['mpobalance']>0)
                $response[$key]['mpobalance']=$suppliers[$key]['mpobalance'];
            else
                $response[$key]['mpobalance']=0;

            if ($suppliers[$key]['carryforward']>0)
                $response[$key]['carryforward']=$suppliers[$key]['carryforward'];
            else
                $response[$key]['carryforward']=0;
            if ($suppliers[$key]['mpicount']>0)
                $response[$key]['mpicount']='<a target="_blank" href="../shop/parchi/inward/filteredListInwardBazarParchi.php?start='.$startDate.'&end='.$endDate.'&vendor='.$key.'">'.$suppliers[$key]['mpicount'].'</a>';
            else
                $response[$key]['mpicount']=0;
            $sum=0;
            foreach ($columns as $column)
            {
                if ($calculations[$key][$column]>0)
                    $response[$key][$column]=$calculations[$key][$column];
                else
                    $response[$key][$column]=0;
                $sum+=$response[$key][$column];
            }
            $response[$key]['total']=$sum;
        }
        $responseindexed=[];
        foreach($response as $key=>$value)
        {
                $responseindexed[]=$value;
...
<body>
	...
	<h1 style="text-align: center;">MPI Weekly Projections</h1>
		...
		(function( $ ) {
			...
			var datatableInit = function() {
                ...
                $("#getTheResults").on("click", function(){
                    ...
                    $.get("mpiweeklyprojections.php?json&from_date="+from+"&to_date="+to, {dataType: "json"}, function(res, status){


                        let tablehtml = '<thead><tr style="background:#424242; color:white;">' +
                            '<th>ID</th>' +
                            '<th>Name</th>'+
                            '<th>C/F</th>'+
                            '<th>I/P</th>'
                        ...
                        let col=parsed.columns;
                        for (x in col) {
                            tablehtml+= `<th>`+col[x]+`</th>`;
                        ...
                        for(let i=0;i<parsed.data.length;i++)
                        {
                            ...
                            let y;
                            let row=eval(parsed.data[i]);
                            let z;

                            col=parsed.columns;
                            tablehtml+= `<td>`+row['id']+`</td>`;
                            tablehtml+= `<td>`+row['suppname']+`</td>`;
                            tablehtml+= `<td>`+row['carryforward']+`</td>`;
                            tablehtml+= `<td>`+row['mpicount']+`</td>`;

                            for (x in col) {
                                tablehtml+= `<td>`+row[col[x]]+`</td>`;

                            }
                            tablehtml+= `<td>`+row['total']+`</td>`;
                            tablehtml+= `<td>`+row['mpobalance']+`</td>`;
                            tablehtml+=`</td>`;
