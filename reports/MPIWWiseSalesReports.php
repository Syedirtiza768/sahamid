<?php 
	...
	if(isset($_GET['json'])){
		...
		$SQL = "SELECT  stockmaster.stockid,
						...
						/ SUM(bpitems.quantity_received) as MPIPrice
				FROM bpitems
		...
		while($row = mysqli_fetch_assoc($res)){
			...
			$SQL = "SELECT SUM(shopsalelines.quantity * shopsalesitems.quantity) as shopSaleQTY,
			...
			$shopSaleRes = mysqli_query($db, $SQL);

			$row['shopSaleQTY'] 		= 0;
			$row['shopSaleAvgPrice'] 	= 0;

			if(mysqli_num_rows($shopSaleRes) == 1){
				$shopSaleRow = mysqli_fetch_assoc($shopSaleRes);
				$row['shopSaleQTY'] 		= $shopSaleRow['shopSaleQTY'];
				$row['shopSaleAvgPrice'] 	= $shopSaleRow['shopSaleAvgPrice'];
			}

			$SQL = "SELECT dcs.GSTAdd,
					...
					AND dcdetails.stkcode = '$stockid'
					AND dcs.debtorno LIKE 'SR%'
					GROUP BY dcdetails.stkcode";
			$dcsRes = mysqli_query($db, $SQL);
			...
			if(mysqli_num_rows($dcsRes) == 1){
				$dcRow = mysqli_fetch_assoc($dcsRes);

				if($dcRow['GSTAdd'] == ""){
					$row['SAHdcQTY'] 		= $dcRow['dcQTY'];
					$row['SAHdcAvgPrice'] 	= $dcRow['dcAvgPrice'];
				}else if($dcRow['inclusive']){
					$row['dcQTY'] 		= $dcRow['dcQTY'];
					$row['dcAvgPrice'] 	= $dcRow['dcAvgPrice'] / 1.17;
				}else{
					$row['dcQTY'] 		= $dcRow['dcQTY'];
					$row['dcAvgPrice'] 	= $dcRow['dcAvgPrice'];
				}
			}
...
<body>
	...
	<h1 style="text-align: center;">MPIW Item wise Sales Details</h1>	
