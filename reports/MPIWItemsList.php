<?php 
	...
	if(isset($_GET['json'])){
		...
		$SQL = "SELECT  stockmaster.stockid, 
						...
						SUM(shopsalelines.quantity * shopsalesitems.quantity * shopsalelines.price * (1 - shopsale.discount / 100)) 
						/ SUM(shopsalelines.quantity * shopsalesitems.quantity) as shopSaleAvgPrice
				FROM shopsale
				INNER JOIN shopsalelines ON shopsalelines.orderno = shopsale.orderno
				INNER JOIN shopsalesitems ON shopsalesitems.lineno = shopsalelines.id
				...
				WHERE shopsale.orddate >= '$startDate'
				AND shopsale.orddate <= '$endDate'
				AND (shopsale.debtorno LIKE 'SR%' 
					OR shopsale.debtorno LIKE 'WALKIN01')
				GROUP BY shopsalesitems.stockid";
		$res = mysqli_query($db, $SQL);
		while($row = mysqli_fetch_assoc($res)){ $items[$row['stockid']] = $row; }
		...
		$SQL = "SELECT  stockmaster.stockid, 
						...
						manufacturers.manufacturers_name as brand,
						dcs.GSTAdd,
						SUM(dcoptions.quantity * dcdetails.quantity) as dcQTY,
						SUM(dcoptions.quantity * dcdetails.quantity * dcdetails.unitprice * (1 - dcdetails.discountpercent)) 
						/ SUM(dcoptions.quantity * dcdetails.quantity) as dcAvgPrice
				FROM dcoptions
				INNER JOIN dcs ON dcs.orderno = dcoptions.orderno
				INNER JOIN dcdetails ON (dcdetails.orderno = dcoptions.orderno
				...
				INNER JOIN manufacturers ON stockmaster.brand = manufacturers.manufacturers_id
				WHERE dcs.orddate >= '$startDate'
				AND dcs.orddate <= '$endDate'
				AND dcs.debtorno LIKE 'SR%'
				GROUP BY dcdetails.stkcode";
		$res = mysqli_query($db, $SQL);
		while($row = mysqli_fetch_assoc($res)){ 
			if(!isset($items[$row['stockid']])){
				$row['shopSaleQTY'] = 0;
				$row['shopSaleAvgPrice'] = 0;
			}

			$items[$row['stockid']] = $row;

			if($row['GSTAdd'] == ""){
				$items[$row['stockid']]['dcQTY'] = 0;
				$items[$row['stockid']]['dcAvgPrice'] = 0;

				$items[$row['stockid']]['SAHdcQTY'] = $row['dcQTY'];
				$items[$row['stockid']]['SAHdcAvgPrice'] = $row['dcAvgPrice'];
			}else if($row['inclusive']){
				$items[$row['stockid']]['dcQTY'] = $row['dcQTY'];
				$items[$row['stockid']]['dcAvgPrice'] = $row['dcAvgPrice'] / 1.17;

				$items[$row['stockid']]['SAHdcQTY'] = 0;
				$items[$row['stockid']]['SAHdcAvgPrice'] = 0;
			}else{
				$items[$row['stockid']]['dcQTY'] = $row['dcQTY'];
				$items[$row['stockid']]['dcAvgPrice'] = $row['dcAvgPrice'];
		...
		$allStockIDs = "'".implode("','", array_keys($items))."'";

		$SQL = "SELECT  bpitems.stockid,
				...
				WHERE bpitems.stockid IN ($allStockIDs)
		...
		while($row = mysqli_fetch_assoc($res)){
			...
			if(!isset($items[$row['stockid']]['count'])) {
				$items[$row['stockid']]['count'] = 0;
				$items[$row['stockid']]['MPIQTY'] = 0;
				$items[$row['stockid']]['MPIPrice'] = 0;
			}

			if($items[$row['stockid']]['count'] < 3) {
				$items[$row['stockid']]['MPIQTY'] += $row['MPIQTY'];
				$items[$row['stockid']]['MPIPrice'] += $row['MPIPrice'] * $row['MPIQTY'];
			}

			$items[$row['stockid']]['count']++;

		}

		echo json_encode(array_values(array_map(function($item){
			if($item['count'] != 0)
				$item['MPIPrice'] = $item['MPIPrice']/$item['MPIQTY'];

			if(!isset($item['MPIQTY'])){
				$item['MPIQTY'] = 0;
				$item['MPIPrice'] = 0;
			}
			...
			if(!isset($item['SAHdcAvgPrice'])){
				$item['SAHdcQTY']		= 0;
				$item['SAHdcAvgPrice'] 	= 0;
			}

			if(!isset($item['shopSaleQTY'])){
				$item['shopSaleQTY'] = 0;
			}

			if(!isset($item['shopSaleAvgPrice'])){
				$item['shopSaleAvgPrice'] = 0;
			}
...
<body>
	...
	<h1 style="text-align: center;">ITEM Sales Details</h1>	
	...
	<script>
		(function( $ ) {
			...
			$("#getTheResults").on("click", function(){
				...
				$.get("MPIWItemsList.php?json&from_date="+from+"&to_date="+to, function(res, status){
