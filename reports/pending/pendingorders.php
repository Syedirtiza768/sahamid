<?php
    ...
    if(!userHasPermission($db,"pending_orders_report")){
        header('Location: ' . $_SERVER['HTTP_REFERER']);
        return;
    }
	if(isset($_GET['json'])) {
        $SQL = "SELECT ocs.salescaseref,ocdetails.stkcode,stockmaster.mnfpno,manufacturers.manufacturers_name, 
              stockmaster.description,salescase.podate, DATEDIFF(CURRENT_DATE,salescase.podate) AS age,
              SUM(ocdetails.quantity*ocoptions.quantity) as sumQtyPO, CASE WHEN debtorsmaster.debtorno LIKE 'MT%' THEN 'Model Town'
              WHEN debtorsmaster.debtorno LIKE 'SR%' THEN 'Show Room' ELSE 'N/A' END as location, debtorsmaster.name,
              salescase.salesman FROM ocs inner join ocdetails ON ocs.orderno=ocdetails.orderno 
              INNER JOIN ocoptions ON (ocdetails.orderno=ocoptions.orderno AND ocdetails.orderlineno = ocoptions.lineno)
              INNER JOIN stockmaster
              ON stockmaster.stockid=ocdetails.stkcode INNER JOIN manufacturers 
              ON stockmaster.brand=manufacturers.manufacturers_id INNER JOIN salescase
              ON ocs.salescaseref=salescase.salescaseref INNER JOIN debtorsmaster
              ON salescase.debtorno=debtorsmaster.debtorno
              WHERE ocs.salescaseref like '%%' 
        ...
        if (isset($_POST['from']) && $_POST['from'] != "") {
            $SQL .= " AND ocs.orddate >= '" . $_POST['from'] . "'";
        }

        if (isset($_POST['to']) && $_POST['to'] != "") {
            $SQL .= " AND ocs.orddate <= '" . $_POST['to'] . "'";
        }
        $SQL .= " group by ocdetails.stkcode,ocs.salescaseref ";
        ...
        while ($row = mysqli_fetch_assoc($res)) {
            ...
            $response[$salescaseref][$item] = $row;
            $response[$salescaseref][$item]['sumQtyDC'] = 0;
            $response[$salescaseref][$item]['sumQtyQ'] = 0;
        }


        $SQL = "SELECT salesorders.salescaseref,salesorderdetails.stkcode,SUM(salesorderdetails.quantity*salesorderoptions.quantity) as sumQtyQ
        ...
        if (isset($_POST['stockid']) && $_POST['stockid'] != "") {
            $SQL .= " AND salesorderdetails.stkcode = '" . $_POST['stockid'] . "'";
        }

        if (isset($_POST['from']) && $_POST['from'] != "") {
            $SQL .= " AND salesorders.orddate >= '" . $_POST['from'] . "'";
        }

        if (isset($_POST['to']) && $_POST['to'] != "") {
            $SQL .= " AND salesorders.orddate <= '" . $_POST['to'] . "'";
        }
        $SQL .= " group by salesorderdetails.stkcode,salesorders.salescaseref ";
        $res = mysqli_query($db, $SQL);
        $response2 = [];
        while ($row = mysqli_fetch_assoc($res)) {
            $salescaseref = $row['salescaseref'];
            $item = $row['stkcode'];
            if (is_array($response[$salescaseref][$item]))
                $response[$salescaseref][$item]['sumQtyQ'] = $row['sumQtyQ'];
        }


        $SQL = "SELECT dcs.salescaseref,dcdetails.stkcode,SUM(dcdetails.quantity*dcoptions.quantity) as sumQtyDC
        ...
        if (isset($_POST['stockid']) && $_POST['stockid'] != "") {
            $SQL .= " AND dcdetails.stkcode = '" . $_POST['stockid'] . "'";
        }

        if (isset($_POST['from']) && $_POST['from'] != "") {
            $SQL .= " AND dcs.orddate >= '" . $_POST['from'] . "'";
        }

        if (isset($_POST['to']) && $_POST['to'] != "") {
            $SQL .= " AND dcs.orddate <= '" . $_POST['to'] . "'";
        }
        $SQL .= " group by dcdetails.stkcode,dcs.salescaseref ";
        $res = mysqli_query($db, $SQL);
        $response2 = [];
        while ($row = mysqli_fetch_assoc($res)) {
            $salescaseref = $row['salescaseref'];
            $item = $row['stkcode'];
            if (is_array($response[$salescaseref][$item]))
                $response[$salescaseref][$item]['sumQtyDC'] = $row['sumQtyDC'];
        }
        $output = [];
        ...
        foreach($responseout as $key=>$resp) {
            if ($resp['sumQtyPO']<=$resp['sumQtyDC'])
                continue;
            $output[] = [
                0 => "<a target='_blank' href='../../salescase/salescaseview.php?salescaseref=".$resp['salescaseref']."'>".$resp['salescaseref']."</a>",
                1 => $resp['stkcode'],
                2 => $resp['mnfpno'],
                3 => $resp['manufacturers_name'],
                4 => $resp['description'],
                5 => $resp['podate'],
                6 => $resp['age'],
                7 => $resp['sumQtyQ'],
		...
		(function( $ ) {
			...
			var datatableInit = function() {
				...
				datatable = $('#datatable').DataTable({
                    ...
                    <?php
                    if(!userHasPermission($db,"director_special")){
                        echo '
