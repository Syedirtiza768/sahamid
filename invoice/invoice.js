	...
	$(document).ready(function(){
		...
		$.ajax({
			type: 'POST',
		    url: rootpath+"/invoice/api/retrieveInvoice.php",
		    data: {orderno: order},
		    dataType: "json",
		    success: function(response) { 
		    	...
		    	if(status == "success"){
	    			...
	    			$('#eorderno').html(data.invoiceno);
	    			$('#salescasereff').html(data.salescaseref);

	    			$('#salesref').val(data.salescaseref);
		    		...
		    		$("#order-details").find(".shopinvoiceno").val(data.shopinvoiceno);
		    		...
		    		$("#order-details").find(".podate").val(data.podate);
		    		$("#order-details").find(".invoicesdate").val(data.invoicesdate);
		    		$("#order-details").find(".due").val(data.due);
		    		$("#order-details").find(".expected").val(data.expected);
		    		...
		    		for (var index in lines) {
					    ...
					    var lineid = lines[index].invoicelineindex;
					    ...
					    for(var optionIndex in options){
						    ...
						    var lineno 	  = lineid;
						    var optionno  = options[optionIndex].invoiceoptionindex;
						    ...
						    for(var itemIndex in items){
							    ...
							    var indx = items[itemIndex].invoicedetailsindex;
							    ...
							    var sprice = items[itemIndex].unitprice;
		...
		for(index in dirtyEditors){
			...
			if(dirtyEditors[index].length > 0){
				...
				if(dEditor.editor.content.isDirty()){
					...
					$.ajax({
						type: 'POST',
					    url: rootpath+"/invoice/api/saveDescription.php",
	...
	function addline(){
		...
		$.ajax({
			type: 'POST',
		    url: rootpath+"/oc/api/createNewLine.php",
	...
	function addlineCallback(line, requirements){
		...
		descHTML += '<h3>Line No. '+vline+' Sales Tax Invoice Description</h3>';
		descHTML += '<textarea  style="width: 100%; height: 100%;" id = "l'+line+'o---desc" cols="90" rows="10"></textarea>';
	...
	function addoptionCallback(line,option, description,stockstatus, quantity){
		...
		descHTML += '<h4 style="color:blue">Option No. '+voption+' Commercial Invoice Description </h4>';
    ...
    function additemCallback(indx,line,option,stkc,bname,description,quantity,sprice,uprice,tot,qoh,dis,update,modal,part){
        ...
        html+='</div>';
        html+='<div class="col-md-4">';
        html+='<input class="quantity" type="number" value="'+quantity+'" disabled>';
        html+='</div>';
        html+='<div class="col-md-2">';
        html+='Total:';
        html+='</div>';
	...
	function removeline(line, vline){
		...
		function(){
		  ...
		  $.ajax({
		  	type: 'POST',
		    url: rootpath+"/invoice/api/deleteLine.php",
	...
	function updateQuantity(item,quantity){
		...
		$.ajax({
			type: 'POST',
		    url: rootpath+"/invoice/api/updateItem.php",
	...
	function insertitem(itemindex, brand){
		...
		$.ajax({
			type: 'POST',
		    url: rootpath+"/oc/api/addNewItem.php",
	...
	$(document.body).on('change','.order-detailss',function(){
	    ...
	    $.ajax({
			type: 'POST',
		    url: rootpath+"/invoice/api/updateOrderDetails.php",
	...
	$(document.body).on('change','.descinfo',function(){
	    ...
	    $.ajax({
			type: 'POST',
		    url: rootpath+"/invoice/api/updateOption.php",
	...
	$("#erbtn").click(function(e){
		...
		$.ajax({
			type: 'GET',
		    url: rootpath+"/previewOC.php?OrderConfirmationNo="+order+"&salescaseref="+salesref,
		    dataType: "json",
		    success: function(response) { 

	    		var ihtml = "<iframe ";
				ihtml += 'id="previewframe" ';
				ihtml += 'src = "../ViewerJS/#../tempPDF/oc/'+order+'int.pdf" ';
	...
	$("#checkforwarnings").click(function(e){
		...
		$.ajax({
			type: 'POST',
		    url: rootpath+"/invoice/api/saveInvoice.php",
		    ...
		    success: function(r) { 
		    	...
		    	if(r.status == "success")
		    		window.location = ""+rootpath+"/invoice/newinvoice.php";
		    	else
