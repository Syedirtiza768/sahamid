...
if (!isset($_POST['IssueItem'])){ //no item selected to issue yet
	...
	echo '<tr>
			<th colspan="5">' . _('Items issued or this Work Order') . '</th>
		</tr>';
	echo '<tr>
			<th colspan="2">' . _('Item') . '</th>
			<th>' . _('Qty Issued') . '</th>
		</tr>';
	$RequirmentsResult = DB_query("SELECT stockmoves.stockid,
				stockmaster.description,
				stockmaster.decimalplaces,
				trandate,
				qty,
	...
	while ($RequirementsRow = DB_fetch_array($RequirmentsResult)){
		...
		echo '<td class="number">' . locale_number_format($IssuedAlreadyRow[0],$RequirementsRow['decimalplaces']) . '</td>
		</tr>';
	}
