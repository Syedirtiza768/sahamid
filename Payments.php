	...
	/*				$SQL = "INSERT INTO gltrans (type,
	  ...
	  /*Its a supplier payment type 22 */
			...
			$SQL = $SQL . "values ('" . $TransNo . "',
					...
					'".$_SESSION['chequefilepath']."',
					'".$_SESSION['chequedepositfilepath']."',
					'".$_SESSION['cashfilepath']."',
...
echo '<form action="' . htmlspecialchars($_SERVER['PHP_SELF'] . '?identifier=' . $identifier) . '" method="post" enctype="multipart/form-data">';
echo '<div>';
echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />
	<br />
...
echo '<tr>
		...
		<td><input type="text" name="ChequeNum"  size="10" value="' . $_POST['ChequeNum'] . '" /> ' . _('(if using pre-printed stationery)') . '</td>
	</tr>';
