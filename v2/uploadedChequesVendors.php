<?php 
    ...
    if(!isset($_GET['json']))
    {
        $SQL="INSERT INTO leads(firstname) VALUES ('')";
        mysqli_query($db,$SQL);
    }
	if(isset($_GET['json'])){
	    $SQL = "SELECT ROUND(ABS(supptrans.ovamount)) as amount,
				ROUND(ABS(supptrans.alloc)) as alloc,
				ROUND(ABS(supptrans.ovamount - supptrans.alloc)) as remaining,
				supptrans.chequefilepath as cheque,
				supptrans.chequedepositfilepath as deposit,
				supptrans.cashfilepath as cash,
				supptrans.id,
				...
				INNER JOIN banktrans ON (supptrans.transno=banktrans.transno AND banktrans.type=supptrans.type)
...
<script>
	...
	$(document).ready(function(){
		...
		$.get("uploadedChequesVendors.php?json", function(res, status){
    ...
    $('#datatable').on('click','.uploadChequeFile',function(){
        ...
        $.ajax({
            url: 'api/uploadChequeFileVendors.php',
            type: 'post',
            data: fd,
            contentType: false,
            processData: false,
    ...
    $('#datatable').on('click','.uploadCdrFile',function(){
        ...
        $.ajax({
            url: 'api/uploadCdrFileVendors.php',
            type: 'post',
            data: fd,
            contentType: false,
            processData: false,
    ...
    $('#datatable').on('click','.uploadCRVFile',function(){
        ...
        $.ajax({
            url: 'api/uploadCRVFileVendors.php',
            type: 'post',
            data: fd,
            contentType: false,
            processData: false,
