...
if (mysqli_num_rows($res) == 1) {
	...
	for ($month = -5; $month <= 0; $month++) {
		...
		$SQL = "SELECT transid_allocfrom as f,transid_allocto as t,datealloc as d,SUM(amt) as amt, dt.trandate as cd, 
			...
			AND debtorsmaster.debtorno LIKE 'MT%'
...
<div class="col-md-8 item" style="height:250px;overflow:auto; width:70%" data-code="companyDetailsMT">
	<link rel="stylesheet" href="path/to/font-awesome/css/font-awesome.min.css">
	<div style="position: relative; padding: 5px; background: white; color: black; cursor: pointer; z-index: 15; width:100%;">
		<?php
		if ($permission == "*") {
		?>
			<select class="js-example-basic-multiple CDMTdata" name="states[]" multiple="multiple" style="width:95%;">
				<?php
		...
		<?php } ?>
		<span class="store-data" onclick="myCDMTFunction()"><i style="color:red;" class="fa fa-search" aria-hidden="true"></i></span>
		<i class="fa fa-trash removeWidget"></i>
	</div>
	<div id="detailscomparisonchartmt" class="item-content" style="width:100%; height:250px; background:white; display:flex; align-items:center; justify-content:center; cursor:pointer" data-loaded="false">
	</div>
	...
	<script>
		$(document).ready(function() {
			...
			Highcharts.chart('detailscomparisonchartmt', {

				title: {
					text: 'Company Analysis MT'
				},
	...
	<script>
		function myCDMTFunction() {
			var data = $(".CDMTdata").val();
			...
			$.ajax({
				type: "POST",
				url: "dashboard/widgets/updated_widgets/compDetailUpdateMT.php",
				data: {
					salesman: salesman
				},
