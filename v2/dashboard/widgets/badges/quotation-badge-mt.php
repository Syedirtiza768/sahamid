...
<div class="col-md-4 col-sm-6 col-xs-12 item" style="height:140px; overflow:auto; margin-top:8px" data-code="quotation-badge-mt">
	<div style="position: relative; padding: 5px; background: white; color: black; cursor: pointer; z-index: 15; width:100%">
	<?php
		if ($permission == "*") {
		?>
			<select class="js-example-basic-multiple dataQuotationMT" name="states[]" multiple="multiple" style="width:90%;">
				<?php
		...
		<?php } ?>
		<span class="store-data" onclick="searchQuoteMT()"><i style="color:red;" class="fa fa-search" aria-hidden="true"></i></span>
  	...
  	<div class="info-box item-content">
    	...
    	<div class="info-box-content">
      		<span class="info-box-text">Quotations MT</span>
      		<span class="info-box-number" id="quotationMT"><?php echo $quotationCount; ?></span>
    	</div>
  	</div>
	  ...
	  <script>
		function searchQuoteMT() {
			var data = $(".dataQuotationMT").val();
			...
			$.ajax({
				type: "POST",
				url: "dashboard/widgets/badges/badge_updated/quotationUpdateMT.php",
				data: {
					salesman: salesman
				},
				success: function(data) {
					$(".ms-usereditor span[class^='ms-error']:contains('External Data')").hide()
					$("#quotationMT").text(data);
				}
			});
		};
	</script>
