...
//	echo $_SESSION['UsersRealName'];
	...
	if(isset($_GET['json'])){
		...
		$SQL = "SELECT `id`, `transno`, `client`, `chequeDate`,
                ...
                FROM `reversedallocationhistory`";
...
<script>
	...
	var datatableInit = function() {
                    ...
                    "render": function ( data, type, row ) {
                     ...
                     //   html='<div class="overlay"  onClick=window.open("lastcomment.php/?revid='.data.'","Ratting","width=550,height=170,0,status=0,scrollbars=1")>...</div>';
                      html=  '<a href="javascript:void(0);"NAME="Revision History Comments"  title=" Revision History Comments "' +
                          'onClick=window.open("lastcomment.php/?revid='+row['id']+'","Ratting","width=550,height=170,0,status=0,scrollbars=1");>View Comments</a>';
                        return html;
                    },
                    "targets": [11]
                }
		...
		$('#datatable_length')
			.find("label")
			.html("<h3 style='margin:0; padding:0; font-variant: petite-caps;'>Reversed Cheques</h3>");
	...
	$(document).ready(function(){
		...
		$.get("reversedCheques.php?json", function(res, status){
    ...
    $('#datatable').on('change','.comment',function(){
        ...
        let value = $(this).val();
        $.post("api/addRevisionHistoryComments.php",{revid,value},function(data, status){
