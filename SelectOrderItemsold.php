...
/*CustomerID defined in header.inc */
		...
		$_SESSION['Items'.$identifier]->GSTadd = $myrow['GSTadd'];
		$_SESSION['Items'.$identifier]->services = $myrow['services'];
		$_SESSION['Items'.$identifier]->WHT = $myrow['WHT'];
...
#Always do the stuff below if not looking for a customerid
	echo '<form name="form" onsubmit="return saveScrollPositions(this);" action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '?identifier='.$identifier . '" id="SelectParts" method="post">';
    echo '<div>';
	echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />';
echo'<input type="hidden" name="scrollx" id="scrollx" value="0" />';
echo'<input type="hidden" name="height" id="height" value="0" />';
...
<h3>Line No. '.($line+1).' Client Requirements </h3><textarea  name = "clientreq'.$line.'" cols="90" rows="10">
...
for ($option=0; $option<= $optionscount; $option++ )
	{
		...
		<tr><td>
		<h3>Line No. '.($line+1)." Option No. ".($option+1).'</h3>
		</td><tr></table>';
		echo '<br>';
		echo'	<table width="90%" cellpadding="2">';
//print_r($_SESSION['Items'.$identifier]);
...
function myFunctionunitrate() {
 ...
 document.getElementById("LineTotal_"+x).value= Math.round(document.getElementById("UnitRate_"+x).value * document.getElementById("Quantity_"+x).value*100)/100;
		...
		$DisplayTotal = locale_number_format($_SESSION['Items'.$identifier]->total,$_SESSION['Items'.$identifier]->CurrDecimalPlaces);
			...
			echo'</table>';
			echo'<input type="submit" value="Add New Item" name="addnewitem'.$count.'">';
...
if (isset($_POST[$nm]))
{
	$_SESSION[$nm]=$_POST[$nm];
	 ...
	 if ((!isset($_POST['QuickEntry'])
			AND !isset($_POST['SelectAsset']) AND isset($_SESSION[$nm]))  
...
<input type="hidden" name="lne" value='.$line.'>
...
// Find the quantity in stock at location
			...
			echo '<td style="text-align:center" colspan="8"><input type="hidden" name="SelectingOrderItems" value="1" /><input tabindex="'.strval($j+8).'" type="submit" value="'._('Add to Option').'" /></td></tr>';
			echo '</table>';
...
//	'',$_SESSION['Items'.$identifier]->externallines[$line]->lineoptions[$option]->optiontext);
	...
	echo'<table width="100%" cellpadding="2">
		...
		<tr>
		<td colspan="8"><b>S A H Description</b><textarea cols="90" rows="10" name = "SAHdesc'.$line.'_'.$option.'"
		value="'.$_SESSION['Items'.$identifier]->externallines[$line]->lineoptions[$option]->optiontext.'"
		>'.html_entity_decode($_SESSION['Items'.$identifier]->externallines[$line]->lineoptions[$option]->optiontext).'
		</textarea></td>
	...
	value="'.$_SESSION['Items'.$identifier]->externallines[$line]->lineoptions[$option]->lineoptionindex.'" >';
		...
		echo'		<br />
				<div class="centre" >
					...
					<input type="submit" name="Recalculate" value="' . _('Re-Calculate') . '" onclick"loadScroll()" onclick="saveScroll()" />';
	
	$varoption= "newoption".$line;
	$varoptionindex= "optionindex".$line;
			echo'	
					<input type="submit" name="'.$varoption.'" value="' . _('Add New Option') . '" />
					<input type="hidden" name="'.$varoptionindex.'" value="' . $line. '" />';
	echo "<hr>";
...
var height = document.body.offsetHeight ? document.body.offsetHeight : document.height
//window.scrollBy(0,height+<?php echo "$height" ?>);
</script>
<?php
include('includes/footer.inc');
?>
