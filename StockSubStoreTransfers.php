...
if (isset($_POST['CheckCode'])) {
	...
	while ($myrow = DB_fetch_array($result)) {
		echo '<tr>
				...
				<td><a href="' . $RootPath . '/StockSubStoreTransfers.php?StockID='.$myrow['stockid'].'&amp;Description='.$myrow['description'].'&amp;NewTransfer=Yes&amp;Quantity='. filter_number_format($_POST['Quantity']).'&amp;From='.$_POST['StockLocationFrom'].'&amp;To='.$_POST['StockLocationTo'].'">'
				._('Transfer') . '</a></td>
			</tr>';
...
if ($NewTransfer){
	...
	if (DB_num_rows($result) == 0){
		...
		if ($myrow['mbflag']=='D' OR $myrow['mbflag']=='A' OR $myrow['mbflag']=='K'){
			...
			echo '.<hr />';
			echo '<a href="' . $RootPath . '/StockSubStoreTransfers.php?NewTransfer=Yes">' . _('Enter another Transfer') . '</a>';
			unset ($_SESSION['Transfer']);
			include ('includes/footer.inc');
			exit;
		}
...
$SQL = "select defaultlocation from www_users where www_users.realname = '".$_SESSION['UsersRealName']."'"; 
