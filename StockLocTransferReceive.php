...
/*Ok Time To Post transactions to Inventory Transfers, and Update Posted variable & received Qty's  to stockrequestitems */
	...
	foreach ($_SESSION['Transfer']->TransferItem AS $TrfLine) {
		...
		echo $TotalQuantity += $TrfLine->Quantity;
		$i++;
...
//exit;
	...
	/*All inputs must be sensible so make the stock movement records and update the locations stocks */
		...
		/*Insert the StockSerialMovements and update the StockSerialItems  for controlled items*/
				...
				$SQL = "SELECT defaultsubstore from locations where loccode = 
				'" . $_SESSION['Transfer']->StockLocationFrom . "'";
				...
				if ($TrfLine->CancelBalance==1){
					$sql = "UPDATE stockrequestitems SET qtyreceived = qtyreceived + '". round($TrfLine->Quantity, $TrfLine->DecimalPlaces) . "',
						qtydelivered = qtyreceived + '". round($TrfLine->Quantity, $TrfLine->DecimalPlaces) . "
			...
			if ($TrfLine->CancelBalance==1){
				$sql = "UPDATE stockrequestitems SET qtydelivered = qtyreceived
						WHERE dispatchid = '". $_SESSION['Transfer']->TrfID . "'
						AND stockid = '".  $TrfLine->StockID."'";
...
if (isset($_SESSION['Transfer'])){
	...
	$sql = "SELECT DISTINCT stockrequest.dispatchid,
				...
				shipdate,qtydelivered,qtyreceived
			FROM stockrequest INNER JOIN locations
				ON stockrequest.shiploc=locations.loccode
