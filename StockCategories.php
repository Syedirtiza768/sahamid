...
if (isset($_POST['submit'])) {
	...
	} elseif (mb_strlen($_POST['CategoryDescription']) >100 or mb_strlen($_POST['CategoryDescription'])==0) {
		$InputError = 1;
		prnMsg(_('The Sales category description must be hundred characters or less long and cannot be zero'),'error');
	...
	if (isset($SelectedCategory) AND $InputError !=1) {
		...
		echo		$sql = "INSERT INTO stockcatproperties (categoryid,
														label,
														controltype,
														defaultvalue,
														minimumvalue,
	...
	echo	$sql = "INSERT INTO stockcategory (categoryid,
											stocktype,
											categorydescription,
											defaulttaxcatid,
											stockact,
...
echo '<tr><td><label for="CategoryDescription">' . _('Category Description') .
	':</label></td><td><input id="CategoryDescription" maxlength="100" name="CategoryDescription" required="required" size="22" title="' .
	_('A description of the inventory category is required') .
	'" type="text" value="' . $_POST['CategoryDescription'] .
	'" /></td></tr>';

// Stock Type input.
echo '<tr style = "position:fixed; left:-999px"><td><label for="StockType">' . _('Stock Type') .
...
echo '<tr style = "position:fixed; left:-999px"><td><label for="DefaultTaxCatID">' . _('Default Tax Category') .
...
// WIP GL Code input.
echo '<tr style = "position:fixed; left:-999px"><td>' . _('WIP GL Code') . ':</td><td><select name="WIPAct">';
...
echo '</select></td>
		...
		<tr style = "position:fixed; left:-999px">
			<td>';
if (isset($_POST['StockType']) and $_POST['StockType']=='L') {
