...
echo		'<tr>
			<td style="font-weight:bold; font-size:18px;">' .  _('Order Confirmation') .':</td>
			
		';
		$sql = 'SELECT * from salesorders where salescaseref="'.$salescaseref.'" AND quotation = 0';
		$result = DB_query($sql,$db);
...
echo		'<tr>
		 ...
		 while ($row_result=DB_fetch_array($result))
			{
				echo '</tr><td><a href="'.$RootPath.'/SelectOrderItems.php?ModifyOrderNumber=' . $row_result['orderno'] . '&salescaseref=' . $salescaseref . '">'. $row_result['orderno'].' </a></td>';
