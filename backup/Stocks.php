...
if (isset($_GET['StockID'])){
	...
	//$StockID =trim(mb_strtoupper($_GET['StockID'])).$_POST['mnf'].$_POST['mnfpno'];
$StockID =$_POST['Brand'].trim(mb_strtoupper($_POST['Condition'])).trim(mb_strtoupper($_POST['CategoryID'])).$_POST['mnfpno'];
		...
		if ($New==0) { /*so its an existing one */
			...
			*/
			...
			if (DB_num_rows($result)==1){
				...
				DB_query($sql,$db);
				$sql = "INSERT INTO increment (item) values ('" . $_POST['Description'] . "')";
				DB_query($sql,$db);
				$sql = "
				
				
				
				INSERT INTO stockmaster (stockid,
												description,
												longdescription,
												categoryid,
												conditionID,
...
echo '<tr>
		<td>' . _('Condition') . ':</td>
		<td><select name="Condition" onchange="ReloadForm(ItemForm.UpdateCategories)">';
...
while ($myrow=DB_fetch_array($result)){
	if (!isset($_POST['Condition']) OR  $myrow['conditionID']==$_POST['Condition']){
...
echo '</select>';
		...
		echo '<tr>
				...
				<td><input type="hidden" ' . (in_array('StockID',$Errors) ?  'class="inputerror"' : '' ) .'" data-type="no-illegal-chars" autofocus="autofocus"  value="'.$StockID.'" name="StockID" size="20" maxlength="20"  title ="'._('Input the stock code, the following characters are prohibited:') . ' \' &quot; + . &amp; \\ &gt; &lt;" placeholder="'._('alpha-numeric only').'" /></td>
			</tr>';
	} else {
		echo '<tr>
				<td>' .  _('Item Code'). ':</td>
...
echo '<tr>
		...
		<td><input type="text" name="mnf" size="12" maxlength="10" value="' . $_POST['mnf']. '" /></td></tr>';
echo '<tr>
		<td>' . _('Manufacturer Part Number') . ':</td>
		<td><input type="text" name="mnfpno" size="12" maxlength="10" value="' . $_POST['mnfpno']. '" /></td></tr>';
