...
<head>
	...
	<meta charset="UTF-8">
	<title>SAHamid ERP - Update Customer Payment Terms</title>
			...
			<table id="searchresults" width="100%" class="responsive">
				<thead>
					<tr style="background:#424242; color:white">
						...
						<th class="fit">Sales Volume</th>
						<th class="fit">Credit Limit</th>
						<th class="fit">Days Due</th>
						<th class="fit">Expected Days</th>
						<th class="fit">Enable</th>
                        <th class="fit">Shop DC</th>
...
$("#bss").click(function(e){
			...
			$.ajax({
				type: 'GET',
			    url: "api/updateCustomerPaymentTerms.php",
...
*/		
 		...
 		$('#searchresults').on('change','.dueDays',function(){
 		 	let debtorno = $(this).attr('data-debtorno');
 		 	let value = $(this).val(); 
 		 	$.post("api/updateCustomerPaymentTermsUpdate.php",{debtorno,value,name:'dueDays'},function(data, status){
		        console.log("Data: " + data + "\nStatus: " + status);
	    	});
 		});
 		$('#searchresults').on('change','.expectedDays',function(){
 		 	let debtorno = $(this).attr('data-debtorno');
 		 	let value = $(this).val(); 
 		 	$.post("api/updateCustomerPaymentTermsUpdate.php",{debtorno,value,name:'paymentExpected'},function(data, status){
		        console.log("Data: " + data + "\nStatus: " + status);
	    	});
 		});
 		$('#searchresults').on('change','.disabletrans',function(){
 		 	let debtorno = $(this).attr('data-debtorno');
 		 	let value = ($(this).is(":checked")) ? 1 : 0;
 		 	$.post("api/updateCustomerPaymentTermsUpdate.php",{debtorno,value,name:'disabletrans'},function(data, status){
		        console.log("Data: " + data + "\nStatus: " + status);
	    	});
 		});
 		$('#searchresults').on('change','.creditlimit',function(){
 		 	let debtorno = $(this).attr('data-debtorno');
 		 	let value = $(this).val();
 		 	$.post("api/updateCustomerPaymentTermsUpdate.php",{debtorno,value,name:'creditlimit'},function(data, status){
		        console.log("Data: " + data + "\nStatus: " + status);
	    	});
 		});
 		$('#searchresults').on('change','.holdreason',function(){
 		 	let debtorno = $(this).attr('data-debtorno');
 		 	let value = $(this).val(); 
 		 	$.post("api/updateCustomerPaymentTermsUpdate.php",{debtorno,value,name:'holdreason'},function(data, status){
		        console.log("Data: " + data + "\nStatus: " + status);
	    	});
 		});
        $('#searchresults').on('change','.disableshopdc',function(){
            let debtorno = $(this).attr('data-debtorno');
            let value = ($(this).is(":checked")) ? 1 : 0;

            $.post("api/updateCustomerPaymentTermsUpdate.php",{debtorno,value,name:'disableshopdc'},function(data, status){
                console.log("Data: " + data + "\nStatus: " + status);
            });
        });
