...
if(!userHasPermission($db, 'cart_report_permissions')) {
...
if(isset($_GET['json'])){
    ...
    $SQL = "SELECT can_access FROM cart_report_access WHERE user = '$username'";
    ...
    while($row = mysqli_fetch_assoc($res)){
        ...
        if(in_array($row['salesperson'],$assignedClients))
            $row['checked']="checked";
        $response[] = $row;
    }
                    ...
                    <div id="resultscontainer" class="" style=" background-color:#ecedf0;float: left;">
                        ...
                        <table id="searchresults" width="100%" class="responsive table-striped">
                            ...
                            <tr style="background:#efefef; color:black">
                                ...
                                <th class="fit">Total Cart Status</th>
...
?>
    <script>
...

                {
                    ...
                    "render": function ( data, type, row ) {
                        ...
                        let background="";
                            ...
                            html+='<input class="check '+data+'" type="checkbox"'+ data+' data-user="" data-canaccess="'+row['salesperson']+'"/>';
        ...
        $.get("cartreportpermissions.php?json", function(res, status){
            res = JSON.parse(res);
            table.clear().draw();
        ...
        $('#searchresults').on('change','.check',function(){
            ...
            let value = ($(this).is(":checked")) ? 1 : 0;
            $.post("api/updateCartReportPermissions.php",{can_access,value,user},function(data, status){
        ...
        $(document.body).on("change", ".comment", function(){
            ...
            let value = ref.val();
            let update = true;
            $.post("cartreportpermissions.php",{column,value,invoice,update}, function(res, status){
                if(res == "success"){
...
*/
        ...
        $(".users").on("click", function(){
            ...
            $.get("cartreportpermissions.php?json&userID="+userID, function(res, status){
                res = JSON.parse(res);
                table.clear().draw();
                table.rows.add(res).draw();
