...
if (isset($_POST['Update'])) {
	...
	if ($InputError==0) {
		$_SESSION['Request']->Location=$_POST['UserStockLocation'];
	...
	if ($InputError==0) {
		...
		$HeaderSQL="INSERT INTO igp (dispatchid,
										...
										VALUES(
											...
											'" . $_SESSION['Request']->source . "',
											'" . $_SESSION['Request']->reference . "',
...
/*
	...
	$SQL = "UPDATE substorestock
					SET quantity = quantity + '" . round($LineItems->Quantity, $LineItems->DecimalPlaces) . "'
					WHERE stockid='" . $LineItems->StockID . "'
					AND substorecode='" . $_POST['substore'] . "'";
...
echo '<table class="selection">';
echo '<tr>
		<th colspan="2"><h4>' . _('Document Details') . '</h4></th>
	</tr>
	<tr>
	...
	if (isset($_POST['Keywords']) AND mb_strlen($_POST['Keywords'])>0) {
		...
		if ($_POST['StockCat']=='All'){
			$SQL = "SELECT stockmaster.stockid,
							...
							stockmaster.mnfCode,
							manufacturerss.manufacturers_name
					FROM stockmaster,
						itemcondition,
					...
					WHERE stockmaster.conditionID=itemcondition.conditionID
						AND stockmaster.brand = manufacturerss.manufacturers_id
