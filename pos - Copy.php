...
if (isset($_POST['Update'])) {
	...
	if ($InputError==0) {
		...
		$_SESSION['Request']->salesperson=$_POST['SalesPerson'];
	}
}
...
if (isset($_POST['Edit'])) {
	...
	$_SESSION['Request']->LineItems[$_POST['LineNumber']]->Comments=$_POST['rate'];
...
foreach ($_POST as $key => $value) {
	if (mb_strstr($key,'StockID')) {
		...
		if (filter_number_format($_POST['Quantity'.$Index])>0) {
			...
			$NewModelNumber_array[$StockID]=$_POST['modelnumber'.$Index];
			$NewItem_array[$StockID] = filter_number_format($_POST['Quantity'.$Index]);
			$NewComment_array[$StockID] =$_POST['Comments'.$Index];
			$NewRate_array[$StockID] = filter_number_format($_POST['rate'.$Index]);
			$NewBrand_array[$StockID] =$_POST['brand'.$Index];
			$_POST['Units'.$StockID]=$_POST['Units'.$Index];
			$DecimalPlaces=$_POST['DecimalPlaces'.$Index];
			$_SESSION['Request']->AddLine($StockID, $ItemDescription,$NewModelNumber_array[$StockID], $NewItem_array[$StockID],$NewComment_array[$StockID],$NewRate_array[$StockID], $NewBrand_array[$StockID],$_POST['Units'.$StockID],$DecimalPlaces);
...
/*
		$EmailSQL="SELECT email
					FROM www_users, SalesPersons
					WHERE SalesPersons.authoriser = www_users.userid
						AND SalesPersons.SalesPersonid = '" . $_SESSION['Request']->SalesPerson ."'";
...
*/
	...
	echo '<p><a href="'.$RootPath.'/PDFDeliveryChallan.php?RequestNo=' . $RequestNo .  '">' .  _('Print the Delivery Challan'). '</a></p>';
	
echo '<br /><div class="centre"><a href="'. htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8') . '?New=Yes">' . _('Create another document') . '</a></div>';
...
if (isset($_GET['Edit'])) {
	...
	echo '<tr>
		...
		<tr>
			<td>' . _('Brand') . '</td>
			<td>' . $_SESSION['Request']->LineItems[$_GET['Edit']]->brand . '</td>
		</tr>
		<tr>
			<td>' . _('Model Number') . '</td>
			<td>' . $_SESSION['Request']->LineItems[$_GET['Edit']]->modelnumber . '</td>
		</tr>
		...
		<tr>
			<td>' . _('Rate') . '</td>
			<td><input type="text" class="number" name="rate" value="' . locale_number_format($_SESSION['Request']->LineItems[$_GET['Edit']]->rate, $_SESSION['Request']->LineItems[$_GET['Edit']]->DecimalPlaces) . '" /></td>
...
echo '<table class="selection">';
echo '<tr>
		<th colspan="2"><h4>' . _('Delivery Document Details') . '</h4></th>
	</tr>
	<tr><td>Issue to</td>
		';
...
while ($myrow=DB_fetch_array($result)){
	if (isset($_POST['SalesPerson']) AND $_POST['SalesPerson']==$myrow['realname']){
...
echo '<td><select name="substore">
		<option value="">' . _('Select a Substore') . '</option>';
while ($myrow=DB_fetch_array($result)){
	if (isset($_POST['substore']) AND $_POST['substore']==$myrow['substoreid']){
...
echo '</select></td>
	...
	<tr style = "position:absolute; left:-999px;">
...
foreach ($_SESSION['Request']->LineItems as $LineItems) {
	...
	echo '<td>' . $LineItems->LineNumber . '</td>
			...
			<td class="number">' . locale_number_format($LineItems->rate, $LineItems->DecimalPlaces) . '</td>
...
$SQL = "SELECT itemcondition.conditionID,
				itemcondition.abbreviation
			FROM itemcondition, internalstockcatrole
			WHERE itemcondition.conditionID = internalstockcatrole.conditionID
				AND internalstockcatrole.secroleid= " . $_SESSION['AccessLevel'] . "
			ORDER BY itemcondition.abbreviation";
...
echo '<table class="selection">
	<tr >
		<td style = "position:absolute; left:-999px;">' . _('In Stock Category') . ':<select name="StockCat">';
...
while ($myrow1 = DB_fetch_array($result1)) {
	if ($myrow1['conditionID'] == $_POST['StockCat']) {
		echo '<option selected="True" value="' . $myrow1['conditionID'] . '">' . $myrow1['abbreviation'] . '</option>';
	} else {
		echo '<option value="' . $myrow1['conditionID'] . '">' . $myrow1['abbreviation'] . '</option>';
	}
}
echo '</select></td>
	...
	if (isset($_POST['Keywords']) AND mb_strlen($_POST['Keywords'])>0) {
		...
		if ($_POST['StockCat']=='All'){
			$SQL = "SELECT stockmaster.stockid,
					...
					WHERE stockmaster.conditionID=itemcondition.conditionID
						AND stockmaster.brand = manufacturers.manufacturers_id
						AND itemcondition.conditionID = internalstockcatrole.conditionID
						AND internalstockcatrole.secroleid= " . $_SESSION['AccessLevel'] . "
			...
			$SQL = "SELECT stockmaster.stockid,
					...
					WHERE stockmaster.conditionID=itemcondition.conditionID
						AND stockmaster.brand = manufacturers.manufacturers_id	AND internalstockcatrole.secroleid= " . $_SESSION['AccessLevel'] . "
						AND stockmaster.mbflag <>'G'
						AND stockmaster.discontinued=0
						AND stockmaster.description " . LIKE . " '" . $SearchString . "'
...
if (isset($SearchResult)) {
	...
	while ($myrow=DB_fetch_array($SearchResult)) {
		...
		echo '<td>' . $myrow['stockid'] . '</td>
				...
				<td><input class="number" ' . ($i==0 ? 'autofocus="autofocus"':'') . ' tabindex="'.($j+7).'" type="text" size="6" name="rate'.$i.'" value="0" />
		...
		echo '<input type="hidden" name="modelnumber'.$i.'" value="' . $myrow['mnfCode'] . '" />';
		echo '<input type="hidden" name="brand'.$i.'" value="' . $myrow['manufacturers_name'] . '" />';
