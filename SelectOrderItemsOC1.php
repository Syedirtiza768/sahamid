...
$SQLline='SELECT MAX(lineno) as maxline FROM ocoptions WHERE orderno="'.$_SESSION['ExistingOrder'.$identifier].'"
		';
$resultline=DB_query($SQLline,$db);
$rowline=DB_fetch_array($resultline);
 //$rowline['maxline'];
		if(isset($_GET['DeleteLine']) AND $rowline['maxline']>0){
...
DB_query($SQLD,$db);			
 ...
 $SQL='SELECT MAX(orderlineno) as maxline FROM ocdetails WHERE orderno="'.$_SESSION['ExistingOrder'.$identifier].'"
			';
$result=DB_query($SQL,$db);
$row=DB_fetch_array($result);
...
//			$result = DB_query($sqlex
	...
	//	 $linecounter."_".$optioncounter;
	 ...
	 $sqllo = "UPDATE ocoptions SET optiontext='" . $optiontext. "',
