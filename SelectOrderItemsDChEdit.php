...
include('includes/DefineDChCartClassEdit.php');
...
if($_SESSION['orderno']==$identifier)	
		{
		$LineItemsSQL = "SELECT dcdetails.internalitemno,
								...
								WHERE  dcdetails.orderno ='" . $_SESSION['orderno'] . "'
...
// everything below here only do if a customer is selected
	...
	//	echo '</b><br />' . _('Terms') . ':<b> ' . $_SESSION['Items'.$identifier]->PaymentTerms;
		...
		echo '<table border="1"><tr><td><a href="SelectOrderItemsDChEdit.php?identifier='.$identifier .'">Bill of Quantities</a></td><td><a href="SelectOrderItemsDChexternalEdit.php?identifier='.$identifier .'">Descriptions</a></td></tr></table>';
...
$SQL = "SELECT stockmaster.stockid,
					...
					where stockissuance.salesperson = '".$_SESSION['Items'.$identifier]->SalesPerson."'
...
echo'<script>
window.location.assign("'.$RootPath.'/SelectOrderItemsDChEdit.php?ModifyOrderNumber=' . $_SESSION['ExistingOrder' . $identifier] . '&salescaseref=' . $_SESSION['salescaseref'] . '");
		</script>';
...
//	print_r ($_SESSION['Items'.$identifier]->LineItems);
 ...
 onClick=window.open("quotationhistory.php/?stockid='.$OrderLine->StockID.'","Ratting","width=550,height=170,0,status=0,scrollbars=1");>' . $OrderLine->StockID . '</a></td>
		...
		$row=DB_fetch_array($result);
					$QOHSQL = "SELECT issued AS qoh
							   FROM stockissuance
							   WHERE stockissuance.stockid='" .$OrderLine->StockID . "' AND
							   stockissuance.salesperson = '" . $row['salesman'] . "'";
					$result=		   DB_query($QOHSQL,$db);
					$row=DB_fetch_array($result);
echo '</td>
					<td class="number">' . $row['qoh'] . '</td>
					';
...
if($_SESSION['ExistingOrder' . $identifier]!=$_GET['identifier'])
	...
	//echo "<h3>Kindly insert option quantity is correct before adding the item</h3>";
	   
			echo'<a   href = "'.$RootPath.'/additemsDCh.php?line='.$line.'
			&option='.$option.'" rel="#overlay" onclick="loadScroll()" onclick="return confirm(\'' . _('Are You Sure?') . '\');">
...
//	'',$_SESSION['Items'.$identifier]->externallines[$line]->lineoptions[$option]->optiontext);
	echo'<table width="100%" cellpadding="2">
		...
		<b>Quantity: </b><input name = "quantity'.$line.'_'.$option.'" required="required"
	...
	$varoptionindex= "optionindex".$line;
		...
		//	echo'	
			//		<input type="hidden" name="'.$varoption.'" value="' . _('Add New Option') . '" onclick="saveScroll()"/>';
				if($_SESSION['ExistingOrder' . $identifier]!=$_GET['identifier'])
