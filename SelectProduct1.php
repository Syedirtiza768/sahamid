...
$SQL = "SELECT distinct stockcategory.categoryid,
		...
		and manufacturers.manufacturers_id = "."'".$_SESSION['brand']."'"."
		ORDER BY categorydescription";
...
$SQL = "SELECT distinct manufacturers_id,
		...
		where stockmaster.brand = manufacturers.manufacturers_id
		and stockmaster.categoryid = stockcategory.categoryid
		and stockcategory.categoryid = "."'".$_SESSION['StockCat']."'"."
...
if (!isset($_GET['Search']) AND (isset($_GET['Select']) OR isset($_SESSION['SelectedStockItem']))) {
	...
	if (in_array($PricesSecurity, $_SESSION['AllowedPageSecurityTokens']) OR !isset($PricesSecurity)) {
		...
		*/
	$sql = "select materialcost, lastcostupdate from stockmaster where stockid = '".$StockID."'";
		...
		echo '<tr>
				...
				<th class="number">' . _('Last List Price update') . '</th>
...
if ($Its_A_Kitset_Assembly_Or_Dummy == false) {
 ...
 onClick=window.open("quotationhistory.php/?stockid='.$StockID.'","Ratting","width=550,height=170,0,status=0,scrollbars=1");>Quotation History</a><br/>';
...
if (isset($_GET['StockCode'])) {
	echo '<input type="text" name="StockCode" value="' . $_GET['StockCode'] . '" title="' . _('Enter text that you wish to search for in the item code') . '" size="15" maxlength="18" />';
} else {
...
echo '<input type="submit" name="UpdateBrands" style="visibility:hidden;width:1px" value="' . _('Categories') . '" />';
...
and 		CAST(stockitemproperties.value as integer) > '".intval($_GET['PropValueMin'][$i])."'
"
	...
	if ($ListCount > 0) {
		...
		while (($myrow = DB_fetch_array($SearchResult)) AND ($RowIndex <> $_SESSION['DisplayRecordsMax'])) {
			...
			echo '<td>' . $ItemStatus . '</td>
				<td><a href = "SelectProduct.php?Select=' . $myrow['stockid'] . '" target = "_blank" >' . $myrow['stockid'] . '</a></td>
				
			<td title="'. $myrow['mnfCode'] . '">' . $myrow['mnfCode'] . '</td>
