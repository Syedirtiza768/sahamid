...
<?php
	...
	if(isset($_GET['NewSalesCase'])){
		unset($_SESSION['CustomerID']);
		header('Location: '.$RootPath.'/salescase.php');
		return;
	}

	if(isset($_GET['salescaseref']) && !isset($_GET['pass'])){
		header('Location: '.$RootPath.'/salescase/salescaseview.php?salescaseref='.$_GET['salescaseref']);
		return;
	}
...
//print_r($_SESSION['salescaseref1503300590']);
...
if (isset($_POST['caseclosereasonsenquiry']) AND !isset($_POST['submitsalescase']) AND !isset($_POST['updatecontacts'])
...
AND !isset($_POST['updatesalescase']))
{
	...
	$sql = 'UPDATE salescase SET closingremarks = "'.$_POST['closingremarks'].'",
								...
								closingreason = "'.$_POST['caseclosereasonsenquiry'].'",
...
if ((!isset($_SESSION['SalesCase'.$_POST['salescaseref']]->DebtorNo)
	...
	if (isset($result_CustSelect)) {
		...
		$TableHeader = '<tr>
							...
							<th>' . _('Phone') . '</th>
							<th>' . _('Fax') . '</th>
						</tr></form><form action="' . htmlspecialchars($RootPath."/salescase/salescasecreate.php", ENT_QUOTES, 'UTF-8') .'" name="CustomerSelection" method="post"><input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />';
		echo $TableHeader;
		...
		while ($myrow=DB_fetch_array($result_CustSelect)) {
			...
			echo '<td style="font-weight:bold; font-size:18px;"><input type="submit" name="Submit'.$j.'" value="' . $myrow['brname'] . '" /></td>
					...
					<td style="font-weight:bold; font-size:18px;">' . $myrow['salesmanname']  . '</td>
...
if ($_SESSION['SalesCase'.$_POST['salescaseref']]->salescasevalue==''){
	...
	';*/
//$SQLquotcount="";
$sqlqcount = 'SELECT * from salesorders where salescaseref="'.$salescaseref.'" and quotation=1';
		$resultqcount = DB_query($sqlqcount,$db);
if($_SESSION['SalesCase'.$_POST['salescaseref']]->closed == 0 and DB_num_rows($resultqcount)==0){
echo '<form name="closeatenquiryform" enctype="multipart/form-data" action="' . htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8') . '?salescaseref=' . $salescaseref.'" method="post">';
echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />';

echo'<table>';

	echo '<tr><td>Close Case </td><td><input type = "checkbox" id = "casecloseatenquiry" name = "casecloseatenquiry" onclick = "atenquiry()"><td></tr>';
	echo '</table></td>
	</tr>
	<tr><td><div id="myDIVenquiry" style = "display:none"><table><tr><td>Reason </td><td>';
	$sql="SELECT reason
		FROM caseclosereasonsenquiry
			
		";

$result=DB_query($sql, $db);
echo '<td><select name="caseclosereasonsenquiry">
		<option value="">' . _('Select a Reason') . '</option>';
while ($myrow=DB_fetch_array($result)){
	if (isset($_SESSION['caseclosereasonsenquiry']) AND $_SESSION['caseclosereasonsenquiry']==$myrow['reason']){
...
echo '</select>';
	...
	echo '<tr><td colspan="2"><input name = "closeatenquiry" type = "submit" value = "Close"></td></tr>';
	echo'</div></table></form></tr></tr>';
	echo'
	<script>
	function atenquiry()
	{
		if (document.getElementById("casecloseatenquiry").checked == true)
		{	
		document.getElementById("myDIVenquiry").style.display = "block";

		
		}
		else
		{
			document.getElementById("myDIVenquiry").style.display = "none";
...
if($_SESSION['SalesCase'.$_POST['salescaseref']]->closed == 0)
{
	/*	echo		'<tr>
		...
		';
*/
	$conx= mysqli_connect('localhost','irtiza','netetech321','sahamid');
	
	$tempSQL = 'SELECT count(*) as count FROM salesordersip WHERE salescaseref="'.$_GET['salescaseref'].'"';
	
	$resp = mysqli_query($conx,$tempSQL);
	...
	if($co['count'] == 0){
		echo		'<tr>
			...
			<td style="font-weight:bold; font-size:18px;"><a href = "'.$RootPath.'/makequotation.php?NewOrder=Yes&salescaseref='.$_GET['salescaseref'].'
		...
		echo '<a href = "'.$RootPath.'/makequotation.php?orderno='.$orderno.'&salescaseref='.$_GET['salescaseref'].'
			&selectedcustomer='.$_SESSION['SalesCase'.$_POST['salescaseref']]->DebtorNo.'&DebtorNo='.$_SESSION['SalesCase'.$_POST['salescaseref']]->DebtorNo.'
			&BranchCode='.$_SESSION['SalesCase'.$_POST['salescaseref']]->BranchCode.'
			" target = "_blank">Continue Quotation</a>';
		echo '</td>';
		echo '<td  style="font-weight:bold; font-size:18px;">';
		echo '<a href = "'.$RootPath.'/makequotation.php?orderno='.$orderno.'&discard=true&salescaseref='.$_GET['salescaseref'].'
			&selectedcustomer='.$_SESSION['SalesCase'.$_POST['salescaseref']]->DebtorNo.'&DebtorNo='.$_SESSION['SalesCase'.$_POST['salescaseref']]->DebtorNo.'
			&BranchCode='.$_SESSION['SalesCase'.$_POST['salescaseref']]->BranchCode.'
...
if($_SESSION['SalesCase'.$_POST['salescaseref']]->closed == 0 and $countrows >0)
{
		...
		echo '</tr><td><a target = "_blank" href="'.$RootPath.'/quotation/api/editExisting.php?rootpath='.$RootPath.'&orderno=' . $row_resultR['orderno'] . '&salescaseref=' . $salescaseref . '">'.'EDIT LAST Quotation no'.$row_resultR['orderno'].'</a></td>';
...
echo		'<tr>
	...
	while ($rowresultpo=DB_fetch_array($resultpo)){
		...
		if($rowresultpo['pono']!= 'Not Set Yet'){
			...
			echo '</tr><td><a href="'.$RootPath.'/oc/createOC.php?NewOrder=Yes&pono='. $rowresultpo['pono'].'&salescaseref=' . $salescaseref . '
&ocref=1&selectedcustomer='.$_SESSION['SalesCase'.$_POST['salescaseref']]->DebtorNo.'&DebtorNo='.$_SESSION['SalesCase'.$_POST['salescaseref']]->DebtorNo.'
			&BranchCode='.$_SESSION['SalesCase'.$_POST['salescaseref']]->BranchCode.'"  
...
{	
echo '<tr><td>
<a href = "'.$RootPath.'/oc/createOC.php?NewOrder=Yes&salescaseref='.$salescaseref.'
			&selectedcustomer='.$_SESSION['SalesCase'.$_POST['salescaseref']]->DebtorNo.'&DebtorNo='.$_SESSION['SalesCase'.$_POST['salescaseref']]->DebtorNo.'
			&BranchCode='.$_SESSION['SalesCase'.$_POST['salescaseref']]->BranchCode.'">
			Make OC Document</a>';
}		
if ( mysqli_num_rows($result)>0)		
		 ...
		 while ($row_result=DB_fetch_array($result))
			...
			if($_SESSION['SalesCase'.$_POST['salescaseref']]->closed == 0)
			{	
			echo '<td><a href="'.$RootPath.'/oc/makeoc.php?ModifyOrderNumber=' . $row_result['orderno'] . '&pono=' . $row_result['pono'] . '&salescaseref=' . $salescaseref . '" target="_blank"> 
	...
	//		$resultSR=DB_query($SR,$db);
		...
		if($_SESSION['SalesCase'.$_POST['salescaseref']]->closed == 0 && $row_result['inprogress'] == 1)
{
		echo'<td style="font-weight:bold; font-size:18px;"><a href = "'.$RootPath.'/dc/createDC.php?NewOrder=Yes&salescaseref='.$salescaseref.'
			&selectedcustomer='.$_SESSION['SalesCase'.$_POST['salescaseref']]->DebtorNo.'&DebtorNo='.$_SESSION['SalesCase'.$_POST['salescaseref']]->DebtorNo.'
			&BranchCode='.$_SESSION['SalesCase'.$_POST['salescaseref']]->BranchCode.'
			" target = "_blank" onclick="return confirm(\'' . _('Press OK only if you are sure to create DC') . '\');">
			Make DC</a>'
		 .'</td>
...
echo'		<table border=1><tr><td style="font-weight:bold; font-size:18px;">Uploaded DCs</td><td style="font-weight:bold; font-size:18px;">Uploaded Courier Slips</td><td style="font-weight:bold; font-size:18px;">Uploaded Invoices</td>
		...
		while ($row_result=DB_fetch_array($result))
			...
			if ($row_result['dcstatus'] == ''){	
				...
				echo "</tr><td>";
				if($row_result['inprogress'] == 1){
				
					echo '<a href="'.$RootPath.'/PDFDCh.php?DCNo=' . $row_result['orderno'] . '&salescaseref=' . $salescaseref . '">' .$row_result['orderno']. '</a>
				<br>
				...
				if($_SESSION['SalesCase'.$_POST['salescaseref']]->closed == 0 AND ($_SESSION['AccessLevel'] == 22 OR $_SESSION['AccessLevel'] == 8 OR $row_result['inprogress'] == 0))
			{
				echo'<a target = "_blank" href="'.$RootPath.'/dc/makedc.php?ModifyOrderNumber=' . $row_result['orderno'] . '&salescaseref=' . $salescaseref . '">'.'EDIT LAST DC no'.$row_result['orderno'].'</a>';
			}			
			echo'</td>
...
while ($row = DB_fetch_array($result))
{
	if($row['type'] == 0)
		...
		echo '<tr><td style="font-weight:bold; font-size:18px;">'.$row['username'].' '.$row['time'].'</td></tr>
				<tr><td><audio controls>
				  <source src="'.$row['comment'].'" type="audio/wav">
...
<tr>
	<td style="font-weight:bold; font-size:18px;">
		<textarea name = "comment" rolws="10" cols="150"></textarea>
	</td>
...
<tr>
	...
	<td style="font-weight:bold; font-size:18px;" align="center">
		<a id="audiocomment" href="<?php echo $RootPath; ?>/audioComment.php">Audio Comment</a>
	</td>
